...
- ox-grid-N 系の運用 reference は「最初に 3 段検証 (render → structure →
  ...
  - ox-grid-5 → `outputs/000_TX/刑TX/刑TX326.html`
  - ox-grid-4 → 完成後の `outputs/000_TX/刑TX/刑TX327.html`
...
- multi-select-N 系の運用 reference は「最初に 3 段検証 (render → structure →
  content) を完走した実例 HTML」とする:
  - ox-grid-5 → `outputs/000_TX/刑TX/刑TX326.html`
  - ox-grid-4 → `outputs/000_TX/刑TX/刑TX327.html`
  - multi-select-5 → 完成後の `outputs/000_TX/刑TX/刑TX328.html`
- canonical ファイル数を増やさないことで、コピー禁則対象とテンプレ役割の
  分裂を防ぐ。
...
- single-choice-N 系の運用 reference は「最初に 3 段検証 (render → structure →
  ...
  - ox-grid-5      → `outputs/000_TX/刑TX/刑TX326.html`
  - ox-grid-4      → `outputs/000_TX/刑TX/刑TX327.html`
  - multi-select-5 → `outputs/000_TX/刑TX/刑TX328.html`
  - **single-choice-5 → 完成後の `outputs/000_TX/刑TX/刑TX329.html`**
...
- combination-N 系の運用 reference は「最初に 3 段検証 (render → structure →
  ...
  - ox-grid-4         → `outputs/000_TX/刑TX/刑TX327.html`
  - multi-select-5    → `outputs/000_TX/刑TX/刑TX328.html`
  - single-choice-5   → `outputs/000_TX/刑TX/刑TX329.html`
  - **combination-5   → 完成後の `outputs/000_TX/刑TX/刑TX330.html`**
...
| 9 | `python scripts/validate_structure.py outputs/000_TX/刑TX/刑TX{326-330}.html` 順次実行 | 各 exit 0、**WARN 件数が 1 ずつ減少**、S51 が消滅 |
...
| ID | リスク | 想定影響 | 回避策 |
|---|---|---|---|
| **R1** | 文言修正で予期せぬ表示崩れ | A-2 解答エリアの UI 表示が壊れる、ボタン配置が変わる | 修正後の `outputs/000_TX/刑TX/刑TX326.html` / `刑TX327.html` を **目視確認**。文字数は微増 (約 +14 char) だが CSS の text-wrap で吸収される見込み |
...
| 7 | `python scripts/validate_structure.py outputs/000_TX/刑TX/刑TX326.html` 実行 | exit 0、WARN 件数が **8 → 7** に減少、S71-AP33 が消滅 |
...
| outputs/000_TX/刑TX/刑TX326-330.html | **変更なし**（byte 不変） |
...
| 出力先 | `outputs/000_TX/刑TX/刑TX{id}.html` | `outputs/000_TX/{科目jp}TX/{科目jp}TX{id}.html` |
...
- 出力パス: `outputs/000_TX/{科目jp}TX/{科目jp}TX{id}.html`。KEI 経路は legacy `outputs/000_TX/刑TX/刑TX{id}.html` と一致する (CP4 担保)。
...
- ✅ `outputs/000_TX/民TX/民TX001.html` / `outputs/000_TX/商TX/商TX001.html` / `outputs/000_TX/民訴TX/民訴TX001.html` 新規生成 (3 HTML)
...
- ✅ 新規 output: `outputs/000_TX/刑訴TX/刑訴TX001.html` (102,755 bytes)
...
- ✅ 新規 output: `outputs/000_TX/憲TX/憲TX001.html` (103,081 bytes)
- 🟡 保留: `problems/KEIS001.json` / `outputs/000_TX/刑訴TX/刑訴TX001.html` (8 blanks のため §6.6b 待ち)
...
- ✅ 新規 output: `outputs/000_TX/行政TX/行政TX001.html` (102,594 bytes)
