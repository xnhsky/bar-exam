...
- multi-select-N 系の運用 reference は「最初に 3 段検証 (render → structure →
  content) を完走した実例 HTML」とする:
  - ox-grid-5 → `outputs/000_TX/001_刑法/刑TX326.html`
  - ox-grid-4 → `outputs/000_TX/001_刑法/刑TX327.html`
  - multi-select-5 → 完成後の `outputs/000_TX/001_刑法/刑TX328.html`
- canonical ファイル数を増やさないことで、コピー禁則対象とテンプレ役割の
  分裂を防ぐ。
...
- single-choice-N 系の運用 reference は「最初に 3 段検証 (render → structure →
  ...
  - ox-grid-5      → `outputs/000_TX/001_刑法/刑TX326.html`
  - ox-grid-4      → `outputs/000_TX/001_刑法/刑TX327.html`
  - multi-select-5 → `outputs/000_TX/001_刑法/刑TX328.html`
  - **single-choice-5 → 完成後の `outputs/000_TX/001_刑法/刑TX329.html`**
...
- combination-N 系の運用 reference は「最初に 3 段検証 (render → structure →
  ...
  - ox-grid-4         → `outputs/000_TX/001_刑法/刑TX327.html`
  - multi-select-5    → `outputs/000_TX/001_刑法/刑TX328.html`
  - single-choice-5   → `outputs/000_TX/001_刑法/刑TX329.html`
  - **combination-5   → 完成後の `outputs/000_TX/001_刑法/刑TX330.html`**
...
| ② 形式 #6 以降の予定が 2 件以上 confirmed | **不充足** | `inputs/000_TX/001_刑法/` には 326-330 の 5 件のみ存在、331 以降の PDF / 確定仕様は無し |
...
| トリガー | 内容 | 期待される対応 |
|---|---|---|
| **形式 #6 の PDF が `inputs/000_TX/001_刑法/` に追加される時点** | `331.pdf` 以降が入力ディレクトリに現れた瞬間 | 形式調査を実施 → 既存 5 本のいずれかの派生で収まるか判定 → 収まらなければ条件 ① 充足へ |
...
| 9 | `python scripts/validate_structure.py outputs/000_TX/001_刑法/刑TX{326-330}.html` 順次実行 | 各 exit 0、**WARN 件数が 1 ずつ減少**、S51 が消滅 |
...
| ID | リスク | 想定影響 | 回避策 |
|---|---|---|---|
| **R1** | 文言修正で予期せぬ表示崩れ | A-2 解答エリアの UI 表示が壊れる、ボタン配置が変わる | 修正後の `outputs/000_TX/001_刑法/刑TX326.html` / `刑TX327.html` を **目視確認**。文字数は微増 (約 +14 char) だが CSS の text-wrap で吸収される見込み |
...
| 7 | `python scripts/validate_structure.py outputs/000_TX/001_刑法/刑TX326.html` 実行 | exit 0、WARN 件数が **8 → 7** に減少、S71-AP33 が消滅 |
...
- 本セッションシリーズ §5 案件完全完了の宣言コメント:
    ...
    **§5 配下の全消化対象案件が完走。326-330 シリーズの整備期は本書 §5.9 を
    ...
    - **形式 #6 入口での AND 条件再判定** (slotmap §5.4 §7):
      - `inputs/000_TX/001_刑法/331.pdf` 以降の追加または ranking / fill-in confirmed
...
| outputs/000_TX/001_刑法/刑TX326-330.html | **変更なし**（byte 不変） |
...
| 着手敷居 | 中：別科目の PDF 入手と JSON 化が必要。inputs/000_TX/001_刑法/ 配下に該当科目の PDF があるかの事前確認 |
...
- [ ] `inputs/000_TX/001_刑法/` 配下に憲法 PDF が存在するか確認
- [ ] 存在する場合: 問題番号と正答率を抽出して 5 問分の format 計画を立案 (slotmap §5.13 として書き起こし)
- [ ] 存在しない場合: xnh に PDF 投入を依頼 (推奨ファイル名: `inputs/000_TX/001_刑法/KEN001.pdf` etc.)
...
§5.11 §13 末尾で「候補 Y - 憲法 (KEN) 展開」を次セッションの最優先候補と判定した後、xnh から 6 科目 (KEN/MIN/SYO/MINS/KEIS/GSE) の PDF (`inputs/000_TX/001_刑法/` 直下に各 1 ファイル) を一括で投入する指示が出された。これにより、本プロジェクトは「刑法 326-330 シリーズ」段階から「7 科目横断 namespace」段階に移行する。
...
| 出力先 | `outputs/000_TX/001_刑法/刑TX{id}.html` | `outputs/000_TX/{科目jp}TX/{科目jp}TX{id}.html` |
...
- 出力パス: `outputs/000_TX/{科目jp}TX/{科目jp}TX{id}.html`。KEI 経路は legacy `outputs/000_TX/001_刑法/刑TX{id}.html` と一致する (CP4 担保)。
...
- 入力命名: `inputs/000_TX/001_刑法/{SUBJECT}.pdf` (例: `KEN.pdf`)。1 PDF = 1 科目分の複数問題、ページ数 = 問題数。
...
**当初前提の誤り**: 起案時は「1 PDF = 1 科目分の複数問題、1 ページ = 1 問題」を想定していたが、xnh が `inputs/000_TX/001_刑法/` に投入した 6 PDF (Wondershare PDFelement で抽出された画像ベース PDF) は、実際には **「1 PDF = 1 問題、1 PDF は問題ページ + 解答+解説ページの組」** という構造であった (本セッション Phase C で全 13 ページを画像読解して確認、`docs/session-6subjects-expansion.md` § O-1 参照)。
...
- `inputs/000_TX/001_刑法/{SUBJECT}.pdf` (6 ファイル、合計 6 問題、1 PDF = 1 問題)
- 出力: `outputs/000_TX/{科目jp}TX/{科目jp}TX{NNN}.html`
...
- ✅ `outputs/000_TX/003_民法/民TX001.html` / `outputs/000_TX/004_商法/商TX001.html` / `outputs/000_TX/005_民事訴訟法/民訴TX001.html` 新規生成 (3 HTML)
...
- ✅ 新規 output: `outputs/000_TX/002_刑事訴訟法/刑訴TX001.html` (102,755 bytes)
...
- ✅ 新規 output: `outputs/000_TX/007_憲法/憲TX001.html` (103,081 bytes)
- 🟡 保留: `problems/KEIS001.json` / `outputs/000_TX/002_刑事訴訟法/刑訴TX001.html` (8 blanks のため §6.6b 待ち)
...
- ✅ 新規 output: `outputs/000_TX/006_行政法/行政TX001.html` (102,594 bytes)
