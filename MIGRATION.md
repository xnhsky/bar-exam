...
**仕様書（spec/tx-v9.0.0-genkei-core.md）の GENKEI 設計と §0-quad で問題自体を解決**したため不要となった。
...
- **`spec/tx-v9.0.0-genkei-core.md` は自己完結**（§Annex A/B/C 全文埋込）。旧 `spec/tx-v8.11.6-annex-{A,B,C}.*` 等の分離ファイル、および前バージョン `spec/tx-v8.11.7-core.md` は不要。
...
本クリーン版では仕様書（`spec/tx-v9.0.0-genkei-core.md`）の §Annex B body skeleton を AI が直接適用するため不要。
