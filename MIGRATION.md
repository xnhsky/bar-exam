...
- `spec/legacy/tx-v8.11.1〜v8.11.6` の **5 世代並列**
...
- `outputs/000_TX/` 配下の **過去生成済 HTML 群**

これらは「正典引用化対策の試行錯誤」で積み上がったものだが、本クリーン版では
**仕様書（spec/legacy/tx-v9.0.0-genkei-core.md）の GENKEI 設計と §0-quad で問題自体を解決**したため不要となった。
...
| 既存 TX HTML（生成済） | `outputs/000_TX/{科目TX}/*TX*.html` | `outputs/000_TX/{科目TX}/` | 検証通過済みのもののみ移動 |
| 既存 JX HTML（生成済） | `outputs/001_JX/{科目JX}/*JX*.html` | `outputs/001_JX/{科目JX}/` | 検証通過済みのもののみ移動 |
| アップグレード対象 | `outputs/000_TX/*/[v8.10.x ファイル]` | `inputs/tx-legacy/` | `/upgrade-tx` で v8.11.1 化 |

**捨ててよいもの：**

- `spec/legacy/tx-v8.11.3〜v8.11.6-*.{md,css,html,js}` 全 24 ファイル（自己完結化により不要）
...
- `outputs/000_TX/*/_quarantine/` / `outputs/000_TX/*/_quarantine_pre_pipeline/`（隔離済み失敗 HTML）
...
foreach ($s in $txSubjects) {
    $src = "$old\outputs\000_TX\$s"
    $dst = "$new\outputs\000_TX\$s"
    if (Test-Path $src) {
...
# 行TX があれば 行政TX にリネーム
if (Test-Path "$new\outputs\000_TX\行TX") {
    Get-ChildItem "$new\outputs\000_TX\行TX\*.html" | Move-Item -Destination "$new\outputs\000_TX\006_行政法\"
    Remove-Item "$new\outputs\000_TX\行TX" -Recurse -Force -ErrorAction SilentlyContinue
}
...
Get-ChildItem "outputs/000_TX/001_刑法/*.html" | ForEach-Object {
...
- **`spec/legacy/tx-v9.0.0-genkei-core.md` は自己完結**（§Annex A/B/C 全文埋込）。旧 `spec/legacy/tx-v8.11.6-annex-{A,B,C}.*` 等の分離ファイル、および前バージョン `spec/legacy/tx-v8.11.7-core.md` は不要。
...
旧プロジェクトの `outputs/000_TX/行TX/` フォルダは `outputs/000_TX/006_行政法/` にリネームして使用する。
JX v3.2 仕様書内部のパレットアンカー名「行JX」は仕様書側の表記であり、ファイル名・フォルダ名とは独立。

### Q. 「templates/ フォルダは何だった？」

A. 旧プロジェクトで「slot 差替方式」の HTML 生成を試みたときの作業残骸。
本クリーン版では仕様書（`spec/legacy/tx-v9.0.0-genkei-core.md`）の §Annex B body skeleton を AI が直接適用するため不要。
