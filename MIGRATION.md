...
if (Test-Path "$new\outputs\000_TX\行TX") {
    Get-ChildItem "$new\outputs\000_TX\行TX\*.html" | Move-Item -Destination "$new\outputs\000_TX\行政TX\"
...
Get-ChildItem "outputs/000_TX/刑TX/*.html" | ForEach-Object {
...
旧プロジェクトの `outputs/000_TX/行TX/` フォルダは `outputs/000_TX/行政TX/` にリネームして使用する。
