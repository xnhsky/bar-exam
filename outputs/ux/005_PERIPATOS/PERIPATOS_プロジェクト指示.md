...
1. 答案構成：問題文と事実の要点を読み、入口の問い（事案の整理・見当づけ・論点）のあと、組み立ての順序（順番・論点名・理由）を読み上げる。骨子を第1から、論点名を言ってから「規範は？」「あてはめで使う事実は？」、項目の最後に「結論は？」と1つずつ組み立てる。詰まったら段階ヒント。各項目の終わりに復唱、最後に通しで口頭構成して流れのまとめで答え合わせ
