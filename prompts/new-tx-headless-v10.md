...
| 変数 | 例 | 意味 |
|---|---|---|
| `{TARGET_PDF}` | `C:\Users\xnh\bar-exam\inputs\000_TX\001_刑法\312.pdf` | 入力 PDF の絶対パス |
| `{PROBLEM_NUMBER}` | `312` | 問題番号（PDF ファイル名先頭の連続数字） |
| `{PROBLEM_ID}` | `刑TX312` | sentinel 用識別子（科目接頭 + TX + 番号） |
| `{OUTPUT_PATH}` | `C:\Users\xnh\bar-exam\outputs\000_TX\001_刑法\刑TX312.html` | 出力 HTML の絶対パス |
...
| **配色 V3 参考** | `memory/reference_palette_v3.md`（11 名前付きパレット × 5 役割） |
| **検証スクリプト** | `scripts/validate-tx-gold.py`（G1〜G16） |
| **必須 feature-tag 先頭文言** | `TX v10.0.0 GOLD-SKELETON` |
| **必須 feature-tag セット** | `genesis-baseline` / `palette-v3-11-named` / `palette: {名前} (P{N})` / `roles: base 70% / main 25% / accent 5% / sub 2` / `svg-overlap-checked` / `content-independence` / `jp-prefix-naming` |
...
- **Phase 1**：PDF 解析と配色 V3 判定（正答率帯 → P1/P2/P3、11 パレットから 1 つ AI 選定、5 色を base/main/accent/sub × 2 役割に割当て）
- **Phase 2**：ファイル名・出力先の確定（CLAUDE.md §2）
- **Phase 3**：`canonical/GENESIS.html` を Read → 対象ファイル名でコピー → **本文空文字列初期化（必須・ハード）**
  - ⚠️ **最重要・AP-42 / G13 リーク防止**：cp 直後に、問題固有の本文領域を
    **必ず空にしてから** Phase 4 を始める。in-place 編集（baseline 本文を残したまま
    上書き）は **絶対禁止**。対象テーマが baseline（311＝不法原因給付×横領／実像狙撃・
    虚像射撃）と似ていても、baseline の解説・記述・判例文を 1 文字も残してはならない。
  - 空にする本文領域（**構造シェルは残す**＝タグ/class/id/CSS/JS/sec-nav/凡例/
    part-title/footer-spec は逐語保持）：`.problem-text` 本文／各 `data-explanation` 値／
    各 `.sub-card` 本文／`.basis-card-body` 本文／PART C 全本文／PART D drill 本文／
    `.arena-premise` の見解・事案・記述／SVG 内テキスト／footer-spec 1〜3 行目
  - 手順厳守：**空にしてから {TARGET_PDF} を見て新規執筆**。baseline の本文を
    参照しながら書き換えてはならない（参照は構造シェルのみ）
...
| 科目 | 接頭辞 | 出力先 |
|---|---|---|
| 刑法 | `刑TX` | `outputs/000_TX/001_刑法/刑TX{NNN}.html` |
| 憲法 | `憲TX` | `outputs/000_TX/007_憲法/憲TX{NNN}.html` |
| 民法 | `民TX` | `outputs/000_TX/003_民法/民TX{NNN}.html` |
| 商法 | `商TX` | `outputs/000_TX/004_商法/商TX{NNN}.html` |
| 民訴 | `民訴TX` | `outputs/000_TX/005_民事訴訟法/民訴TX{NNN}.html` |
| 刑訴 | `刑訴TX` | `outputs/000_TX/002_刑事訴訟法/刑訴TX{NNN}.html` |
| 行政法 | `行政TX` | `outputs/000_TX/006_行政法/行政TX{NNN}.html` |
...
- ⚠️ **G13（baseline 本文 5-gram 一致）ERROR は「baseline の問題本文を流用した」
  という意味**。validator は構造シェルを除外し本文のみ照合するので、検出されたら
  **必ず該当本文領域（解説・記述・判例文・premise 等）を {TARGET_PDF} に基づいて
  全面的に書き直す**こと。G13 リークを未解決のまま COMPLETED_WITH_ISSUES で
  終えるのは禁止（リークは「未達」ではなく「規律違反」。3 回で消えなければ
  該当 section を baseline 構造シェルから作り直して本文を新規執筆する）
