...
`claude -p` headless 実行用 JX 問題生成プロンプト（v3.3 GOLD-SKELETON / ATHENA-CLONE）。
...
> 各問題固有で完全新規執筆**（content independence）。`outputs/jx/*/` 配下の
...
| `{TARGET_PDF}` | `C:\Users\OWNER\bar-exam\inputs\jx\刑\32.pdf` | 入力 PDF の絶対パス（`inputs\jx\{科目}\NN.pdf`） |
| `{TRANSCRIPT_PATH}` | `C:\Users\OWNER\bar-exam\inputs\jx\刑\32.txt` | **同番号の講義逐語ファイル**（whisper 文字起こし等）の絶対パス（`inputs\jx\{科目}\NN.txt`）。本問解説講義の逐語であり、論点・規範・あてはめの**第一次情報源**。必ず全文を読む |
