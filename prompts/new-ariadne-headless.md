...
3. **各部を鋳造**（spec §2〜5）：
   ...
   - **骨子**（`.bone.matrix-bone`）＝第1/第2/…の番号＋見出しを、JX019正典の**マトリクス型**で作る。各行は `.mrow`、本文は `.mcell`、中身は `.mline` に分け、ラベルは `.mstage q/r/a/c/cut`（問・論・規・当・結・削）、本文は `.mtext`。違法性・責任は必要な場合だけ一言で通過し、無理に独立行を作らない。
...
- **★ ベースカラーは難易度別に染色選定**：{SKELETON} 継承の `:root` 「▼ ACTIVE」プリセットを、問題の難易度で **EASY=ローズ(P1)／STD=クリスタルブルー(P2)／HARD=バイオレット(P3)** から1つ選び、ラベル＋値2行を差し替える（3案 hex は `:root` コメントにカタログ常時記載）。基礎・典型→EASY／中堅・頻出（正当防衛・過失共犯等）→STD／重論点・罠多・錯誤や不作為や原自行為等の難所→HARD。○×ボタンはコンパクト固定（`flex:1` にしない）。
