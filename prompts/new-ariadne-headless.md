...
4. **周回○×（10枚前後・最重要・spec §4／§9-5）**：
   ...
   - **【qa-mark 契約・2026-07-26】`.quiz-answer` 冒頭の判定マークは `<b class="qa-mark">○</b>`／`<b class="qa-mark">×</b>` と書く**（1.5em 円バッジで描画）。**qa-mark を○×1文字以外に付けない・文中の強調 `<b>` は素の `<b>` のまま**（旧 `.quiz-answer b:first-child` 円バッジは文中最初の `<b>` 長文キーワードを 1.5em 円へ押し込み縦落ちさせたため廃止＝validate A42 ERROR）。想起（recall）型の答えはマーク無しの文章で始めてよい。
   ...
   - **○×バランス（当て勘封じ）**：通常○×（想起除く）は**誤り命題（典型的な誤解を述べさせて×）を意識的に混ぜ、
     ○率 25〜75% に収める**（validate A41 WARN・spec §4）。全部を正しい命題（○）で書かない。
