# TX v9.1.0 MINDMAP 仕様書

**GENKEI-skeleton · mindmap-section · forged-from-archetype · readability-enhanced · hanging-indent-grid · single-document-self-sufficient · jp-prefix-naming · content-independence · a2-two-stage-reveal · 3-type-support · spoiler-leak-eradication · spoiler-strong-elimination · ox-grid-fa-unification · host-injection-safe**
...
## §22-quad. 論点詳細マインドマップ section（v9.1.0 新規）

> **配置上の注記**：本サブセクションは Latin 序数の自然な順序（bis→ter→quater→quinta→sexta→新規）に従い §22-sexta の直後に配置。「quad」は §22-quater との重複ではなく、4 体系層 (quadrant) を統合視覚化するマインドマップ機能の固有識別子。

### §22-quad-1. 配置と役割

- 参考｜共通根拠条文・判例 section の直後、PART C-1 の直前に配置
- section `id="mindmap"`、章番号なし（A/B/C/D の枠外）
- 役割：本問の論点を「体系×記述×根拠」の 3 軸で 1 画面に統合視覚化
- 学習者が「どの論点がどの条文・判例で論じられているか」を即座に把握

### §22-quad-2. 構造（HTML スケルトン）

```html
<section class="section" id="mindmap">
  <nav class="sec-nav"><a href="#basis">↑参考</a><a href="#c-1">↓C-1</a></nav>
  <h2 class="section-title"><span class="sec-icon">🧭</span>論点詳細マインドマップ</h2>

  <div class="figure-wrap">
    <svg viewBox="0 0 1100 900" xmlns="http://www.w3.org/2000/svg"
         role="img" aria-label="[本問テーマ] 論点詳細マインドマップ：[体系構造の要約]">
      <defs>
        <!-- 5 種 linearGradient: §22-quad-3 参照 -->
      </defs>

      [凡例：transform="translate(20,15)" 4 種ノードラベル + 右端に問題識別]
      [中心ノード：transform="translate(550,450)" ellipse rx=100 ry=52]
      [4 体系層：transform="translate(170,130)" 等 4 隅配置]
      [各体系層配下に記述・条文・判例ノードと接続線]
      [正解表示：transform="translate(550,870)"]
    </svg>
    <p class="figure-caption">図：[本問テーマ] の体系構造と各記述・条文・判例の対応関係</p>
  </div>
...
- viewBox: **1100 × 900** 固定（C-5 フローチャート SVG 720 × 700 から +25% 縦長化）
- 5 種ノードグラデーション（パレット連動・`var()` 経由）：

| ID | 0% stop | 100% stop | 用途 |
|---|---|---|---|
| centerGrad | `var(--accent)` | `var(--accent)` を 20% 暗く | 中心ノード |
| branchGrad | `var(--accent-soft)` | `var(--accent-soft)` を 15% 暗く | 体系層 |
| recordGrad | `var(--accent-3)` | `var(--accent-3)` を 10% 暗く | 本問記述 |
| statuteGrad | `#eaf6ee` | `#d4ecdd` | 条文（緑系・パレット非依存） |
| caseGrad | `#fdecec` | `#f8d7d7` | 判例（赤系・パレット非依存） |

- 中心ノード配置：cx=550, cy=450（中央）、ellipse rx=100, ry=52
- 体系層配置：4 隅 (170,130) / (930,130) / (170,770) / (930,770)
- 凡例配置：左上 transform="translate(20,15)"
- 正解表示：transform="translate(550,870)"

### §22-quad-4. フォント割当（役割別）

SVG 内 text 要素は以下の CSS class または font-family 属性で割当：

| ノード種別 | font-family | 実フォント例 |
|---|---|---|
| 中心ノード（楕円内タイトル） | `var(--font-display)` | Shippori Mincho B1 |
| 体系層タイトル | `var(--font-display)` | Shippori Mincho B1 |
| 体系層サブラベル | `var(--font-body)` | A1 Gothic |
| 記述ノードタイトル | `var(--font-display)` | Shippori Mincho B1 |
| 記述ノード本文 | `var(--font-body)` | A1 Gothic |
| 条文名 | `var(--font-statute)` | Noto Serif JP |
| 判例名 | `var(--font-judgment)` | Zen Old Mincho |
| 凡例 | `var(--font-note)` | Zen Kaku Gothic Antique |
| 正解表示 | `var(--font-impact)` | M PLUS 1p |

### §22-quad-5. ノード命名規則

- 中心ノード：本問の中心法理（例：「詐欺罪体系」「窃盗罪体系」等）
- 体系層（4 個）：問題テーマの体系的分類（例：客体論 / 行為論 / 因果論 / 罪数・隣接）
- 記述ノード：「記述ア（○正/×誤）」＋ 1 行論点要約（半角 18 字相当以内）
- 条文ノード：「N 条 M 項」＋ 法律名要約（1 行・半角 14 字相当以内）
- 判例ノード：「裁判所略称＋年月日」＋ 核心法理要約（1 行・半角 14 字相当以内）

### §22-quad-6. 記述数別の配置調整

本問の記述数に応じて 4 体系層配下の配置を調整：

- **5 記述（標準）**：体系層 4 → 記述 5 を 4 体系に分散配置（1+2+1+1 等）
- **4 記述**：体系層 4 → 記述 4 を 1+1+1+1
- **6 記述以上**：体系層を増設（5 体系層も許容）、または quadrant 内に縦並び
- **3 記述以下**：体系層 3 に減らすか、3 quadrant のみ使用

### §22-quad-7. 規律（validate-tx.py S84 対応）

- SVG は完全静的（onclick/animation/script 禁止 → AP-41/host-injection-safe 準拠）
- viewBox は 1100 × 900 を厳守（CSS で width:100% スケール）
- 全ノードは `<rect>`/`<ellipse>` + `<text>`、線は `<line>` のみ使用（path 禁止）
- ノード間の線は `stroke-opacity=".55"〜".6"` 統一
- 体系層→記述、記述→条文/判例の線は `stroke-dasharray="3,2"`（中心→体系層は実線）
...
## §31. SEVERE 自己検証 S1〜S84（v9.1.0-mindmap：S84 追加）
...
S84 : mindmap section 構造検査（v9.1.0-mindmap 専用 / version-aware）
      ...
      対象外：v9.0.0-genkei 以前のファイル（version-aware check）
      判定方法：footer-spec の feature-tag 列に "TX v9.1.0 MINDMAP" を含む
                ファイルのみ S84 を厳格適用。それ以外は S84 をスキップ。
```
...
**STEP 3**：出力先サブフォルダを §1-bis-3 対応表通り（`outputs/000_TX/{科目TX}/` 配下）に移動。S78〜S82 検証通過を確認。
...
<div class="container">
  ...
  <section class="section" id="basis">
    <nav class="sec-nav"><a href="#choice-[最終記述番号]">↑記述[最終番号]</a><a href="#mindmap">↓マインドマップ</a></nav>
  ...
  <!-- ============================================================
       論点詳細マインドマップ（v9.1.0-mindmap 新規）
       配置：参考 section の直後・PART C-1 の直前
       役割：体系×記述×根拠の 3 軸を 1 画面に統合視覚化
       規律：§22-quad-2 (HTML) / §22-quad-3 (SVG) / §22-quad-7 (S84 規律) 準拠
       ============================================================ -->
  <section class="section" id="mindmap">
    <nav class="sec-nav"><a href="#basis">↑参考</a><a href="#c-1">↓C-1</a></nav>
    <h2 class="section-title"><span class="sec-icon">🧭</span>論点詳細マインドマップ</h2>
    ...
    <div class="figure-wrap">
      <svg viewBox="0 0 1100 900" xmlns="http://www.w3.org/2000/svg"
           role="img" aria-label="[論点詳細マインドマップ説明文：例「詐欺罪論の体系×記述×根拠の対応関係」]">
        <defs>
          <!-- 5 種 linearGradient（§22-quad-3 参照）：
               centerGrad / branchGrad / recordGrad / statuteGrad / caseGrad -->
          <linearGradient id="centerGrad" x1="0" x2="0" y1="0" y2="1">
            <stop offset="0" stop-color="var(--accent)"/>
            <stop offset="1" stop-color="var(--bg-dark)"/>
          </linearGradient>
          <linearGradient id="branchGrad" x1="0" x2="0" y1="0" y2="1">
            <stop offset="0" stop-color="var(--accent-soft)"/>
            <stop offset="1" stop-color="var(--light)"/>
          </linearGradient>
          <linearGradient id="recordGrad" x1="0" x2="0" y1="0" y2="1">
            <stop offset="0" stop-color="var(--accent-3)"/>
            <stop offset="1" stop-color="var(--soft)"/>
          </linearGradient>
          <linearGradient id="statuteGrad" x1="0" x2="0" y1="0" y2="1">
            <stop offset="0" stop-color="#eaf6ee"/>
            <stop offset="1" stop-color="#d4ecdd"/>
          </linearGradient>
          <linearGradient id="caseGrad" x1="0" x2="0" y1="0" y2="1">
            <stop offset="0" stop-color="#fdecec"/>
            <stop offset="1" stop-color="#f8d7d7"/>
          </linearGradient>
        </defs>

        <!-- 凡例：左上 transform="translate(20,15)" -->
        <g transform="translate(20,15)">
          [4 種ノードのカラーキー（中心／体系層／記述／条文／判例）＋右端に問題識別文]
        </g>

        <!-- 中心ノード：transform="translate(550,450)" -->
        <g transform="translate(550,450)">
          <ellipse rx="100" ry="52" fill="url(#centerGrad)" stroke="var(--accent)" stroke-width="1.5"/>
          <text text-anchor="middle" y="6" font-family="var(--font-display)" font-weight="700" font-size="18" fill="#fff">[中心法理：例「詐欺罪体系」]</text>
        </g>

        <!-- 4 体系層：4 隅配置 -->
        <g transform="translate(170,130)">
          <rect x="-90" y="-32" width="180" height="64" rx="8" fill="url(#branchGrad)" stroke="var(--accent)" stroke-width="1.2"/>
          <text text-anchor="middle" y="-4" font-family="var(--font-display)" font-weight="700" font-size="15" fill="var(--bg-dark)">①[体系層1：例 客体論]</text>
          <text text-anchor="middle" y="16" font-family="var(--font-body)" font-size="11" fill="var(--text)">[サブラベル]</text>
        </g>
        <g transform="translate(930,130)">
          <rect x="-90" y="-32" width="180" height="64" rx="8" fill="url(#branchGrad)" stroke="var(--accent)" stroke-width="1.2"/>
          <text text-anchor="middle" y="-4" font-family="var(--font-display)" font-weight="700" font-size="15" fill="var(--bg-dark)">②[体系層2：例 行為論]</text>
          <text text-anchor="middle" y="16" font-family="var(--font-body)" font-size="11" fill="var(--text)">[サブラベル]</text>
        </g>
        <g transform="translate(170,770)">
          <rect x="-90" y="-32" width="180" height="64" rx="8" fill="url(#branchGrad)" stroke="var(--accent)" stroke-width="1.2"/>
          <text text-anchor="middle" y="-4" font-family="var(--font-display)" font-weight="700" font-size="15" fill="var(--bg-dark)">③[体系層3：例 因果論]</text>
          <text text-anchor="middle" y="16" font-family="var(--font-body)" font-size="11" fill="var(--text)">[サブラベル]</text>
        </g>
        <g transform="translate(930,770)">
          <rect x="-90" y="-32" width="180" height="64" rx="8" fill="url(#branchGrad)" stroke="var(--accent)" stroke-width="1.2"/>
          <text text-anchor="middle" y="-4" font-family="var(--font-display)" font-weight="700" font-size="15" fill="var(--bg-dark)">④[体系層4：例 罪数・隣接]</text>
          <text text-anchor="middle" y="16" font-family="var(--font-body)" font-size="11" fill="var(--text)">[サブラベル]</text>
        </g>

        <!-- 中心→体系層 接続線（実線・stroke-opacity=".55"） -->
        <line x1="450" y1="450" x2="260" y2="130" stroke="var(--accent)" stroke-width="1.5" stroke-opacity=".55"/>
        <line x1="650" y1="450" x2="840" y2="130" stroke="var(--accent)" stroke-width="1.5" stroke-opacity=".55"/>
        <line x1="450" y1="450" x2="260" y2="770" stroke="var(--accent)" stroke-width="1.5" stroke-opacity=".55"/>
        <line x1="650" y1="450" x2="840" y2="770" stroke="var(--accent)" stroke-width="1.5" stroke-opacity=".55"/>

        <!-- 各 quadrant 配下：記述（recordGrad）＋条文（statuteGrad）／判例（caseGrad）
             各ノード→親体系層へ dashed line（stroke-dasharray="3,2"） -->
        [Q1 (左上) 配下：記述ノード 1〜2 個 + 条文/判例 1〜2 個、quadrant 外側に配置]
        [Q2 (右上) 同]
        [Q3 (左下) 同]
        [Q4 (右下) 同]
        ...
        <g transform="translate(550,870)">
          <rect x="-220" y="-17" width="440" height="34" rx="6" fill="#fffae8" stroke="var(--freq-mid)" stroke-width="1.5"/>
          <text text-anchor="middle" y="6" font-family="var(--font-impact)" font-weight="800" font-size="16" fill="var(--freq-mid-deep)">正解：[正解番号]（[結論ラベル]）</text>
        </g>
      </svg>
      <p class="figure-caption">図：[本問テーマ] の体系構造と各記述・条文・判例の対応関係</p>
    </div>
  ...
  <section class="section" id="c-1">
    <nav class="sec-nav"><a href="#mindmap">↑マインドマップ</a><a href="#c-2">C-2→</a></nav>
