...
骨子は **JX019で確定したマトリクス型**を正典とする。`.bone.matrix-bone` 内に、伏せ字対象を次の class で必ずマークする：
...
- **余白・2カラム規律（JX019で確定）**：`.draft-card` は左右に十分な余白を取り、`.rel-map`/`.timeline` は左に寄りすぎないよう内側に少し逃がす。
  `.facts li` は引用 `.ph` と理由 `.cue` の2カラム（引用側をやや広め）にして、右側に大きな空白を残さない。小画面は1カラムへ落とす。
...
- **答案構成（骨子）**：`.bone.matrix-bone` を必須の新正典とする。旧来の `<pre>` 風・箇条書き風の骨子は新規生成では使わない。
