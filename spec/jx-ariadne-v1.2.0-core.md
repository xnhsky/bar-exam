# ARIADNE v1.4.0 ARENA-PURE — JX 解法ナビ＋周回 正典（現行 active）

> **【最新版・2026-07-11 確定】ARIADNE v1.4.0 ARENA-PURE が active。** 版マーカー＝`ARIADNE v1.4.0 ARENA-PURE`／
> 契約＝`ARIADNE_SLOT_CONTRACT v1.4.0 ARENA-PURE`。v1.3.0 の誌面・判例カード・placeholder 契約は全面継承し、
> **周回ドリルの「arena 純度」を正典化**した minor 改定。背景＝2026-07-11 監査：全74ファイルの周回ドリル1544枚が
> すべて `data-arena="1"`（Lexia SM-2 プール直行）で、うち**完全一致の横断重複 436枚（28%）・作法エリアは95%重複**
> （「構成順を予測してから書く」×72 等の答案入門命題が全問からプールへ量産）。原因＝旧 §12-2 が bc-wrap 5ドリルを
> 「汎用（流用可）＋data-arena」と公認・旧 A27 が枚数を強制・横断ゲート不在。確定事項：
>
> 1. **原則：arena（`data-arena="1"`＝復習プール対象）に載せてよいのは、当該問題の法的実体
>    （規範・要件・判別基準・判例の射程・条文）だけ。** 科目共通の答案方法論・受験技術（体系順・4点セット・
>    評価語・構成順の予測・重い罪から先に・配点重心/紙幅・実益薄論点の切捨て・「争いのない要件は一言」等）は
>    プールに載せない。ブラックリスト正典＝`scripts/ariadne_arena_rules.py`（METHOD_RE・単一情報源）。
> 2. **bc-wrap（答案構成の作法）のドリルは data-arena を付けない**＝ページ内確認専用（§12 本文改定済み）。
>    汎用の作法ドリル自体は教育上残してよいが、プールには流さない。
> 3. **正典のステップ例文差替**：MARCH 体系順→未必の故意（認容説）／BREAK 4点セット想起→不作為犯の実行の着手時期／
>    BREAK 評価語→排他的支配の「自己の意思によらない」設定（いずれも刑JX001 の deep-dive に根拠あり）。
> 4. **検証**：`validate-ariadne.py` **A40**（arena 方法論ドリル・bc-wrap 内 data-arena ＝ ERROR）／A13 は bc-wrap を
>    data-arena 要求から除外／A27 はコラム5のみ必須（ドリル枚数強制を撤廃）／A36 現行版＝`ARIADNE v1.4.0`。
>    **corpus 横断ゲート＝`scripts/check-ariadne-quiz-dedup.py`**（D1: 同一正規化設問が3ファイル以上=ERROR・2=WARN／
>    D2: 方法論 arena／D3: bc-arena）を `check-ariadne-canonical.py` が毎回実行（`--no-corpus` で抑止可）。
> 5. **既存一括是正＝`scripts/ariadne-arena-pure.py`**（冪等・生テキスト編集で無関係行の改行不変）：全74ファイルへ
>    適用済み（bc-arena 除去370・方法論ドリル削除112・版スタンプ更新222）。残った「自殺・自傷不可罰」×5 は
...
# ARIADNE v1.3.0 TXLEX-UNIFY — JX 解法ナビ＋周回 正典（v1.4.0 に継承）

> **【2026-07-04 確定】ARIADNE v1.3.0 TXLEX-UNIFY。** 版マーカー＝`ARIADNE v1.3.0 TXLEX-UNIFY`／
...
> 7. **検証**：`validate-ariadne.py`（A1〜A36・A36 は版スタンプ整合＝当時 `ARIADNE v1.3.0`・現行は v1.4.0/A40）＋`check-ariadne-canonical.py`
...
| 起点 | `canonical/ATHENA.html` | `canonical/ARIADNE.html`（v1.4.0 ARENA-PURE active）＋`canonical/ARIADNE.placeholder.html` |
...
- **`data-arena="1"` 必須（解法ナビ側のみ・v1.4.0）**：App.jsx:2803 で復習プール投入は `kind∈{comp,arena,stmt}` のみ。素の self-check は除外＝周回不能。`data-arena` で `kind=arena`。**bc-wrap（作法）内のドリルは逆に data-arena を付けない**（§12・A40）。
- **【arena 純度・v1.4.0 ARENA-PURE・最重要】arena に載せてよいのは当該問題の法的実体
  （規範・要件・判別基準・判例の射程・条文）だけ。** 科目共通の答案方法論（体系順・4点セット・評価語・
  構成順の予測・重い罪から先に・配点重心/紙幅・実益薄論点の切捨て・「争いのない要件は一言」等＝
  `scripts/ariadne_arena_rules.py` METHOD_RE）を arena で問うと、全問に同じ命題が複製されプールへ量産される
  （2026-07-11 監査で完全一致 436/1544 枚の実害）。方法論の確認は bc-wrap のページ内ドリルへ。
  検証＝validate A40（per-file）＋`check-ariadne-quiz-dedup.py`（corpus 横断：同一設問3ファイル以上=ERROR）。
...
- **○×バランス（当て勘封じ・2026-07-11）**：arena の通常○×（想起除く）は**誤り命題（典型的な誤解・引っかけを
  述べさせて×）を意識的に混ぜ、ファイル内で ○ に偏らせない**（目安 ○率 25〜75%・validate **A41** が WARN。
  corpus 実測は是正前 ○率70%＝常に○で7割通る状態だった）。正命題の裏返しで×を作るのが原則形
  （TX の「正命題＋典型的誤りを混ぜた真の○×」方針と同旨）。既存の是正は正解値の書換えではなく設問文の
  命題反転で行う（answer-key だけ弄ると極性反転事故になる）。
- **卒業＝別日2回正解**（correctSessions≥2）・誤答で巻き戻し。
- **件数 10〜15枚（arena＝法的実体で数える）**／手ごとに 1〜複数。能動想起（▶問い→自力→開く）も併用。
...
機械ガードは `scripts/validate-ariadne.py` A23 / A24 / A33 / **A40（arena 純度・v1.4.0）** に置き、
corpus 横断の複製は `scripts/check-ariadne-quiz-dedup.py`（`check-ariadne-canonical.py` が毎回実行）で検出する。
生成・更新した ARIADNE は必ず対象ファイル単位＋corpus で通す。
...
3. **判例 完全プロファイル**（`.basis-card.case-card`）＝1判例1カード。ヘッダ `⚖ {判例名}`＋
   ...
   **判例百選 配線（必須・§11-1-ter）**：その判例が判例百選に収録されていれば、開始タグに
   `data-hyakusen="{科目}百選{巻}-{番号}"`（例 `data-hyakusen="刑法百選I-6"`）を付す。番号は
   `references/hyakusen/_index-{科目}.md`（date→番号 索引）で判決日・裁判所種別から逆引きする。
   ARIADNE（論文）は**百選フル深度**＝事案/争点/判旨原文/射程/学説対立まで鋳造（短答Lexは簡潔アンカーのみ）。
   id は `ref-case-{裁判所略号}-{元号1字}{年}-{月}-{日}`（問題横断で一意・Lexia の caseId 結合キー）。
   **百選未収録の判例には付さない**（旧版番号・別事件番号の流用禁止＝Lexia で誤バッジ・誤マージになる）。
...
### 11-1-quater. 判例百選仕様 case-card（本文＝百選内容・2026-07-04 確定）

**方針（ユーザー指示）**：`data-hyakusen` を付した case-card は、**本文自体を判例百選の内容に揃える**
（Lexia は case-card 本文をそのまま抽出表示するので、百選仕様で書けば判例部門にその内容が出る＝
別途の取り込み機構は不要）。Lexia 側「解説・評釈を探す」ボタンは補助外部リンクとして残す。

**⚠ 著作権の切り分け（厳守）**：判例百選の**執筆者「解説」テキストは有斐閣の著作物**＝丸写し禁止。
- **【判旨】＝決定要旨/判決要旨の原文引用**（判決文そのもの＝著作権フリー・引用可）。`.judgment-text`。
- **【事実の概要】【解説の要点】【射程】＝要旨を自分の言葉で再構成**（執筆者解説の逐語転写をしない）。
- **番号・掲載頁・第N版・事件の位置づけ**は事実情報として記載可。

**一次資料**：`inputs/判例百選/{科目}{N}.pdf`。**PDF頁＝本文頁＋12**（刑法1/2とも実証）。case は見開き2頁＝
本文頁と本文頁+1（PDF＝本文頁+12, +13）。本文頁は `_index-{科目}.md` の頁列。

**case-card 本文の順序（百選仕様・gold＝刑JX001 I-6 シャクティ）**：
1. 見出し `⚖ {事件通称}（{科目}百選{巻}-{番号}）` ＋ `.freq-badge`。
2. `<p><strong>事件名/裁判所/判決日/出典（刑集・判時・判タ）/事件番号</strong>` の識別情報。
3. `【事実の概要】`＝事案の要旨（自分の言葉）。
4. `【判旨】`＝決定/判決要旨の**原文引用**（`.judgment-text`）。補足・反対意見があれば明記。
5. `【解説の要点】`＝問題の所在・学説対立・本判決の位置づけを要旨化。
6. `【射程】`＝含まれる/含まれない事案・後続判例との関係。
7. `【百選での位置づけ】`＝`{科目}百選{巻}-{番号}`・第N版・掲載頁・（あれば）調査官解説。
8. `【答案での使い方】`＝本問での使い方（問題固有・簡潔）。
9. 開始タグに `data-hyakusen` 維持。id は `ref-case-{裁判所}-{元号1字}{年}-{月}-{日}`。

同一判例が複数問に出ても、Lexia が caseId で最も濃い1件へ統合＝**フル版は1判例1回でよい**（106ユニーク）。
既存刑法 ARIADNE の一括百選化は `references/hyakusen/` の一次資料照合＋決定論適用で進める。

**誌面の質感（2026-07-04・STEP7以降 重厚化）**：判例百選プロファイルの各 `<p class="hanging">` に共有クラス
`hy-sec` ＋役割クラス（`r-jouhou`/`r-jian`/`r-hanshi`/`r-kaisetsu`/`r-shatei`/`r-hyakusen`/`r-honmon`）を付し、
**役割別カラー＋食み出しラベルタブ＋本文1字下げ**で表示する（実ラベルは【事件情報】【事案】【判旨】【解説】
【射程】【百選】【本問での使い方】＝gold の実装形。上記 3〜8 の別名も可だが実装は本ラベル群）。条文判例カードも
影・ヘッダ・`blockquote.statute` の📜条文タグで重厚化。CSS は canonical/ARIADNE.html の
`HYAKUSEN-PROFILE-RESTYLE` ブロック。既存一括反映＋新規後追いは
`python scripts/ariadne-hyakusen-restyle.py [--apply]`（役割クラス付与＋CSS注入・冪等・本文不変・LF保持）。

**番号付き問題文の2カラム化（2026-07-04）**：`<p class="pq">1　… / 2　…` のように**番号付き段落が2つ以上**ある
問題文は、連続する `.pq` を `<div class="pq-list">` の**番号｜本文の2カラム行**（`.pq-grid` ＞ 円形番号バッジ
`.pq-n` ／本文 `.pq-b`）へ再構築し、**行ごとに点線区切り＋交互背景＋本文1字下げ**にする。末尾の設問文
（上記事例…／以下の小問…）は番号「問」のピル。単一段落問題は `.pq` のまま不変。`.problem .pq{text-indent:1em}`
は温存（A30）。ツール＝`python scripts/ariadne-problem-restyle.py [--apply]`（`ARIADNE-PROBLEM-COLUMNS` CSS＋
本文再構築・冪等・LF保持・番号付き2未満は対象外）。

**旧型判例カードの見た目統一（2026-07-04）**：百選変換は primary occurrence のみを百選型(hy-sec 役割箱)に
するため、非primary/非百選の判例カードは旧型（`<h5 class="kd-label">事案/審級経過/判旨/先例との関係/射程/
学説評価/答案での使い方</h5>`＋プレーン段落＝**③型**）のまま残り、同一問題内で書式が混在する。ユーザー指示で
**③の分析構成（節・キーワード強調・xref を保持）＋④の役割別カラー箱＋食み出しタブ**へ見た目を統一する
（非破壊）。各 `<h5>` 節を `<div class="cx-sec cr-{role}">` でくくり、ラベルを食み出しタブ `.cx-lab`、本文を
`.cx-body`（1字下げ）に。役割色はラベル語から決定（事案=blue/判旨=rose/射程=violet/答案=teal/解説・学説・先例=
green/審級・補足・出典・事件情報=slate）。冒頭の裁判所/判決日/出典は「事件情報」箱。百選型(hy-sec)カードは対象外。
ツール＝`python scripts/ariadne-case-unify.py [--apply]`（`ARIADNE-CASE-UNIFY` CSS＋節ボックス化・冪等・LF保持）。

### 11-1-ter. 判例百選 → Lexia 判例部門 配線（2026-07-03 確定）

各 `.case-card` の `data-hyakusen` を Lexia が読み、リーガルリファレンス「判例」部門で判例百選バッジ＋
解説・評釈リンクを出す。同一判例は caseId（id の `ref-` 除去形）で問題横断統合され、**最も濃いカードが
ライブラリ正典**になる（ARIADNE のフル版が自動的に代表になるので重複埋め込み不要）。

- **番号索引の正典**：`references/hyakusen/_index-{科目}.md`（`百選番号｜論点｜裁判所｜年月日｜頁`）。
  刑法は完備（I 107件＋II 124件・第8版）。他科目は各冊の目次を画像化（`pdftoppm`）して Read で追補する
  （OCR は日付が乱れるため目次画像を直接 Read）。索引が無い科目は当面 data-hyakusen 見送り可。
- **既存 ARIADNE への一括後追い**：`python scripts/hyakusen-wire-ariadne.py [--file 刑JXNNN] [--apply]`。
  見出し/出典の**明記番号**＋**判決日→索引**の二系統で確定し、CONFIDENT のみ付与（冪等・LF保持・bytes I/O）。
  id 末尾の日付は流用残りで陳腐化しうるため自動確定の根拠にしない。
- **CONFLICT/REVIEW は一次資料で是正**：明記番号と索引日付が矛盾＝旧版番号・別事件番号・誤引用の兆候。
  百選PDF/索引で照合し、番号訂正・未収録明記・陳腐化 id 修正をしてから配信する（機械の自動付与に委ねない）。
- 生成後は `validate-ariadne.py`／`check-ariadne-canonical.py` で契約非破壊を確認（data-hyakusen は契約に影響しない属性）。
...
④**ステップ別 ページ内確認ドリル○×**（`.self-check-quiz`・**data-arena を付けない**＝v1.4.0 ARENA-PURE。
作法ドリルは全問共通の汎用命題なので、プールに載せると問題数ぶん複製される。その場の能動確認専用）。
...
### 12-2. 汎用と問題固有の分離（2026-07-11 v1.4.0 改定）
- **汎用（流用可・ただしページ内専用）**：5ステップの手順タイトル・5枚のドリル○×
  （＝答案構成の**転用可能な手順原理**：設計優先／重い罪先行／規範先出し／重心集中／実益薄論点の切捨て）。
  **ドリルに data-arena を付けて復習プールへ流すことは禁止**（旧規定はこれを公認しており、74問×5枚の
  同一命題がプールへ量産された＝2026-07-11 監査。A40 が ERROR で弾く）。
- **問題固有（鋳造）**：🎓教授のひとことコラム5枚（TTS由来の本問コーチング）・各 `.bc-inst`（登場人物・罪名・順序）・
  `.bc-weight`/`.bc-cap`（配点・字数）・`.box-trap`（書かないこと）・`.bc-rhythm`。
...
### 12-4. 検証（2026-07-11 v1.4.0 改定）
`validate-ariadne.py` **A27**：`.bc-wrap` 内に 🎓教授のひとことコラム ≥5 で PASS（**旧「ドリル ≥5」の枚数要求は
撤廃**＝全問同一の汎用命題を量産させる強制側だった。ドリルは任意・枚数は情報表示のみ）。
**A40**：bc-wrap 内ドリルの data-arena ＝ ERROR（作法ドリルはページ内専用）。
