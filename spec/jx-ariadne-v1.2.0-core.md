# ARIADNE v1.5.0 LOOP-MODE — JX 解法ナビ＋周回 正典（現行 active）

> **【最新版・2026-08-11 確定・ユーザー指示】ARIADNE v1.5.0 LOOP-MODE が active。** 版マーカー＝
> `ARIADNE v1.5.0 LOOP-MODE`／契約＝`ARIADNE_SLOT_CONTRACT v1.5.0 LOOP-MODE`。v1.4.0 の内容規律
> （arena 純度）と v1.3.0 の誌面・判例カードは全面継承し、**周回ごとに読む量を落とす段階フェード**と
> **周回ごとの学習プロトコル案内**を加えた minor 改定。
>
> **背景（構造的な欠陥）**：corpus 実測で ARIADNE 1 本は本文 約20,000字（**周回層 約11,500字**＝問題文644／
> 解法ナビ7手4,392／答案作法2,845／下書き＋骨子1,713／照合413／模範答案1,501 ＋ 深掘り8,531・ドリル arena9＋
> ページ内5）。**2周目・3周目も同じ物量を通る設計**で、周回教材でありながら周回ごとの減量機構が無かった。
> ユーザー実機の「1問に時間がかかりすぎて周回できない」の直接原因はここ（実力ではなく設計）。とくに
> **答案作法 2,845字は全問共通の一般論**で、v1.4.0 で復習プールからは外したが誌面からは外していなかった
> ＝75問読めば同じ作法を75回読み直す構造だった。
>
> **1. 3つの周回モード（`.wrap[data-loop="1|2|3"]`）**
>
> | モード | 表示するもの | 伏せるもの | 実測の可視文字数 |
> |---|---|---|---|
> | **1周目**（既定） | 全部 | なし（深掘り・模範答案は従来どおり details 既定クローズ） | 約8,500字 |
> | **2周目** | 問題文・登場人物・解法ナビ7手の「次の一手」＋閉じた▶・骨子・照合・模範答案 | 答案作法(`#sec-build`/`.bc-wrap`)・下書き(`.drafting`)・講師プルクオート(`.pull`)・ヒント箱(`.box-tip`)・ページ内ドリル(`.self-check-quiz`) | 約4,000字（**−53%**） |
> | **3周目〜** | 問題文・骨子・照合・模範答案 | 上記＋解法ナビ本体(`#sec-nav`/`.steps-rail`/`.step`) | 約2,400字（**−71%**） |
>
> **2. DOM 温存が鉄則**：伏せるのは **CSS の display だけ**（`data-lm-max="N"`＝N周目までは表示・末尾 JS が
> 読込時に付与）。要素を DOM から消さないので、**Lexia の復習プール抽出・`validate-ariadne` の DOM 走査・
> RX/TREE 生成は無改修で動く**（v4 LOOP-FOLD の「折りたたみは DOM 温存」と同じ理由）。実機確認＝全モードで
> DOM 内ドリル15枚を保持・ドリル採点エンジンも通常どおり動作・JS エラー 0。JS 不動作時は `data-loop="1"` の
> 既定値で全表示にフォールバックする。`@media print` では全部展開（紙では全文）。
>
> **3. 周回ごとの案内文は常時表示（ユーザー指示「毎回確認したい」）**：`.lm-guide` は現在モードのぶんを
> **必ず露出**する（トグルに隠さない）。内容は全問共通の固定文で、**問題固有スロットではない**（placeholder 契約）。
> - **1周目＝解かない**：初見の事例は考えても出てこない。出ないのは実力ではなく**型（何を・どの順で書くか）**が
>   無いから。ここで粘るのが時間を溶かす最大の原因。やること＝①人物と引っかかる行為だけ書き出す（5分）
>   ②各手で自分の一手を出す→**詰まったら5分粘らず▶を開く** ③骨子の見出しと結論一行を写す ④照合→模範答案。
>   やらないこと＝**全文起案／深掘りを最初から読む／10分以上考え込む**。ゴール＝「何を、どの順で書くか」が言える。
> - **2周目＝解く。ただし答案構成まで**。合否を分ける**論点の抜け漏れ**と**規範を言えるか**は構成までで判定できる。
>   やること＝①**5分で論点の見出しだけ**挙げる ②骨子パズルで順序と結論を組む ③照合で自己採点＝**取れた論点は終了
>   （読み返さない）** ④**外した論点だけ**その手か深掘りへ降りる。初見の問題は1周目へ戻す旨を明記。
> - **3周目〜＝3分で口頭構成**（論点→条文→規範→結論）。言えたら終わり。詰まった論点だけその場で確認。
>
> **4. ゲート三層**：①canonical＋placeholder 契約（loop-mode ブロックは逐語コピー・文言書換禁止＝作らせない）
> ②`validate-ariadne.py` **A43**（CSS区画 `ARIADNE-LOOPMODE`／UI `.loop-mode`／JS `data-lm-max`／
> `.wrap` の `data-loop="1"` 既定値／1・2・3 の `.lm-guide` と `.lm-btn` の欠落＝**ERROR**）
> ③`check-ariadne-canonical.py` が canonical の3ブロック存在を検査。
>
> **5. 既存一括伝播＝`scripts/ariadne-loop-mode.py`**（決定論・冪等・**本文不変**・改行様式保持・canonical から
> 3ブロックを逐語抽出＝単一情報源）。全133ファイルへ 2026-08-11 適用済み（1ファイルあたり +158行・
> 本文差分ゼロ・CRLF 64本を含めて改行の全行差分なし）。未適用検査は `--check`（exit 1）。
>
> **6. 併せて確認した運用（教材ではなく回し方の規律）**：1周目は写経・2周目以降は答案構成止め・全文起案は
> 月に数問だけ・間隔反復は Lexia の復習プールに任せ ARIADNE 本体の周回は月1程度。案内文がこれを毎回再提示する。
>
> 以下は v1.4.0 の記述（arena 純度の内容規律は継続有効）。
