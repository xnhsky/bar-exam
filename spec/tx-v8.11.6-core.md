# TX v8.11.6 規律本体（core spec）

**KTX301-canonical · readability-enhanced · hanging-indent-grid · spoiler-safe · a2-multi-ox-support · spoiler-leak-eradication · spoiler-strong-elimination · ox-grid-fa-unification · lessons-from-chat-002-and-007 · host-injection-safe (hotfix1)**
...
> - `spec/tx-v8.11.6-annex-A.css`（canonical CSS · 既定 P1 ローズシャンブル）
> - `spec/tx-v8.11.6-annex-A-bis-2.css`（P2 セージブラリー override · 末尾追記用）
> - `spec/tx-v8.11.6-annex-A-bis-3.css`（P3 ラベンダードーン override · 末尾追記用）
> - `spec/tx-v8.11.6-annex-B.html`（canonical body skeleton template）
> - `spec/tx-v8.11.6-annex-C.js`（canonical JS）
> - `canonical/KTX301.html`（canonical 実装例。byte-level 正典）

## §0-prime-hotfix1. v8.11.6 hotfix1（host-injection-safe）

### 背景・きっかけ

`刑TX299.html` (K299) を Lexia (host アプリ) に取り込むとクイズハンドルもリンクジャンプも全く効かない事件が発生。原因調査の結果、TX 仕様側のバグであることが判明：

**症状**：Lexia は単一HTML ファイルを iframe `srcdoc` に注入する際、`</body>` 直前にホスト用 QUIZ_TRACKER スクリプトを注入する。注入ロジックは正規表現 `result.replace(/<\/body>/i, ...)` で**最初に出現する `</body>`** をマッチさせる。

**根本原因**：v8.11.6 までの `spec/tx-v8.11.6-annex-C.js` 冒頭コメントに以下のリテラルが存在した：
...
このコメントは全 TX ファイルにバイトレベルでコピーされ、結果として全 TX ファイルの `<script>` ブロック内（先頭コメント）に「`</body>`」文字列が含まれる状態だった。Lexia の正規表現は本物の `</body>` よりも先にこの**コメント内の `</body>` リテラル**にマッチし、ホスト用スクリプトを script タグの中（コメントの途中）に注入してしまい、JS 構文が完全崩壊。クイズハンドル・リンクジャンプ・その他あらゆる JS が iframe 内で動かなくなる。

### v8.11.6-hotfix1 改訂 1 項目（host-injection-safe）

1. **`<script>` 内に `</body>` 文字列出現禁止**（AP-41 / host-injection-safe）：
   コメント・文字列リテラル・テンプレートリテラル等あらゆる箇所で、`<script>...</script>` ブロック内に `</body>` のリテラル文字列を含めてはならない。代替表記として「`body 閉じタグ`」「`</` + `body>`」「`</body\u200b>`（ゼロ幅スペース挟む）」等を使う。S77 で検出。

   この規律は host アプリ (Lexia 等) の正規表現注入バグから TX ファイルを保護するためのもの。host 側も `lastIndexOf('</body>')` を採用すべきだが、生成側でも厳密に禁止することで二重に保護する。

   修正済み canonical：`spec/tx-v8.11.6-annex-C.js` / `canonical/KTX301.html` の冒頭コメントから当該リテラルを除去済み（「body 閉じタグ直前の script ブロック内部に逐語コピー」と表記変更）。

### v8.11.5 → v8.11.6-hotfix1 minor 更新ルート

既存 v8.11.5 / v8.11.6 ファイルは以下 2 ステップで minor 更新可能：

1. `<script>` ブロック先頭コメントを上記 canonical 表記に置換
2. `python scripts/validate.py <ファイル>` で S77 が通過することを確認

---

## §0-prime. v8.11.4 → v8.11.6 差分

### v8.11.5 改訂 2 項目

1. **PART A 内の `<strong>N（XX）</strong>` 太字 完全排除**（AP-39 / spoiler-strong-elimination）:
   組合せ型問題で選択肢列挙のうち正解組合せのみが `<strong>4（ウエ）</strong>` のように太字で強調されていると、accent カラーで視覚的にネタバレが残存する。v8.11.5 で `<strong>` タグごと削除し、全選択肢を同じ書式の平文に統一（「選択肢:1（アイ）／2（アオ）／3（イウ）／4（ウエ）／5（エオ）」）。S76 で検出。

2. **ox-grid 型 FA answer-num を 1 つの数字に統一**（AP-40 / ox-grid-fa-unification）:
   v8.11.4 では ox-grid 型 FA で「正(1)判定の記述ラベル」を `<div class="answer-num answer-num-multi">` 構造で表示していたが、A-2 解答フィードバックでは「正解は 11112」と数字で表示しているため、FA との表記が不一致だった。v8.11.5 で FA も `<span class="answer-num">11112</span>` 形式に統一し、A-2 と完全一致させる。single 型と同じ単一数字レイアウトを共有し、視認性も向上。
...
| **ox-grid** | `"11112"` | `<span class="answer-num">11112</span>`(v8.11.5 で multi 構造から変更) |

### v8.11.4 までの差分（既存・継承）

3 Type 対応・a2-two-stage-reveal・spoiler-safe・multi-answer-css・spoiler-leak-eradication・AP-26〜AP-38・S64〜S75・K302-17〜21 は完全継承。

### v8.11.4 → v8.11.5 minor 更新ルート

既存 v8.11.4 ファイルは §34-septies 5 ステップで minor 更新可能。

---

## §0-prime-prev5. v8.11.3 → v8.11.4 差分（履歴・参考）
...
3. **§Annex A に §22-quinta CSS パッチ追加**:`.reveal-answer-btn` + `.answer-slot.selected` の 4 規則を §22-quater の直後・§23 cross-grid の直前に逐語追加。`spec/tx-v8.11.6-annex-A.css` を canonical 化。
...
1. **spoiler-safe canonical 確立**:すべての `<div class="final-answer">` に `hidden` 属性を必須化。`spec/tx-v8.11.6-annex-C.js` の `revealFinalAnswer()` は v8.7 以来存在するが、対応する HTML 側 `hidden` 初期宣言が欠落しており JS reveal が実質 no-op となっていた canonical 欠陥を修復。`spec/tx-v8.11.6-annex-A.css` に `.final-answer[hidden]{display:none !important}` ＋ reveal アニメーション規則を追加

2. **§22-ter answer-num-multi CSS の §Annex A 組込み**:v8.11.0 で HTML canonical のみ定義されていた `.answer-num.answer-num-multi` / `.ans-cell` / `.ans-stmt` / `.ans-val` の 5 規則を `spec/tx-v8.11.6-annex-A.css` の §22 final-answer 直後・§23 cross-grid 直前に逐語追加。P2 セージブラリー・P3 ラベンダードーンで多解答セルが背景と同化する現象を恒久解消
...
4. §31 S1〜S77 の自己検証（`scripts/validate.py` 実行）
5. 検証通過後に配信
...
1. §Annex B-link の Google Fonts `<link>` タグ（`spec/tx-v8.11.6-annex-B.html` 冒頭コメント参照）
2. `spec/tx-v8.11.6-annex-A.css` 全文
3. `spec/tx-v8.11.6-annex-B.html` の body skeleton
4. `spec/tx-v8.11.6-annex-C.js` 全文
...
| 40〜60% | P2 セージブラリー | `spec/tx-v8.11.6-annex-A-bis-2.css` の `:root{}` ブロックを `<style>` 末尾に追記 |
| < 40% | P3 ラベンダードーン | `spec/tx-v8.11.6-annex-A-bis-3.css` の `:root{}` ブロックを `<style>` 末尾に追記 |
...
`scripts/validate.py` を実行し、S1〜S77 全件通過を確認。違反があれば修正してから配信。
...
13. **`scripts/validate.py` で S1〜S77 自己検証** → 全件通過後に配信
...
- **§22-quater-3**: `spec/tx-v8.11.6-annex-A.css` に CSS パッチ正典組込み（§22-bis fa-summary 直後・§23 cross-grid 直前）。`.answer-num.answer-num-multi` / `.ans-cell` / `.ans-correct` / `.ans-incorrect` / `.ans-stmt` / `.ans-val` / `.final-answer[hidden]` / `@keyframes faReveal` の 8 規則
- **§22-quater-4**: `spec/tx-v8.11.6-annex-C.js` の `revealFinalAnswer()` は変更なし（v8.7 以来既存）


### §22-quinta. A-2 reveal-answer-btn + answer-slot.selected canonical（v8.11.2 新規）

`spec/tx-v8.11.6-annex-A.css` に CSS パッチ正典組込み（§22-quater の直後・§23 cross-grid の直前）:
...
`spec/tx-v8.11.6-annex-A.css` に §22-quinta の直後・§23 cross-grid の直前に CSS パッチ追加:
...
- **S77（v8.11.6-hotfix1 新規）**：`<script>...</script>` ブロック内に `</body>` リテラル文字列不存在（AP-41 検出）
  - host アプリ (Lexia 等) の正規表現注入バグ防止。詳細は §0-prime-hotfix1 参照。
...
v8.11.4 で生成済みの既存ファイルは §0-tri ゼロベース再構築を経ずに、以下 5 ステップで v8.11.5 化可能。

**STEP 1**: PART A `<section id="problem-text">` 内の `<strong>(\d+[（\(][^）\)]+[）\)])</strong>` 形式を `\1` に置換（strong タグごと削除・AP-39 解消）。組合せ型問題の選択肢列挙で正解のみ太字になっていた状態を解消し、全選択肢を同じ書式の平文に統一。
...
**STEP 3**: footer-spec の `TX v8.11.4` → `TX v8.11.6` 置換、`spoiler-strong-elimination` および `ox-grid-fa-unification` feature-tag を追加。

**STEP 4**: spec ファイル参照を `tx-v8.11.4-*` → `tx-v8.11.6-*` に更新。
...
### AP-41（v8.11.6-hotfix1 新規）：`<script>` 内 `</body>` リテラル混入

`<script>...</script>` ブロック内のコメント・文字列リテラル等あらゆる箇所に `</body>` のリテラル文字列を含めることは禁止。

**症状**：host アプリ (Lexia 等) が iframe srcdoc に単一HTML を注入する際、`</body>` 直前にホスト用スクリプトを注入する正規表現が**最初の出現** = script 内のリテラルにマッチ。注入が script タグ内部 (コメントの途中等) になり JS 構文崩壊。クイズハンドル・リンクジャンプ・○×ボタンなど iframe 内の **全ての JS 機能が完全に死ぬ**。
