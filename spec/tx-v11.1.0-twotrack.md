...
>
> **要点先頭・全文折りたたみ（高速判定・2026-06-25 ユーザー指示）**：各 `.ox-row` は `.ox-label ＋
> .ox-main ＋ .ox-btn-group` の3点構成。`.ox-main` の先頭に **`<p class="ox-gist">`（要点一行・記号フリー・
> 30〜50字・○×をにじませない）** を置き、自己完結命題の全文（`.ox-stmt`）は **`<details class="ox-detail">`
> に格納＝既定で畳む**。移動中は太字の要点だけで○×判定でき、全文＝Lexia 記録は無改変ゆえ G30/G31 は無改修で
> 通る。CSS は `canonical/SOLVE-NAV.html` [STYLE] 同梱。**`tip`（💡コツ）も決め手1点の一行**に凝縮する。
>
> **全科目共通・後追い整備（2026-06-25）**：この要点化は**7科目すべてに適用**する標準。新規生成は
> 上記の正典で最初からこの形を出すが、**正典更新前に作った既存 `_lex` の取りこぼし**は
> `python scripts/tx-lex-gist-backfill.py [--subject 00N_科目]` で掃き取る（CSS は機械注入、
> 要点執筆が必要な問は worklist 出力→gold=刑TX351 に倣い 1問1エージェントで執筆→`validate-tx-core` Errors:0→commit）。
> 刑法 362 問は展開済み（todo 0）。詳細は `scripts/README.md`。

### 4-1-ter. TX360 型インライン周回面（2026-06-29 正典化）

`_lex` の通常周回は、問題文を読んでから別位置の ox-grid へスクロールする導線ではなく、**問題文各肢に OX と解説を対応**させる。gold は `outputs/ux/000_TX/001_刑法/刑TX360_lex.html`。構造・CSS/JS は `canonical/GENESIS-CORE.html` の `tx-inline-*` を正典とする。

- 各肢は `.tx-inline-card[data-stmt]` とし、`.tx-inline-ox[data-stmt][data-value]` が裏の `.answer-ox-grid .ox-row[data-stmt] .ox-btn` を click して同期する。
- 裏の `.answer-area` は `inline-prototype-mode` を付け、表示上の素 ox-grid は隠してよい。ただし **Lexia 記録・answer-key・検証の単一情報源**なので削除しない。
- 各 `.tx-inline-explain` は、(1) `.tx-mini-law`（登場条文本文）→ (2) `.tx-article-flow`（文言・趣旨・射程・切断点・転用）→ (3) `.tx-cycle-aids`（記憶フック・答案圧縮）→ (4) `.tx-inline-detail`（対応 PART B 詳説トグル）の順に置く。
- 条文カードは、メイン条文＝ブルー濃、補助条文＝ブルー淡、周辺条文＝グレー。判例を入れる場合はピンク系。注釈は原文より目立たせない。
- `記憶フック` は紫系、`答案圧縮` はピンク系。どちらも一文で、決め手を思い出すための補助に限る。問題を解く前から正解番号だけが分かるネタバレ文にしない。
- `詳説を開く` は対応する PART B の `sub-card.synthesis` / `choice-points` / `basis-link` をその場に展開する。PART B は通常周回の主導線ではなく、必要時の詳説アーカイブとする。
- `解説だけ閲覧` は全5肢未選択でもインライン解説と `.final-answer` を開くが、`answered` を付けず、Lexia/SM2 の記録を発生させない。もう一度押すと閉じられる。
- `.final-answer` 冒頭に旧 `fa-summary` を置かない。順序は **`fa-narrative` → `statement-verdict-table`**。

### 4-1-quater. 問題都合ラベルから論点コア・テーゼへの置換

組合せ番号、空欄①〜⑤、選択肢(a)〜(j)、見解A/B、事例Ⅰ/Ⅱ、記述ア/イ等は、その問題を成立させるための包装であり、Lexia/SM2 に残す知識ではない。`_lex` では、これらを **論点コア・テーゼ** に置換してから `.ox-stmt`・`.tx-reflex-core`・`.tx-cycle-aids` に入れる。

- 過去問原文・組合せ表には記号を残してよい。正規化対象は **SM2 に載るテキスト**だけ。
- 見解ラベルは、学説名または見解の定義内容へ置換する。
- 空欄記号・選択肢記号は、語句・制度・要件・判例テーゼの実体へ置換する。
- 「本問」「上記」「記述ア」「肢3」依存を避け、翌月カード単体で読んでも正誤判断できる文にする。
- 正解番号・消去順・組合せの絞り込みは解法ナビまたは詳説に置き、SM2カード本文へ混ぜない。

### 4-1-bis. 出題形式 Type A / B と「議論形式は解法ナビ主役・空欄単位」（2026-06-25 ユーザー指示・最重要）

**判別の決め手**：最終解答（番号・組合せ）が**複数の小判断の集計にすぎず、各小判断こそ転用可能な知識**なら **Type A**。
要素が**それ自体で真偽が決まる独立記述**なら **Type B**。分類は `python scripts/tx-classify-format.py`（PART A 解答
選択肢のトークン構成で判定。`①a②d…`の空欄→選択肢割当＝A／カタカナ記述ラベルの組合せ・完全文＝B。`--map` でマップ出力）。

- **Type A（議論形式・空欄補充＝組合せ）**：組合せ○× ox-grid は**廃止**。**解法ナビを主役**にし、一番堅く決まる空欄
  （アンカー）から各空欄①〜N を**1つずつ2択**で誘導（会話断片＋💡決め手）。**ox-grid は空欄単位**（`data-oxgrid-mode="blank"`・
  `data-correct-value` は全 ○）＝**論点コアは `ox-core-wrap` で解答前ネタバレ防止**・各空欄の論点が **Lexia 記録単位**
  （外した論点だけ復習プールへ）。エンジン正典は `canonical/SOLVE-NAV.html` の **[SCRIPT-A]**（gold＝刑TX351）。
  既存問の作り直しは **`python scripts/tx-build-typeA.py <CODE> <DATA.json>`**（エンジン固定・問題固有データ JSON
  だけ差替・空欄⑮まで対応）。刑法の Type A 22 問は展開済み。
- **Type B（独立5択・記述ア〜オの正誤組合せ）**：記述＝学習単位が正しいので **§4-1 の要点化（要点一行＋折りたたみ）**
  のまま。
- **生成側**：`/new-tx` は Type A 問題では SCRIPT-A（空欄単位ナビ）＋空欄単位 ox-grid を出す（Type B は従来どおり）。
  詳細・ツールは `scripts/README.md`。
...
`python scripts/validate-tx-core.py <file>` を**両ファイル**に対して実行（G1〜G33 ERROR 0 件）。
...
### 6-2. Lexia/SM2 解説ペイロード（TX360 正典以後）

Lexia の SM2 は「長い解説を読む場所」ではなく、誤答した肢を再判定する場所である。したがって TX `_lex` から SM2 に渡す説明は、次の優先順位で扱う。

1. `.ox-stmt`：カード表面に近い自己完結命題。
2. `.ox-pool-explain`：既存 Lexia 抽出器が GIST/POINT として最優先で読む SM2 専用 payload。中身は `.tx-reflex-core` の文言・趣旨・射程・切断点・転用の5点セット。
3. `.ox-pool-explain` 内の記憶フック／答案圧縮：元 `.tx-cycle-aids` から短くミラーする想起補助。
4. メイン条文番号・条文本文：条文文言で切る問題では裏面に添える。
5. `.tx-inline-detail` / PART B 詳説：誤答時の展開情報。通常カードの主説明に長く載せない。

`.fa-narrative` は初回理解・全体像の救済テキストであり、SM2 の通常カード本文には載せない。必要時参照に留める。

TX360 inline 型では、`scripts/tx-sm2-payload-backfill.py` で `.tx-reflex-core`＋`.tx-cycle-aids` を各 `.ox-row` 直下の `.ox-pool-explain` へ冪等注入する。`validate-tx-core.py` G34 は `.ox-pool-explain` 欠落、`fa-narrative`/詳説混入、問題都合ラベル残留を ERROR にする。
