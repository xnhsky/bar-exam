...
- `python scripts/validate-tx-core.py <_lex>`：**v13 判定＝`.tx-v13-verdict` 検出**。v13 では
  ...
  - **G54（WARNING・2026-07-07）＝記憶のフック未変換**：`.syn-image` が旧『💭 INTUITION』（2〜3文の比喩）のまま＝
    v13m規約B（🗝記憶のフックの締めの一行標語）未達を検出（§v13m B）。旧 v13.0.0 の 44本/220カードが未変換。
    内容品質の合意で機械整形不可＝執筆で締めの一行標語へ圧縮する。充足後 ERROR 化。
