...
- **断定命題形式（★2026-07-18・G70）**：ox-stmt は**要件事実込みの断定完結命題**（文末は句点）で書く。
  **疑問形（「〜は成立しないか」）は○×の向きが読み手ごとに割れ、体言止め・電報体（「宝くじの番号
  ...
  ある」等の判定枠が読める定型は可。機械ゲート＝`validate-tx-core.py` **G70**（WARNING・末尾
  ヒューリスティック＝疑問形「か」終わり／句点なし×漢字・カタカナ・英数字・記号終わりを検出・定型は
  除外・ひらがな終わりの用言終止は句点欠落のみとして許容）＋push 前 `check-tx-lex-engine.py` の
  非ブロッキング助言。実害＝刑TX395/397（公務執行妨害罪の成否を問うことが単独カードで読めない・
