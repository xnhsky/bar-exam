...
2. `.sub-card.synthesis` … 🎯統合解説。中身の順＝
   ...
   `.syn-image`（🗝記憶のフック＝締めの一行。旧💭INTUITION の位置・class 流用）。**SYNTHESIS の外箱は平坦化**
   （枠・背景を外し、内側の GIST/記憶のフックだけ箱）。**執筆規約**（syn-lead＝やさしい版・用語サンドイッチ／
   syn-image＝🗝記憶のフックの一行標語）は `docs/tx-v12.2.1-inline-lock.md` §v13m が正典。
...
- `python scripts/validate-tx-core.py <_lex>`：**v13 判定＝`.tx-v13-verdict` 検出**。v13 では
  ...
  - **正典セルフチェック（2026-07-11）**：`canonical/GENESIS-CARD.html` 自身も `_lex` 同様に G23/G25 期待値＋
    G30-G62 の検査対象（`is_lex_target()`）。「作らせない層」の正典が v13 ゲートの検査対象外だった穴を恒久修正。
