...
> - コマンド：`.Codex/commands/new-tx.md`（v11）。検証：`scripts/validate-tx-core.py`（G1〜G26）／
...
- **生成コマンド**：`.Codex/commands/new-tx.md`（v10.0.0 GOLD-SKELETON 経路の全工程）
...
詳細は `.Codex/commands/new-tx.md` 参照。要約：
...
- **実レイアウト（入力）**：`inputs/001_JX/{00N_科目}/重問PDF/{n}.pdf` ＋ `inputs/001_JX/{00N_科目}/講義逐語/{科目名}_重問{nn}.txt`
  （科目フォルダは 2026-06-20 に 00N_科目 へ統一。AGENTS.md 旧記載の「同フォルダ同居・NN.txt」は実態と異なる。マニフェストが正典）。
...
> Codex 側がこの規約に従って黙って 3 種まで仕上げる。**「JX だけ作って副産物は省略」は禁止**（§7）。
...
- **リモート（Codex on the web）**：`/new-jx` の **Phase 9** が `Agent` サブエージェントを
  RX → TREE → ARIADNE の順に起動して同じ副産物を生成する（バッチランナーの別 `Codex -p` 相当）。
...
**目的：** 講義逐語の番号は動画通し番号で PDF 問題番号と別系統（AGENTS.md §4 取り込み
...
詳細は `.Codex/commands/` 配下の各 `*.md` を参照。
...
- headless 起動は巨大プロンプトを **stdin パイプ**で `Codex -p` に渡す（`-p 引数`渡しは PowerShell が壊す・nested 実行で顕著）。
...
Codex Web セッションで並行運用する。過去、各セッション／PC が**古い `master`
...
1. **本線は `master` の一本**。入力 PDF・spec・code はすべて `master` に集約する
   （HTML 成果物も 2026-06-05 から git 管理。生成＝コミットで永続化＝AGENTS.md §9）。
...
全工程は `.Codex/commands/batch-tx.md` 参照。
...
5×2 モードはバッチ 1 完了後に必ず `/exit` & `Codex --resume` してから
...
リモート実行環境（Codex on the web）は **ephemeral でコンテナが回収**される。
