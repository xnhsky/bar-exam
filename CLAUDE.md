...
| 入力 PDF | 科目 | シリーズ | 出力 |
|---|---|---|---|
| `inputs/tx-pdfs/299.pdf` | 刑法 | TX | `outputs/000_TX/001_刑法/刑TX299.html` |
| `inputs/jx/民/15.pdf` ＋ `inputs/jx/民/15.txt` | 民法 | JX | `outputs/001_JX/003_民法/民JX015.html` |
| `inputs/tx-pdfs/予備R1-16詐欺.pdf` | 刑法 | TX | `outputs/000_TX/001_刑法/刑TX001.html`（※最初の連続数字「1」を採用） |
| `inputs/jx/憲/5.pdf` ＋ `inputs/jx/憲/5.txt` | 憲法 | JX | `outputs/001_JX/007_憲法/憲JX005.html` |
| `inputs/tx-pdfs/民訴.pdf`（数字なし） | 民訴 | TX | **処理中断 → 番号確認** |

> **JX 入力レイアウト（2026-06-06 確定）：** 問題 PDF と同番号の講義逐語を
> **科目フォルダに同居** させる ── `inputs/jx/{00N_科目}/NN.pdf` ＋ `inputs/jx/{00N_科目}/NN.txt`
...
> 番号一致ペアリングが初めて正しく機能する。刑法は `inputs/jx/001_刑法/重問PDF/NN.pdf` ＋
> `inputs/jx/001_刑法/講義逐語/刑法_重問逐語NN.txt`（NN は PDF と一致）に整備済み・正典は
> **`inputs/jx/001_刑法/逐語-PDF対応表.md`**。番号抽出は `Get-TranscriptNumber` が
...
> `inputs/jx/001_刑法/逐語-PDF対応表.md` 付録）。**番号を結合キーにしてはならない。中身だけが正。**
> 新規科目の取り込みは必ず次の順で行う：
...
> 4. 結果を `inputs/jx/{00N_科目}/逐語-PDF対応表.md`（科目別正典）に記録し来歴も残す。
...
1. 問題 PDF を読解（実レイアウト：`inputs/jx/{00N_科目}/重問PDF/{n}.pdf`）。**手順0で解決した講義逐語を必ず併読**（逐語が論点・規範・あてはめの第一次情報源）。刑法の逐語は `inputs/jx/001_刑法/講義逐語/刑法_重問逐語NN.txt`（内容照合で PDF と同番号に改名済み・§2 注記）。**併読の冒頭で逐語の事案と PDF の事案が一致するか自己照合**し、不一致なら使わず中断・報告する（番号ミス検知の照合ガード）
...
- **実レイアウト（入力）**：`inputs/jx/{00N_科目}/重問PDF/{n}.pdf` ＋ `inputs/jx/{00N_科目}/講義逐語/{科目名}_重問{nn}.txt`
