...
10. **処理済 PDF 削除（§4-5②）**：`scripts/jx-cleanup-pdf.sh {科目} {番号} --commit` → 再 push（HTML が commit 済のときのみ削除＝安全ガード）
...
| ② | **処理済 PDF 削除**（git 管理） | `scripts/jx-cleanup-pdf.sh` | HTML が **commit 済のときのみ** `git rm`（履歴に残り復元可）。既定 dry-run・`--commit` で実行 |
| ③ | **回収動線**（リモートコンテナ→git push） | `scripts/jx-push.sh`（＋`scripts/jx-retrieval-manifest.py`） | add→commit→push を1コマンド＋指数バックオフ再試行。push 後に GitHub URL 付き回収一覧を表示 |

- **実レイアウト**：`inputs/jx/{科目}/重問PDF/{n}.pdf` ＋ `inputs/jx/{科目}/講義逐語/{科目名}_重問{nn}.txt`
