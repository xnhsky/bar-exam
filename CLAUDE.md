...
> **【省エネ規律・2026-07-02 恒久】この内容レビューは「執筆した本人（同一エージェント/セッション）が丁寧に1回、自己照合する」方式で行う。**
> 別エージェントによる多重（例：5並列）敵対的レビュー、機械整形後の際限ない再修正ループ、生成やり直しの多重掛けは常態化しない
> （トークン消費の主因はこれらであり、機械ゲート＝validate 系ではない）。品質は執筆段階で一文一文丁寧に作り込み、被り・接ぎ木を執筆時点で出さないことで担保する。
> 安いpython機械ゲート（validate-tx-core／check-duplicates／check-tx-lex-engine／check-tx-reuse／check-lexia-book-quality＝トークンほぼ0）は必ず通す。詳細は feedback メモリ `feedback-lean-verification-author-once`。
>
