...
|---|---|---|---|
| **TX** | 短答式（5択・○×・組合せ）の単問解説 HTML | **v13.1.0 LOOP-CARD**（active）：`canonical/GENESIS-CARD.html`＋`GENESIS-CARD.placeholder.html`／`spec/tx-v13.1.0-loopcard-core.md`（＋内容規律 `docs/tx-v12.2.1-inline-lock.md` §v13m）。既存 v12 資産の保守は `GENESIS-CORE`／`GENESIS-DEEP` | 1 問 = 公式＋Lexia `_lex`（別冊は必要時） |
...
> **【最重要・2026-07-03 更新】新規生成・v13 化は v13.1.0 LOOP-CARD が active。** 唯一起点＝
> `canonical/GENESIS-CARD.html`（gold=刑TX359）＋`canonical/GENESIS-CARD.placeholder.html`（スロット契約）、
> 構造 spec＝`spec/tx-v13.1.0-loopcard-core.md`、内容規律＝`docs/tx-v12.2.1-inline-lock.md` §v13m。
> **版は固定記憶でなく `docs/canonical-lineage.md` の active 行で確定する。** 以下の v11/v12 記述は設計の
> **基盤（継承）と既存資産の保守用**であり、新規は上記 v13.1.0 経路（`new-tx`）で行う。
...
**JX は入力原本（PDF＋逐語）を Drive に常在バックアップし、inputs から削除**する（画像重で
git 肥大化するため）：
> **注（2026-07-07 方針変更）：** かつては TX の抽出PDF も同型に「Drive バックアップ→inputs
> 削除」していたが、**TX は削除せず inputs に残す**方針へ変更した（§9「TX 入力 PDF の保持」）。
> この項の「inputs から削除」は **JX 限定**の運用として読むこと。
...
| コマンド | 用途 |
|---|---|
| `/new-tx <PDFパス>` | 新規 TX を問題 PDF から生成（**active v13.1.0 LOOP-CARD ＋二系統出力**：公式 `000_TX`＝本物5択／Lexia 用 `ux/000_TX/..._lex`＝統合解説＋📚BASIS＋体系マップSVG＋正誤マーキング＋解法ナビ。GENESIS-CARD＋`canonical/SOLVE-NAV.html`・G1〜G45＋**G50**を含む validate-tx-core＋check-tx-lex-engine） |
...
### TX 入力 PDF の保持（2026-07-07 方針変更・ユーザー確定）

**TX の抽出 PDF は生成後に削除せず `inputs/000_TX/{00N_科目}/` に残す**（従来は「Drive へ
バックアップ→inputs から削除」だった。§4-5-bis 冒頭の「TX と同型」参照はこの旧運用を指す）。
変更理由：

1. **「どの問題を処理済みか」の追跡は outputs＋分類器で完結**する（削除を進捗マーカーにする
   必要がない）。`scripts/tx-classify-type.py` が `outputs/ux/000_TX` を走査して版・特殊型・
   **ローカル PDF 有無**まで機械判定するので、inputs の PDF を消しても進捗は分かる。
2. **PDF が無いと直せない**。特殊型（≠5記述・組合せ・穴埋め等）の v13 化は決定論ラッパー
   非対応で **PDF からの R 再生成が唯一の道**（`docs/tx-v13-migration-targets.md`）。360-445 の
   特殊型 backlog は「PDF が Drive のみ＝repo に無い」ためリモートで再生成できず滞留していた
   （2026-07-07 実証）。最新法令・判例更新や誤り訂正でも原典 PDF が要る。
3. **リポジトリ肥大化は TX では軽微**（短答抽出 PDF は画像重の JX より小さい）。JX は従来どおり
   Drive のみ（`repo ミラーは git 肥大化回避で対象外`・§4-5-bis）で据え置く。

運用：
- TX 生成・バッチ後に inputs の PDF を `git rm` しない（**削除は opt-in の明示操作のみ**）。
- Drive の「抽出PDF」（445 問原本）は引き続きバックアップとして常在。**repo に PDF が無い
  番号を最新化するときは、Drive → `inputs/000_TX/001_刑法/` へ回収してから R 再生成する。**
- 現状 repo のローカル PDF は 000〜330。**360-445 等は Drive から回収して補完対象**にする。
