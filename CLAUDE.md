...
1. 問題 PDF を読解（`inputs/jx/{科目}/NN.pdf`）。**同番号の講義逐語を必ず併読**（逐語が論点・規範・あてはめの第一次情報源）。刑法は `inputs/jx/刑/講義逐語/刑法_重問逐語NN.txt`（内容照合で PDF と同番号に改名済み・§2 注記）。**併読の冒頭で逐語の事案と PDF の事案が一致するか自己照合**し、不一致なら使わず中断・報告する（番号ミス検知の照合ガード）
