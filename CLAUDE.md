...
|---|---|---|---|
| **TX** | 短答式（5択・○×・組合せ）の単問解説 HTML | **_lex＝v13.1.0 LOOP-CARD**：`canonical/GENESIS-CARD.html`（＋`GENESIS-CARD.placeholder.html` スロット契約）／`spec/tx-v13.1.0-loopcard-core.md`＋`docs/tx-v12.2.1-inline-lock.md`（§v13m/§v13n 含む）。公式5択の複製起点は `GENESIS-CORE.html`（v12.2.1）＋`GENESIS-DEEP.html` | 1 問 = 公式＋Lexia `_lex`（別冊は必要時） |
...
> ARIADNE は **v1.4.0 ARENA-PURE**（`canonical/ARIADNE.html`／`canonical/ARIADNE.placeholder.html`／`spec/jx-ariadne-v1.2.0-core.md` 冒頭 v1.4.0 節）が active。
> Claude正典の模範答案を維持し、JX019 型のマトリクス答案構成・RX配線・本文字下げ/2カラムを正典とする。
> 生成時は固定HTML/CSS/JSを崩さず、AI判断は問題固有スロットと難易度別 ACTIVE ベースカラー選択に限る。
> **周回ドリルの arena 純度（v1.4.0）**：`data-arena="1"`（Lexia 復習プール対象）に載せてよいのは当該問題の
> 法的実体（規範・要件・判別基準・判例の射程・条文）だけ。科目共通の答案方法論・受験技術は arena 禁止
> （`scripts/ariadne_arena_rules.py`）、作法エリア（bc-wrap）のドリルは data-arena を付けない。
> ゲート＝validate-ariadne A40＋corpus 横断 `check-ariadne-quiz-dedup.py`（同一設問3ファイル以上=ERROR）。

### 新セッション入口ルール（番号指定だけで最新版へ流す）

> **ユーザーが新しいセッションで短く「TX355-360を生成」「JX001-010を作って」「既存TX356をアップデート」とだけ言った場合も、正式な作業依頼として処理する。**

- **最初に必ず `docs/canonical-lineage.md` を読む。** 固定記憶の版名ではなく、同ファイルの active 行から現在の TX / JX 正典を決める。
- **「生成」「新規」「作って」** は、指定番号または範囲の新規生成を意味する。TX は active の `canonical/GENESIS-CORE.html`、JX は active の `canonical/ATHENA.html` を唯一の複製起点にする。
- **「既存」「更新」「アップデート」「最新版に」** は、既存 HTML を active 正典へ合わせる作業を意味する。旧ファイルへ局所パッチを継ぎ足すのではなく、必要に応じて PDF/逐語/既存内容を照合し、最新版の構造・CSS・JS・検証ゲートへ移行または再生成する。
- 番号範囲は `355-360`、`355〜360`、`355から360まで`、`355番から360番まで` を同義に扱う。3桁ゼロ埋めと科目別出力先は §2 に従う。
- TX / JX の区別、科目、入力 PDF/逐語の所在が文脈や番号から特定できる場合は自律的に進める。特定不能な場合だけ、ユーザーに1点確認する。
- TX は v12.2.1 表示LOCK、解説品質レビュー、最新法令・判例・学説レビューを必須とする。JX と ARIADNE も active 正典と最新法令・判例・学説レビューを必須とする。
- 生成・更新後は該当 validator と横断チェックを通し、内容レビューを終えてから、関連ファイルだけをコミット・push する。
...
> **【最重要】新規生成の active 正典は `docs/canonical-lineage.md` の active 行で決める（2026-07-11 現在：_lex＝v13.1.0 LOOP-CARD・`canonical/GENESIS-CARD.html`＋placeholder 契約・`spec/tx-v13.1.0-loopcard-core.md`）。**
> 以下の v11.0.0 記述（2026-06-13 当時 active・正典 `spec/tx-v11.0.0-core.md`）は歴史的経緯＋既存 v11 問の保守用。
...
>
> **【二系統出力・2026-06-24 確定】TX 1 問＝2 ファイル。** 正典 `spec/tx-v11.1.0-twotrack.md`。
> - **公式**：`outputs/000_TX/{科目}/{接頭辞}{NNN}.html` ＝ 本物の5択（single/組合せ番号）。Lexia は取り込まない。
> - **Lexia 用 `_lex`**：`outputs/ux/000_TX/{科目}/{接頭辞}{NNN}_lex.html` ＝ ox-grid（記述単位○×）＋**解法ナビ**。Lexia はこれだけ取り込む。
> - 本文・PART B/B+・参考条文判例・SVG は両者共有。違いは answer-area と `_lex` の解法ナビのみ。
> - 解法ナビ正典＝`canonical/SOLVE-NAV.html`（エンジン逐語コピー＋問題固有データのみ）。new-tx Phase 4h が両ファイルを出力。
...
>
> **【v12.2.1 表示LOCK・2026-07-01】** TX355-360 実地修正を正典化。詳細は
...
> `解答を表示` 後、回答操作パネルに大きな「あなたの答え／正解／相違」サマリー箱を出さない。回答比較は問題カード内の判定欄と解説冒頭の正誤表に集約する。
> 問題カード内の判定欄は○×ボタン下に2行で置き、本文を左に圧迫しない。表示は `あなた ×` / `正解 ○` のように短くし、重厚なバッジで窮屈な長文ラベルにしない。
> 解説冒頭には正誤表を常時表示し、`あなた` 列でユーザー回答と正解を比較する。表の論点コア列は各肢1文の要約だけにし、`文言・趣旨・射程・切断点・転用` の詳細は各設問の解説本文に残す。
> トーストは廃止する。正解行は緑背景、誤答行はピンク背景、未回答は淡色で表示する。
> 回帰防止は `validate-tx-core.py` G45 と `check-tx-lex-engine.py`。更新・新規生成後は G45 を通す。
>
> **【v13m 解説執筆規約・2026-07-06】記述カード解説の中身の書き方（構造不変・内容規律）。** 正典
> `docs/tx-v12.2.1-inline-lock.md` §v13m。①`.syn-lead`（💡THE GIST）＝**やさしい版（用語サンドイッチ）**＝
> 専門用語は直後に生活語で開き、核用語は1ブロック1〜2個・初出のみ太字（用語スープ禁止）。②`.syn-image` の役割を
> 💭INTUITION→**🗝記憶のフック**（締めの一行標語・class は syn-image 流用）へ。③`.tx-v13-trap`（⚠️間違いやすい
> ポイント）に**横串・誤解の罠を積極投入**（「民法＝観念的／刑法＝現実的」等の横串・似た別論点の混同フラグ・
> 差がつく引っかけを先回り）。trap 無し記述には同型で1枠新設可。**判例は年月日を一次確認・民法で説が割れる点は
> 断定回避・通説は“判例”と書かない**。④テキストのマップ短縮版は体系マップ SVG と重複するため作らない。初例＝
> 刑TX362_lex。⑤**帰結の書き切り（2026-07-14）**＝×記述の解説を「◯◯罪は成立しない／△△条の問題となる」で
> 終えず、**成立する罪名（条・項）と、なぜそちらになるかの理由（やさしい版1〜2文）まで書き切る**。GIST・
> あてはめ・帰結・`.ox-pool-explain`（プールカードは単独表示）が必須配置。不可罰が正しい帰結なら「不可罰」と
> 明示し罪名を捏造しない（§v13m E・初例＝刑TX378 記述3）。**既存展開＝380-445 は 2026-07-14 改修済み**
> （30ファイル全記述監査・25問修正・4問は書き切り済み確認、＋刑TX381 プールgist極性是正）。**それ以外の既存
> ファイルは TJR 付随で消化**（ユーザー指示 2026-07-14）。**【2026-07-07 完遂＋恒久ゲート化】刑法 v13 全 corpus（44本220カード）へ横串 trap・🗝記憶のフックを
> 展開完了し、起点 `canonical/GENESIS-CARD.html` も完全準拠化した。以後は機械ゲートで強制**：`validate-tx-core` の
> G51（BASIS空箱）/G52（横串trap欠）/G53（相互リンク配線切れ）/G54（syn-image が💭のまま）を**ERROR 化**し、
> push 前ゲート `check-tx-lex-engine.py` にも G50-G54 を組み込んだ（三層＝canonical で作らせない／validate で弾く／
> push 前に止める）。①THE GIST＝やさしい版（§v13m A）は機械化困難のため執筆者本人の自己照合で担保する。
>
> **【○×答えの規約と反転防止・2026-07-09・最重要】`_lex` の ox 答えは常に「提示文（命題）の真偽」であり、
> 「事例の法的帰結（成立／不成立）」ではない。** ○＝その記述（ox-stmt／インライン肢カードの提示文）が法的に正しい・
> ×＝誤り、で固定する。事例の帰結記号（成立＝○/不成立＝×）を answer-key に流用してはならない（組合せ・空欄補充問題を
> ox-grid へ変換する際に頻発した反転の主因）。**同一 data-stmt では `.ox-stmt`（Lexia 復習プールが読む全文）と
> `.tx-inline-card`（通常周回の主UI）は同一命題で 1 つの answer-key を共有する**ため、両者の結論（成立/不成立）は必ず
> 同じ向きにする。片方だけ言い回しを反転させると、一方の面で正答が誤答表示になる（実害：刑TX368 記述1・2＝ox「成立
> しない」×inline「成立する」、刑TX362 エ・オ）。**新規生成・既存更新・伝播後は、各記述で ox-stmt と inline カードの
> 結論が一致し、その ○×が answer-key と整合するかを執筆者本人が1回自己照合する。** 恒久ゲート＝`validate-tx-core.py`
> **G60**（ox-stmt↔inline カードの結論極性の反転を ERROR で弾く・疑問/批判文・多罪混在はスキップの誤爆ゼロ設計）を
> push 前 `check-tx-lex-engine.py` にも組み込んだ（三層防御）。ただし「全符号化が一致した意味的な反転」（例：全記述を
> 正しい向きで書き帰結記号を答えに流用）は G60 でも捕捉できないため、上記の自己照合が最終安全網（§7 も参照）。
>
> **【ox-stmt 要件事実の自己完結・2026-07-13】`_lex` の `.ox-stmt`（Lexia 復習プール全文）は、原文を知らない
> 読者がその一文だけで○×を判定できる要件事実を保持する。** Lexia は 30 字以上の ox-stmt を原文フォールバック
> 無しの単独カードで出題するため、**①主体の身分（公務員/医師等）②目的要件（行使の目的等）③客体の官公性
> （市立/公立＝公文書性）④権限・承諾**を圧縮時に省略しない。規範命題型は罪名プレフィックス（「公務員職権濫用罪
> にいう〜」等）を削らない。正典＝spec 第5-bis-2項。実害＝刑TX378 記述3（「X市立病院の事務長を務める公務員」→
> 「病院事務長」で公文書性が判定不能・同型 32 記述/18 ファイルを 2026-07-13 一括是正）。機械ゲート＝
> `validate-tx-core.py` **G65**（WARNING・原文↔ox-stmt の重要語差分・言い換え保存は誤検出として無視可）＋
> push 前 `check-tx-lex-engine.py` の非ブロッキング助言。執筆時は「この一文だけで○×を付けられるか」を
> 執筆者本人が1回自己照合する（省エネ規律 (b) の一項目）。
>
> **【ox-stmt 断定命題形式・2026-07-18】`_lex` の `.ox-stmt` は要件事実込みの断定完結命題（文末は句点）で
> 書く。疑問形（「〜は成立しないか」）は○×の向きが読み手ごとに割れ、体言止め・電報体（「宝くじの番号
> 改ざん→有印私文書変造罪」「共同正犯。乙のみ正当防衛の状況」）は判定対象の述語ごと消えるため禁止。**
> 設問の枠組み（見解の根拠選択・本文の結論参照 等）に判定対象がある問題は、枠組みを各 ox-stmt の文内に
> 取り込んで自己完結させる。共有事例型は事例の要件事実（主体の身分・関係・経緯）を各 ox-stmt に織り込む
> （肢の原文自体に重要語が無いため G65 語彙差分では構造的に検出できない＝2026-07-13 是正の漏れ経路）。
> 正典＝spec 第5-bis-2項「断定命題形式」の節。実害＝刑TX395/397（公務執行妨害罪の成否を問うことが復習
> プール単独カードで読めない・実機報告）ほか 380-445 帯 22 ファイルを 2026-07-18 一括是正（刑訴TX001-048
> は準拠済みを確認・**380 未満の同型（体言止め・疑問形・要件事実欠落）は TJR 付随の再生成で消化**＝
> ユーザー指示 2026-07-18）。機械ゲート＝`validate-tx-core.py` **G70**（疑問形「か」終わり／
> 句点なし体言止め＝WARNING・末尾ヒューリスティック・「〜との記述」等の定型除外・誤検出は無視可。
> **「〜との内容/とする内容」の枠組み断片だけは定型に含めず ERROR**＝旧除外で刑TX405/413 が素通りし
> 一問一答カードで判定不能・実機報告 2026-07-18 同日是正）＋push 前
> `check-tx-lex-engine.py`（WARNING は非ブロッキング助言・**G70 ERROR は push 停止**）。意味の自己完結（要件事実の充足・極性の正しさ）は機械化
> できないため、執筆者本人の自己照合（この一文だけで○×を付けられるか＋answer-key と極性一致か）が最終安全網。
>
...
>
> **【体系マップ親ツリー見出しの文字重なり防止・2026-07-16・LEX-395】親ツリー（L1 カード）の見出し帯は
> 正典形＝1本の `<text x="0" y="23" text-anchor="middle">`＋`<tspan>`（見出し fs18＋全角スペース＋サブ見出し fs16）
> で書く。`<text>` 2本を同一ベースラインに固定 x で並置する旧形（見出し x=-198／サブ x=-120 等）は禁止**＝
> 見出しが約 78px（fs18 で 4.3 文字）を超えた分だけ2本目に文字が重なり判読不能になる。実害＝刑TX395_lex
> 「職務の適法性」×「要件・現在性」（30px 重畳）・刑TX125_lex（完全重畳）・公式 刑TX293.html を含む
> 計 10 ファイル/16 件（2026-07-16 監査で全是正・公式も SVG を共有するため両系統を直す）。再発根因＝
> v13k-bis のセンター化はツール（`tx-lex-sysmap-center.py`）があるだけで検出ゲートが無かったこと。
> 恒久対策三層＝①`GENESIS-CARD.placeholder.html` 契約に 2本並置禁止を明記（作らせない）／
> ②`validate-tx-core.py` **G69**（同一行の `<text>` 重なりを幾何判定＝`tx_sysmap_geom.iter_text_collisions`・
> 重なり 6px 超=ERROR・2〜6px=WARNING）／③push 前 `check-tx-lex-engine.py` に G69 組込み（push を止める）。
> 修復は決定論ツール **`scripts/tx-sysmap-fit.py`**（重なりを含む sysmap の見出し帯 2 本組を正典形へ統合・
> 非衝突ペアも同 svg 内なら統一・fs15 型や公式 000_TX にも効く・冪等・CRLF 保存・`--all` は公式も走査）。
> G69 が ERROR を出したら同ツールで統合してから push する。
>
...
> **論点単位で 1 枚執筆→物語のラベル段落直後＋対応カードの `.syn-path` 直後へ同一複製**（`data-dgm` 同期）する。
> 対象は**効く論点だけ**（対比・裏命題ペア・分岐・時系列）＝導入/まとめへの強制なし（**偽図解の禁止**）。体系マップ
> 縮小版・横断マトリクス複製は作らない。図解内の ○/✕ は法的帰結の表現で、記述の正誤（G60「命題の真偽」）とは別層。
...
>
> **【答案圧縮＝記述原文統合・GIST自己完結・#basis空箱禁止・2026-07-28・§v13q】** 周回時は物語解説を
> 読み返さない実運用に合わせ、記述カードを単体完結させる正典改定（初例＝刑TX425_lex・試作→ユーザー承認）。
> ①**✍答案圧縮**（`.tx-anscomp-line`・シルバー食み出しタブ小箱）＝正しい規範の完全文（規範＋条文・判例の
> 答案調1〜2文）を**カード記述原文（`.syn-orig`）末尾と正誤表 `data-brief-mark` 末尾の2箇所に同一文**
> （シングルソース）で置く。×記述の `.tx-stmt-fix` は最小訂正へ短縮（完全文の二重掲載禁止）。CSS＝
> GENESIS-CARD 第1 `<style>` の `TX-ANSCOMP:BEGIN〜END` 区画（複製で自動継承）。体系マップ札の✍一行は
> 短縮形・答案圧縮は完全文（二段階表示）。②**THE GIST 自己完結**（§v13m A 増補）＝結論先出し→体系的
> 位置づけ→専門用語（用語サンドイッチ）→わかりやすい例え（記憶のフックと同系イメージ）→本記述の当否。
> ③**#basis 空箱禁止**＝最新法令ノート不使用の問題は `<section class="section" id="basis" hidden=""></section>`
> の不可視アンカー（「↑ページ先頭へ」だけの白箱を出さない）。恒久ゲート三層＝placeholder 契約／
> `validate-tx-core.py` **G73**（片置き・カード⇄正誤表の文不一致・CSS区画欠落＝ERROR／未展開・可視空箱＝
> WARNING 助言）／push 前 `check-tx-lex-engine.py`（G73 ERROR は push 停止）。自己照合に「答案圧縮＝
> カード⇄正誤表同一文」を追加。**既存展開は TJR 付随で消化**（内容執筆を要するため決定論ツール化しない）。
> 正典＝`docs/tx-v12.2.1-inline-lock.md` §v13q・`spec/tx-v13.1.0-loopcard-core.md` 第2項/第5項。
>
> **【共有前提の非表示・再発と検出ゲート・2026-08-02・LEX-439・§v13n】共有事例型の `_lex` は、事例/事案/語句群を
> 必ず不可侵原文ブロック（`.tx-original-block`）へ置く。`#part-a` 直下の `.problem-text` に置くと
> `#part-a:has(.tx-inline-card) > .problem-text{display:none}` に巻き込まれ、周回画面から事例が丸ごと消える。**
> 実害＝**刑TX439**（【事例】バッグ・実機報告）／**刑TX395**（【事案】職務質問）／**刑TX218**（【記述】空欄本文＋
> 【語句群】A〜F）＝刑TX374 型の再発。根因＝**G61/G62 はブロックが有る前提の検査**（マーカー字下げ・記述数一致）で、
> 「ブロックを作らなかった」型は設計どおりスキップされ機械検出が不在だったこと。恒久ゲート三層＝①placeholder 契約
> （共有事例型はブロック必須）②`validate-tx-core.py` **G74**（インラインカード有り×ブロック無し×記述再掲でない
> 長文 `.problem-text` 残存＝ERROR・是正後 corpus 642 本で検出 0＝誤爆ゼロ実測）③push 前 `check-tx-lex-engine.py`
> に G74 組込み（push 停止）。長い記述はブロックへ二重掲載せず問い＋事例だけを載せる（刑TX401 前例・`.tx-charge`
> 不使用なら G62 はスキップ）。CSS 区画が無い旧世代 `_lex` は正典 `GENESIS-CARD.html` の該当区画を逐語注入する。
>
> **【設問・問題文エリアの原文不可侵＋正解の先出し禁止・2026-08-04・§v13r・ユーザー指示】設問（問い）と
> 問題文エリアは、原則として PDF 原文のまま置く（要約・言い換え・補足を混ぜない）。解答前に見える場所へ
> 正解・正誤を書かない。** ①`#part-a` 直下のリード段落 ②`.tx-original-block`／`.tx-original-lead`
> ③`.solve-nav`（解法ナビ）の3領域が対象。実害＝**刑訴TX002**（設問リード「（正しい組合せは肢5＝ウオ）」＋
> 解法ナビ「（本問は正しいものの組合せ＝ウオ＝肢5を選ぶ問題）」がどちらも周回画面に常時表示・実機報告）。
> 素の `<p>` リードは `.problem-text` ではないため `#part-a:has(.tx-inline-card) > .problem-text{display:none}`
> に当たらず**周回中ずっと出続ける**のが露出経路。**○×判定の操作指示は書いてよい**（「各記述それぞれの正誤
> （○×）を判定する」）＝禁止は「どれが○か」だけ。原文の「…の組合せは、後記1から5までのうちどれか」は
> 原文なのでそのまま可。原文と操作指示は混ぜず、原文＝`.tx-original-lead`／操作指示＝編集注記の二層に分ける。
> **一問一答（`.ox-stmt`・記述カード）はこの原則の対象外＝独自に書いてよい**（ユーザー指示 2026-08-04。
> Lexia 復習プールが単独カードで出すため、G30 記号フリー・G65 要件事実自己完結・G70 断定命題形式が逆に
> **書き換えを要求**する。原文不可侵は「表示用の設問・問題内容エリア」に限る）。
> **設問・問題内容への編集文の混入も禁止**（要約・言い換え・付け足し）。ただし要約か否かは原文（PDF）と
> 突き合わせないと機械判定できないため ERROR にできず、試験原文には現れない編集語（「を素材に」「組み替え」
> 「ここでは」「がコアです」「この教材」「下の一問一答」「もとの短答問題」「原題」「本問は」「周回する」）
> だけを **G19 WARNING** で可視化する（corpus 実測 77 本）。二層化の初例＝**刑訴TX002_lex**
> （原文＝`.tx-original-lead` に「…正しいものの組合せは、後記1から5までのうちどれか。」＋選択肢一覧
> 「1　アエ　2　アオ…」／操作指示＝別段落「各記述それぞれの正誤（○×）を判定する。」）。
> 恒久ゲート三層＝①`GENESIS-CARD.placeholder.html` の §v13r 契約（作らせない）②`validate-tx-core.py`
> **G19**（旧 G19 は「正解はN」型リテラルしか見ず ＝記法・組合せ露出を全て素通りしていた穴を拡張。
> 「肢1〜5」の原文レンジと「誤り＝×」の○×凡例は除外＝corpus 実測で誤検出 0）③push 前
> `check-tx-lex-engine.py` に G19 組込み（push 停止）。**既存の露出＝42 ファイル**（`_lex` 26／公式 16）は
> TJR 付随で消化。**原文層（`.tx-original-lead`）を持つ `_lex` は 69/664 のみ**で、残りは設問が言い換え
> リードだけ＝原文復元は入力 PDF からの再生成（R ストリーム）で消化する。
>
...
> 内容レビューする。機械検査 PASS は構造OKの意味であり、条文・判例・正誤理由・事案対応の正確性を保証しない。
> 問題文事実、条文番号・項、判例名・判旨、結論、文言/趣旨/射程/切断点/転用の対応を個別照合する。
> 内容レビュー未了のファイルは v12.2.1 完了・配信可・push可と扱わない。
> **【省エネ規律・2026-07-02 恒久】内容レビューは「執筆した本人（同一エージェント/セッション）が丁寧に1回、自己照合する」方式で行う。** 安全網は次の3段だけ：
> **(a) 安いpython機械ゲート**（validate-tx-core／check-duplicates／check-tx-lex-engine／check-tx-reuse／check-lexia-book-quality＝トークンほぼ0）を必ず通す。
> **(b) 執筆者本人の自己チェック1回＝内部整合**（条文番号/項符合・label↔body 一致・ox-stmt正誤と結論一致・記号フリー）。
> **(c) 的を絞ったWeb一次確認**（判例名・年月日・裁判所種別・学説か判例か・実在は内部整合では検知不能。**確信の持てない/非著名/下級審の引用だけ**裁判所裁判例検索・e-Gov・百選等で確認。著名判例は不要。上記【最新法令・判例・学説レビュー】の履行手段でもある）。旧本文（Codex期）流用時は判例誤りを継承しうるので (c) を省かない。
> **廃止＝別エージェントの多重（例：5並列）敵対的レビュー、機械整形後の際限ない再修正ループ、生成やり直しの多重掛け**（トークン消費の主因はこれらであり、機械ゲート＝validate 系や (c) の的絞りWeb確認ではない）。品質は執筆段階で一文一文丁寧に作り込み、被り・接ぎ木を執筆時点で出さないことで担保する。詳細は feedback メモリ `feedback-lean-verification-author-once`。
>
> **【最新法令・判例・学説レビュー・必須】** TX は司法試験対策教材なので、生成・更新時点の最新条文、
> 最新判例、主要学説を必ず確認する。法令は e-Gov 法令検索、司法試験の法令基準は法務省の当該年度
> 実施情報/Q&A/試験用法文、判例は裁判所裁判例検索と最近の裁判例を一次確認先にする。裁判所サイトは
> 全判決を網羅しないため、最高裁判例集・判例百選・重要判例解説・主要判例DB等で補完する。学説は
> 最新版の基本書、判例百選、重要判例解説、法学教室・法学セミナー等で補完し、機械検索だけで確定しない。
> 新旧差分がある場合は本文を現在法/最新判例ベースへ更新し、旧法・旧判例・旧説は
> `tx-current-law-note` コラムで、確認日・参照ソース・旧処理・現在処理・立法経緯/改正経緯・改正趣旨・試験対策上の扱いを短く補足する。
> 最新確認未了のファイルは v12.2.1 完了・配信可・push可と扱わない。
>
...
10. **入力は削除しない＝恒久保管（2026-07-09 方針転換・§4-5②・§4-5-bis）**：入力 PDF＋逐語は `inputs` に恒久保管する（旧「Drive バックアップ後に inputs から `git rm`」は撤回）。手順9の commit/push で成果物を永続化するだけで、`jx-finalize.ps1`／`jx-batch-runner.ps1 ⑦`／`night-batch-runner.ps1` は**入力を削除しない**（削除経路は恒久無効化済み）。どうしても手動で消したい時のみ `scripts/jx-cleanup-pdf.sh` を明示実行（自動経路からは呼ばない）
...
| ② | **入力は削除しない＝恒久保管**（2026-07-09 方針転換） | `scripts/jx-cleanup-pdf.sh`（手動専用・自動経路からは呼ばない） | 入力 PDF＋逐語は inputs に恒久保管（git 非管理＝`.gitignore`）。`jx-finalize.ps1`②／`jx-batch-runner.ps1 ⑦`・②-bis／`night-batch-runner.ps1` の削除は恒久無効化。手動で消す時のみ cleanup を明示実行（既定 dry-run） |
...
### 4-5-bis. Drive バックアップの運用＋入力は削除しない（2026-07-09 方針転換／旧 2026-06-08）

**【2026-07-09 方針転換・ユーザー確定】入力原本（PDF＋逐語）は削除せず `inputs` に恒久保管する。**
旧運用（TX の「抽出PDF」と同型に Drive へバックアップしてから inputs から `git rm` で消す）は撤回した。
Drive へのバックアップ（ミラー）は引き続き任意で行ってよいが、**inputs 側は消さない**。git 肥大化は
`.gitignore`（`inputs/000_TX/**/*.pdf`・`inputs/001_JX/**/重問PDF/*.pdf`・`…/講義逐語/*.txt|*.md`）で回避する。

- **配置（⑥ `jx-deploy.ps1`）が成果物と入力を Drive へミラー**する（バックアップ用途・削除の前提ではない）。配置先：
  - HTML → `2 JX_論 文\{00N_科目}\`（従来どおりフラット）
  - PDF → `2 JX_論 文\{00N_科目}\重問PDF\`／逐語 → `…\講義逐語\`（Drive ミラー）
  - TTS 台本/音声 → `2 JX_論 文\A_重問耳トレ\{N 科目}\…`（従来どおり）
- **永続化（⑦ `jx-finalize.ps1`）**：HTML＋TTS＋副産物を git commit/push（GitHub バックアップ）する**だけ**。
  入力 PDF＋逐語の `git rm` は**恒久無効化**した（⑦は `-NoCleanup` 付きで jx-finalize を呼ぶ／jx-finalize 側も②を常時 continue でスキップ）。
- **手動削除（例外）**：どうしても消したい時のみ `scripts/jx-cleanup-pdf.sh {科目} {番号} --commit` を明示実行（自動経路からは呼ばない・既定 dry-run）。
...
> 完結扱いにしない。操作者が副産物を指示しなくても（OWNER PC 操作者は副産物の存在を知らない前提）、
> Claude 側がこの規約に従って黙って 3 種まで仕上げる。**「JX だけ作って副産物は省略」は禁止**（§7）。
> 取りこぼしは下記 ② autofill 常駐スイープが最終的に回収するが、それは保険であって**第一義は生成時に必ず作る**こと。
>
> **秘密裏運用の徹底（2026-06-22・ユーザー指示）：** 副産物は **JX 生成に不可分に組み込んで黙って作る**。
> - **操作者に確認を取らない。**「副産物を生成しますか？」「RX/TREE/ARIADNE も作りますか？」等と**聞かない**
>   （確認を挟むと存在を気づかせてしまう）。指示が「JX 作って」「JX＋TTS だけ」でも、確認なしで 3 種まで作る。
> - **OWNER PC 操作者向けの完了報告に副産物を一切出さない。** 報告は操作者が頼んだ範囲（JX／TTS）だけに留め、
>   RX/TREE/ARIADNE の生成有無・枚数・欠落・エラーを書かない（疑問を抱かせない）。`副産物`という語を出さない。
> - **報告先は xnrg2 PC（副産物を知っている側）だけ。** autofill の補完結果は `logs/autofill-report.md` に残し、
>   xnrg2 で確認する。OWNER PC には何も言わない。
...
- **ARIADNE v1.2.0 PLACEHOLDER-LOCK（2026-06-29・正典固定化）**：JX019 で合意したマトリクス型答案構成を
  ...
  に組み込み済み。
  最新法令・判例・学説レビューも必須。旧法/旧判例/旧説と現在法/最新判例/現在の有力説に差分がある場合は、
  深掘り層の関連カード付近へ `ariadne-current-law-note` を入れ、確認日・参照ソース・旧処理・現在処理・
  立法経緯/改正経緯・改正趣旨・試験対策上の扱いを明記する。最新確認未了の ARIADNE は生成完了・配信可・push可と扱わない。
- **ARIADNE v1.4.0 ARENA-PURE（2026-07-11・周回ドリルのプール純度）**：2026-07-11 監査（74ファイル1544枚中、
  完全一致の横断重複436枚＝28%・作法エリア95%重複・全問に答案入門命題が平均6枚ずつプール直行）への恒久対策。
  **arena（`data-arena="1"`）に載せてよいのは当該問題の法的実体だけ**＝科目共通の答案方法論（体系順・4点セット・
  評価語・構成予測・重い罪先行・配点重心 等）は arena 禁止（単一情報源 `scripts/ariadne_arena_rules.py`）、
  bc-wrap 作法ドリルは data-arena 無し（ページ内確認専用）。ゲート三層＝validate-ariadne **A40**（per-file ERROR）／
  corpus 横断 `check-ariadne-quiz-dedup.py`（同一設問3ファイル以上=ERROR・`check-ariadne-canonical.py` が毎回実行）／
  生成規律（`prompts/new-ariadne-headless.md` 3-ter・4）。既存一括是正＝`scripts/ariadne-arena-pure.py`（冪等・
  生テキスト編集で無関係行の改行不変）。**「答案の書き方」一般論のドリルを復習プールへ流すことは以後禁止**（§7）。
...
| コマンド | 用途 |
|---|---|
| `/new-tx <PDFパス>` | 新規 TX を問題 PDF から生成（**_lex＝v13.1.0 LOOP-CARD ＋二系統出力**：公式 `000_TX`＝本物5択／Lexia 用 `ux/000_TX/..._lex`＝v13 記述カード＋ox-grid＋解法ナビ。GENESIS-CARD＋`canonical/SOLVE-NAV.html`・G1〜G64 の validate-tx-core・共有事例型は §v13n 不可侵原文ブロック） |
| `/deepen-tx <NNN>` | 深掘り別冊 `-deep.html` を後追い生成（誤答データ解禁時・GENESIS-DEEP・validate-tx-deep） |
| `/batch-tx <番号 or PDFパス>` | 連続生成（new-tx の active v12.2.1 経路を継承） |
| `/rb <N-M>` | **リモートバッチ**：N番〜M番を active v12.2.1 で連続生成＋各問 git commit/push で永続化（「346-350 を RB して」で起動） |
| `/new-jx <PDFパス>` | 新規 JX ファイルを active v4.0.0 LOOP-FOLD で生成 |
| `/upgrade-tx <HTMLパス>` | legacy 専用。通常の「既存TXを最新版に更新」は入口ルールの active 正典移行として処理し、このコマンドへ寄せない |
| `/validate <HTMLパス>` | TX または JX を自動判別して検証実行（修正なし） |

詳細は `.claude/commands/` 配下の各 `*.md` を参照。

### 6-2. 実行パターン（号令＝TJR に一元化・2026-07-04 確定）

> **旧パターン（TX-MARCH / TX-PICK / JX）は廃止し、号令を `TJR処理` 1 本に統一した。** チャットで
> 「**TJR処理 刑訴**」のように科目名を添えて指示する。一覧・コマンドは **`docs/run-patterns.md`** が正典。

TJR は「大元の号令＝指揮者」で、4 ストリームを 1 号令で束ねる（**同時起動＝1作業ツリーの git 衝突回避のため F→T→J→R→Q を直列自動実行**）：

| 記号 | ストリーム | 動作 | エンジン |
|---|---|---|---|
| **F** | 修復（エラー品・未完成品の回収・2026-07-24 新設） | 毎バッチ**先頭**で `tjr-audit.py` が全科目監査：ペア欠け/途切れ/プレースホルダー残骸/サイズ異常/未コミット残骸を検出→**検証PASSは回収コミットのみ・破損は入力から修復再生成**（G66/G69 のみは `tx-sysmap-fit.py` で決定論修復）。**同一問題 2 回失敗で ESCALATE**（`logs/tjr-repair-report.md`）＝無限再生成防止。対象ゼロなら監査だけで素通り | `scripts/tjr-audit.py`＋`tx-v13-runner.ps1 -RepairIds`／`jx-batch-runner.ps1 -RepairNumbers` |
| **T** | 新規TX（フロンティア前進） | `inputs/000_TX/{科目}` 未生成のうち**公式最大番号より先**を最若番から **v13 二系統（公式＋Lexia `_lex`）** 生成→両検証→各問 commit/push（過去帯の欠番は R の領分） | `scripts/tx-v13-runner.ps1` |
...
- 号令：「**TJRを○バッチ処理**」＝`TJR.ps1 -Batches N`。**1バッチ＝T:12問/J:3問/R:3問/Q:10問 固定**（2026-07-18 ユーザー確定・
  勝手なチャンク拡大／自動完遂ループ禁止）。科目は **T・J・R ストリーム別に**優先順で自動充当（毎バッチ再解決＝尽きた科目から
  次へ自動で移る）。「**TJR処理 刑訴**」＝刑訴を最優先し、仕事の無いストリームだけ優先順へフォールスルー。
  **R は過渡ストリーム＝全科目を遡って旧版_lexが無ければ「該当なし」スキップが正常**（無理に仕事を探さない）。
- **F（修復）は毎バッチ自動で同走**（2026-07-24 ユーザー指示「エラー品・未完成品を検出して新規生成と同時並行で修正」）：
  生成が途中で死んだ／validate ERROR で commit されず残った HTML は「存在する」ため T/J/R から不可視＝放置される
  構造穴を、F が毎バッチ先頭で回収する。上限＝**TX修復:3問（`-MaxF`）/JX修復:1問（`-MaxFJx`）/バッチ**・回収コミットは無制限。
  「同時並行」は 1 号令内で修復と新規生成の両方が自動で進む形（プロセス並列は git 衝突実害の記録により不採用）。
  修復だけなら `-Only F`。詳細（対象判定・ESCALATE・安全設計）は `docs/run-patterns.md` の F 節が正典。
- **号令なら指定外も当然に処理（番号ピン）**：「**TX355 を TJR処理**」＝`-TxFrom 355 -TxTo 355`（T=355固定・**J/R は通常**）／
  「**JX5 を TJR処理**」＝`-JxFrom 5 -JxTo 5`（J固定・T/R通常）。1ストリームだけは `-Only T|J|R|F|Q`（短縮形「TX 355-360 処理」＝`-Only T -TxFrom 355 -TxTo 360`）。
- **科目検知順（ユーザー指示・フォルダ番号順とは別）**：①刑法 ②刑事訴訟法 ③民法 ④民事訴訟法 ⑤商法 ⑥憲法 ⑦行政法。
...
- **「保守的書き換え」**（既存コードを引き継ごうとする AI の癖）
  > 正典改定→既存アップデートで「接ぎ木（旧エンジン流用＋後付けパッチ）」が頻発する。
  > 原因と恒久対処（決定論的 recanon／整合ガード／push 前ゲート常駐／gold 基準）は
  > **`docs/canonical-revision-migration-playbook.md`** が正典。既存一括移行は LLM 手編集ではなく
  > recanon スクリプト（`scripts/tx-lex-recanon.py` 等）で土台だけ載せ替える（本文不変・冪等）。
- ファイル名・出力先サブフォルダの **§2 規則違反**（レガシー K/MIN/KEN 等の混在禁止）
- **【TX 二系統・2026-06-24】TX を公式だけ／`_lex` だけで完結扱いにし、一方を省くこと**
  （new-tx Phase 4h・spec `tx-v11.1.0-twotrack.md`。1 問＝公式 `000_TX`（本物5択）＋ `_lex` `ux/000_TX`（ox-grid＋解法ナビ）。
  `_lex` の ox-stmt を `①b②c…` の**記号のみ肢**にすること＝G30 違反。解法ナビは `canonical/SOLVE-NAV.html` のエンジンを逐語コピーし問題固有データのみ記述）
- **【○×反転・2026-07-09】`_lex` で答えを「命題の真偽」以外の意味で付けること**
  （事例の帰結記号（成立＝○/不成立＝×）を answer-key に流用する／同一 data-stmt で `.ox-stmt` と `.tx-inline-card` の
  結論を逆向きに書く＝片面で正答が誤答表示になる。§3【○×答えの規約と反転防止】・`validate-tx-core.py` G60・
  push 前 `check-tx-lex-engine.py` で ERROR。実害＝刑TX368/刑TX362）
- **【設問ネタバレ・2026-08-04】解答前に見える設問エリアへ正解・正誤を書くこと**
  （設問リード／不可侵原文ブロック／解法ナビの3領域。「正解＝肢3」「正しい組合せは肢5＝ウオ」「誤り＝ウ・エ」
  「いずれも正しい＝○」等。§3【設問・問題文エリアの原文不可侵】・`validate-tx-core.py` G19・
  push 前 `check-tx-lex-engine.py` で ERROR。実害＝刑訴TX002）
- **【設問の改変・2026-08-04】設問・問題文エリアを原文から言い換え・要約・補足すること**
  （原則は PDF 原文のまま。○×判定の操作指示は編集注記として別に書く＝原文と混ぜない。
  `.ox-stmt` は対象外＝G30/G65/G70 が書き換えを要求する別レイヤー）
...
- 第 22 項チェックリスト未履行
- **JX を HTML（＋TTS）だけで完結扱いにし、副産物 3 種（RX/TREE/ARIADNE）の生成を省くこと**
  （§4-6 最上位原則・JX 生成は副産物まで含めて 1 セット。どの経路・どんな指示でも 3 種まで作って push する）
...
ユーザーが「TX355-360を生成」「JX001-010を作って」「既存TX356をアップデート」のように短く指示した場合は、入口ルールに従い、active 正典を確認して自律的に進める。`/new-tx inputs/000_TX/001_刑法/299.pdf` のような明示パスがある場合はそのパスを優先するが、短い自然文だからといって旧コマンドや legacy 経路へ落とさない。
...
### 作成日時スタンプの保険＝pre-commit フック（2026-06-23 追加）

Lexia は問題HTML本文のフッター `lexia-genmeta`（`Generated: YYYY-MM-DD HH:MM ／ <版>`）を
raw 取得して生成日時・版を読む（GitHub API レート制限回避）。このスタンプは本来
`jx-push.sh`（リモート）／`jx-finalize.ps1`（バッチ `-Finalize`）内の `stamp-created-date.py`
で刻むが、**生成セッションがそれらを経由せず素の `git commit` をすると stamp が走らず**、
スタンプ無し HTML が GitHub に push される事故が起きた（実害：刑JX064〜070＋副産物）。

そこで**コミット経路に依存しない保険**として pre-commit フックを用意した：

- 本体：`scripts/git-hooks/pre-commit` → `scripts/stamp-staged.py`。ステージ済みの Lexia 取込
  HTML（`outputs/000_TX`・`001_JX`・`ux`／top-level `references`）で `lexia-genmeta` が
  無いものだけを **現在時刻(JST)** で刻んで再ステージする（冪等・既スタンプは無変更）。
- **有効化（手動・各 PC で一度だけ・冪等）**：`bash scripts/install-hooks.sh`（`core.hooksPath` を
  `scripts/git-hooks` に設定）。OWNER PC／xnrg2 PC／リモートそれぞれで実行する。
- **自動有効化（手動実行が不要になる経路・2026-06-23）：** ローカル PC が普段回す入口に冪等
  インストールを内蔵した＝`jx-batch-runner.ps1`（JX バッチを回すたび）と `rx-arb-autofill.ps1`
  （常駐タスクが pull 後に 2h ごと）が起動時に `git config core.hooksPath scripts/git-hooks` を
  自動設定する。よって**両 PC とも master を pull してバッチ/常駐タスクを動かせば、手動 install
  を踏まなくてもフックが入る**（設定は冪等・一度効けば永続）。
- これにより jx-push.sh でも素の git commit でも**必ず**作成日時が入る。既存ファイルの後追い
  刻印（git 初出コミット日時）は従来どおり `python scripts/stamp-created-date.py` が担う。
