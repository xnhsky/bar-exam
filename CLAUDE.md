...
### 6-1-bis. 既存展開の優先順＝学習の区切りで2レーンに割る（2026-08-31・ユーザー指示・一般則）

> **正典改定を既存ファイルへ後追い展開するときの対象の選び方は、この一本に統一する。**
> S ストリーム（CONTEXT 帯）に限らず、**以後の「既存分の修正依頼」全般に適用する**。
> 詳細は `docs/run-patterns.md`「既存展開の優先順（学習の区切りで2レーンに割る）」が正典。

学習の進度で対象を 2 つに割り、**両方を毎バッチ並行して流す**（既定 10 本ずつ）。
①**学習レーン**＝学習の区切りより先を、学習順（②刑訴 ③民法 ④民訴 ⑤商法 ⑥憲法 ⑦行政法）で若番から。
②**過去分レーン**＝学習の区切りまでを、学習順のうち学習済みの帯（①刑法 → ②刑訴…）で若番から。
別々に上限で切ってから合流させるので、量の大きい過去分（刑法だけで `_lex` 427 本）が学習レーンを
押し出さない。片方を優先順に並べるだけだと必ずどちらかが飢えるため、この形にしている。

**区切りの単一情報源＝`scripts/v13v-runner.ps1` 冒頭の 2 行**（`$FullyLearned` / `$LearnedUpTo`）。
学習が進んだらここだけを書き換える（レーンごとの番号境界は自動導出）。**番号をスクリプト本体や
プロンプトへ直接書かない**（散らばると次に学習が進んだとき直せなくなる）。2026-08-31 時点は
`$FullyLearned = @('刑法')` ／ `$LearnedUpTo = @{ '刑訴' = 74 }`＝刑法は一巡・刑訴は TX074 まで学習済み。
新規生成（T/J/R）の科目検知順（①刑法〜⑦行政法）は別系統で、ここで割っているのは既存展開の優先順だけ。

**修正依頼を受けたときの型**＝①gold を 1 本作って型を承認してもらう ②土台（CSS/エンジン）は決定論
ツールで一括伝播する ③**内容の執筆はこの 2 レーンで消化する** ④再発防止の機械ゲートを足す。
