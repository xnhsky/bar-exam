...
> 組込み（ERROR は push 停止）。**是正済み＝原文ポインタ 11 ファイル**（刑訴TX016/020/089/090/158/188/
> 197/234/235/266/269＋刑TX415）**・裸見解ラベル 3 ファイル**（刑TX170・刑訴TX070・刑TX415）。
> **残件（WARNING・23 ファイル）は TJR 付随で消化**＝一覧は **`docs/tx-pool-selfcontained-audit.md`**
> （再生成＝`python -X utf8 scripts/tx-pool-selfcontained-audit.py`）。正典＝`docs/tx-v12.2.1-inline-lock.md` §v13y。
>
