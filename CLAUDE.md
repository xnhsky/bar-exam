...
|---|---|---|---|---|
| 刑法 | `刑TX` | `outputs/000_TX/刑TX/` | `刑JX` | `outputs/001_JX/刑JX/` |
| 憲法 | `憲TX` | `outputs/000_TX/憲TX/` | `憲JX` | `outputs/001_JX/憲JX/` |
| 民法 | `民TX` | `outputs/000_TX/民TX/` | `民JX` | `outputs/001_JX/民JX/` |
| 商法 | `商TX` | `outputs/000_TX/商TX/` | `商JX` | `outputs/001_JX/商JX/` |
| 民訴 | `民訴TX` | `outputs/000_TX/民訴TX/` | `民訴JX` | `outputs/001_JX/民訴JX/` |
| 刑訴 | `刑訴TX` | `outputs/000_TX/刑訴TX/` | `刑訴JX` | `outputs/001_JX/刑訴JX/` |
| 行政法 | `行政TX` | `outputs/000_TX/行政TX/` | `行政JX` | `outputs/001_JX/行政JX/` |
...
| 入力 PDF | 科目 | シリーズ | 出力 |
|---|---|---|---|
| `inputs/tx-pdfs/299.pdf` | 刑法 | TX | `outputs/000_TX/刑TX/刑TX299.html` |
| `inputs/jx/民/15.pdf` ＋ `inputs/jx/民/15.txt` | 民法 | JX | `outputs/001_JX/民JX/民JX015.html` |
| `inputs/tx-pdfs/予備R1-16詐欺.pdf` | 刑法 | TX | `outputs/000_TX/刑TX/刑TX001.html`（※最初の連続数字「1」を採用） |
| `inputs/jx/憲/5.pdf` ＋ `inputs/jx/憲/5.txt` | 憲法 | JX | `outputs/001_JX/憲JX/憲JX005.html` |
...
> （2ファイル分割 `-deep.html` は採らない。理由＝ARB/TTS 副産物が第4部に依存し deep 後追いは
...
1. **エグゼクティブサマリー（`#exec-summary`）は作らない**。「答えを先に見ずに構成する」導き書
   ...
   論点抽出）は前半コアに残す**（ARB の主題抽出に `#issue-extraction` が必須）。`#issue-extraction`
   は順序・優先度・配点を伏せた論点見出しリストにとどめる（結論を先出ししない）。
2. **後半 deep（第4部体系化＋第5部）は `<details id="deep-dive">` でデフォルト折りたたみ**。
   1周目は視界から外す。**折りたたみは DOM 温存が鉄則**（要素は DOM に残す＝RX/ARB/TTS 生成も
...
| # | 目的 | ツール | 要点 |
|---|---|---|---|
| ① | **入力アラインメント**（PDF↔逐語のズレ検出） | `scripts/check-jx-alignment.py`＋`inputs/jx/transcript-map.json` | 生成前に逐語を解決。既定は同番号、ズレは overrides に明示し keyword で本文照合。`[ERROR]` なら生成中断 |
...
- **実レイアウト（入力）**：`inputs/jx/{科目}/重問PDF/{n}.pdf` ＋ `inputs/jx/{科目}/講義逐語/{科目名}_重問{nn}.txt`
...
**ARB 樹形図**（`outputs/004_JX_EX/ARB/`・ARBOR 正典仕様）をバッチランナーの ②-rx / ②-arb 段で
...
| **JX** | JX 生成 | inputs/jx 最若番から N問（既定3）JX→validate→台本→validate→配置まで（ATHENA・**音声なし**） | — |
...
2. **本線 `master` に集約**（§8）。作業ブランチで commit → 本線へマージ。
   ファイル名は §2 命名規則どおり（`outputs/000_TX/刑TX/刑TX346.html` 等）。
