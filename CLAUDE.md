...
- **【解法ナビ・2026-06-25】解法ナビの常時表示ヘッダー `sn-sub` に各話者・各説の到達結論を断定形で書くこと**
  （例「学生Bは限定説・判例反対、学生Aは認識不要説」＝組合せ問題の正答が解く前に割れる・実害 刑TX351_lex／是正 389df14）。
  許容＝手順説明＋「立場の一貫性が鍵」型の中立メタヒントのみ。結論誘導は STEP の💡コツ（tip）側で段階的・疑問形に。`validate-tx-core.py` G32 が機械検出（spec 第4-3項）
