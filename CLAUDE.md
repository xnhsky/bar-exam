...
> ユーザー指示 2026-07-18）。機械ゲート＝`validate-tx-core.py` **G70**（WARNING・疑問形「か」終わり／
> 句点なし体言止めを末尾ヒューリスティックで検出・「〜との記述」等の定型除外・誤検出は無視可）＋push 前
> `check-tx-lex-engine.py` の非ブロッキング助言。意味の自己完結（要件事実の充足・極性の正しさ）は機械化
...
| 記号 | ストリーム | 動作 | エンジン |
|---|---|---|---|
| **T** | 新規TX | `inputs/000_TX/{科目}` 未生成を最若番から **v13 二系統（公式＋Lexia `_lex`）** 生成→両検証→各問 commit/push | `scripts/tx-v13-runner.ps1` |
| **J** | 新規JX | `inputs/001_JX/{科目}` 未生成を最若番から JX＋**副産物 RX/TREE/ARIADNE**＋台本→配置→finalize | `scripts/jx-batch-runner.ps1`（内部エンジン） |
| **R** | 旧版TXLEX再生成 | `_lex` が旧版（v13でない）かつ**PDFが残る**番号を **PDFから最新v13で作り直す**（過渡・全件最新化で消滅） | `scripts/tx-v13-runner.ps1 -Regen` |
