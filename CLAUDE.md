...
TJR は「大元の号令＝指揮者」で、3 ストリームを 1 号令で束ねる（**同時起動＝1作業ツリーの git 衝突回避のため T→J→R を直列自動実行**）：
...
- **号令なら指定外も当然に処理（番号ピン）**：「**TX355 を TJR処理**」＝`-TxFrom 355 -TxTo 355`（T=355固定・**J/R は通常**）／
  「**JX5 を TJR処理**」＝`-JxFrom 5 -JxTo 5`（J固定・T/R通常）。1ストリームだけは `-Only T|J|R`（短縮形「TX 355-360 処理」＝`-Only T -TxFrom 355 -TxTo 360`）。
