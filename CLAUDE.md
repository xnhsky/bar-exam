...
>
> **【12 役割タイポグラフィの割当て是正・2026-08-19・LEX-442・§v13u-2/-3】**ユーザー再指摘（刑訴TX054
> 「フォントちがうくない？」「ほぼすべてのフォントが割り当て通りになってない」）を受け、`spec/jx-v3.2-master.md`
> 第6項の 12 役割マトリクスと TX v13 実装を **computed style で全数照合**した。根本原因＝正典 CSS に
> **12 役割の外の第2フォント系統 `--ed-serif`／`--ed-sans`／`--ed-quote`** が存在し（§6-7 禁則「独自フォントの
> 新設」）、①本文・⑤引用を後段から上書きしていたこと。スタックは 12 役割の別名で `--ed-quote`＝`--font-statute`
> （**游明朝ではなく Noto Serif JP**）。実効の逸脱は 4 つ＝**④条文**（条文本文が答案書体・当たるのは `.para-num`
> 等の飾りだけ）／**⑤引用**（`.syn-orig`＝条文明朝 vs `.tx-inline-stmt`＝游明朝で割れていた）／**⑥答案**
> （`.tx-inline-explain` 全域へ過剰適用＝解説の地の文まで答案書体）／**⑦KEY**（参照 0・`.key-phrase-box` は
> `--font-impact`）。①③⑨は別名経由の衛生是正（見た目不変）、⑩教授は v13 カードに実体 0（コンテンツ側）。
> 是正後は「書体＝情報種別のラベル」（§6-1）が成立＝**条文=Noto Serif JP／判旨=Zen Old Mincho／記述原文・提示文=
> 游明朝／答案圧縮=しっぽりアンチック／解説の地の文=本文ゴシック**。ツール＝`scripts/tx-font-roles-recanon.py`
> （font-family 宣言の置換）＋`scripts/tx-basis-statute-font.py`（BASIS 条文カード本文への CSS 区画
> `TX-BASIS-STATUTE` を正典から逐語伝播）。いずれも決定論・冪等・**本文不変**で、**CRLF/LF が同一ファイル内で
> 混在する**実態に合わせ LF 版・CRLF 版の両方を試して当たった側の様式を保つ（`tx-lex-verdict-redesign.py` の
> 同型バグの再発防止）。**適用＝1,616 ファイル・19,990 箇所＋BASIS 区画 1,083 ファイル**（本文テキスト差分 0・
> validate-tx-core G1〜G77 PASS・check-tx-lex-engine PASS・check-font-vars 2,529 [OK]・check-duplicates PASS）。
> **実機確認が必要**＝生成環境は Google Fonts を取得できず（`document.fonts.size = 0`）視覚検証ができないため、
> とくに ⑥（解説の地の文が答案明朝→本文ゴシック）は端末で見て判断する。戻す場合は recanon の `SUBS` のうち
> ⑥ を付した 3 エントリを前後入替して `--apply --all` を再実行する（冪等）。
