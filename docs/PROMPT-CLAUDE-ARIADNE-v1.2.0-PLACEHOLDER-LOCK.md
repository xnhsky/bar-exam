...
- 最新法令・判例・学説レビューは必須。旧法/旧判例/旧説と現在法/最新判例/現在の有力説に差分がある場合は、
  深掘り層の関連カード付近へ `.ariadne-current-law-note` を置き、確認日・参照ソース・旧処理・現在処理・
  立法経緯/改正経緯・改正趣旨・試験対策上の扱いを記載する。最新確認未了の ARIADNE は完了扱いにしない。
