...
|---|---|---|---|---|---|---|---|
| **328** | multi-select-5 | 盗品等罪 | R7-19 (司法試験 令和7年第19問) | `[1, 4]` (int 配列) | P2 (セージブラリー) | `outputs/000_TX/刑TX/刑TX328.html` (101,151 bytes) | 全 3 段 PASS |
...
1. 実装着手前の `outputs/000_TX/刑TX/刑TX326.html` と `刑TX327.html` を `.bak` として退避。
...
- `outputs/000_TX/刑TX/刑TX326.html` / `刑TX327.html` / `刑TX328.html` (本書作成時点では再生成しない)
