...
**現状の出力例**: outputs/tx/刑TX/刑TX303.html 内 `<section id="mindmap-tree">` 周辺 viewBox `0 0 1100 600` か 700
...
4. `outputs/tx/刑TX/刑TX303.html`（現状 SVG 出力・改修前 baseline）
...
`outputs/tx/刑TX/刑TX303-eta-wip.html` (210,868 bytes) は戦略 η（AI 直接生成）の WIP。Phase 13C では参照しなくて OK（render.py 経路に集中）。
