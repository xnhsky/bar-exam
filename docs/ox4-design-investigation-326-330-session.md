...
3. **運用上の reference は「最初に完走した実例 HTML」とする**。
   - ox-grid-5 の reference → `outputs/000_TX/刑TX/刑TX326.html`
   - ox-grid-4 の reference → 完成後の `outputs/000_TX/刑TX/刑TX327.html`
...
| **Step 7** | 327 の 3 段検証完走 | (a) `python scripts/render.py 327` exit 0、`outputs/000_TX/刑TX/刑TX327.html` が生成、未置換 slot 0<br>(b) `python scripts/validate_structure.py outputs/000_TX/刑TX/刑TX327.html` exit 0、ERROR 0<br>(c) `python scripts/validate_content.py outputs/000_TX/刑TX/刑TX327.html problems/327.json` exit 0 (negative + positive 双方 PASS)<br>(d) ブラウザで開いて ox-grid 4 行が正しく動作するか目視 (任意) | 上記 a〜c すべて exit 0 | 30 分 |
