...
| **ARIADNE**（解法ナビ＋答案構成周回） | **`canonical/ARIADNE.html`**（v1.1.0 MATRIX-THREAD・2026-06-29 active） | `/new-ariadne` / `prompts/new-ariadne-headless.md` | `scripts/validate-ariadne.py`（A1〜A31）＋`scripts/check-ariadne-canonical.py` |
...
- **ARIADNE v1.1.0 MATRIX-THREAD（2026-06-29）**：JX019 で確定したマトリクス型答案構成を major 正典化し、
  さらに字下げ・ラベル＋本文2カラム・拾う文言近接2カラム・点線区切り・マイルドライナー系ラベル色・RX `data-rx`
  配線を minor 正典として固定。模範答案は Claude 正典の問規当結カードを維持する。恒久対策は
  `validate-ariadne.py` A30/A31 と `check-ariadne-canonical.py`、同期前 `check-lexia-preflight.py` に組込み済み。
