...
判定式は 2026-09-09 に 3 点精密化した（誤検出 27 件を解消）。
① 実体名は**2 字以上で 説/見解/立場/論 に終わる語**とし、`条件説`・`折衷説`のような短い実体名も通す
（ラベルそのもの＝`A説`・`見解Ⅱ` は除外）。② `取調受忍義務を認める見解Ⅰ`のように**修飾節が
ラベルへ直付け**されている形も通す（節が用言で終わることを要求＝`批判が向くのは見解Ⅰ`のような
助詞終わりは通さない）。③ **正誤表 原文帯もタグを落としてから判定**する（他 2 面は `get_text` で
剥がしているのに属性値の `data-brief-mark` だけ生のままで、タグ文字列が前後文脈に混じって
実体名の直付けを見落としていた）。**検出ゲートと復元ツールは同じ窓・同じ式**で動かす。

## 現況

- **(a) ERROR：0 ファイル**（2026-09-09 の是正で 0 件になった）
- **(b) WARNING：0 ファイル**（2026-09-09 に 18 ファイル 120 箇所を書き換えて完了。worklist は空）
...
**残件なし。** 2026-09-09 に 18 ファイル・120 箇所を書き換えて corpus を 0 件にした
（刑TX011/029/047/057/059/060/069/139/145/358/416/430・刑訴TX068/069/184/223/247/301）。
今後この表に行が現れたら `docs/run-patterns.md`「既存展開の配り方」に従いラウンドロビンで消化する。
