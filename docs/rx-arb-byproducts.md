...
- **構成（ユーザーと確定・固定）**：答案構成（問題文→事実の要点→入口の問い→組み立ての順序＝順番・論点名・順番の理由→骨子を論点名つきで「規範→使う事実」、項目の最後に結論、
  段階ヒント4段・項目ごとの復唱・通しの口頭構成）→ 規範クイズ → ○×クイズ（どちらも全問を最後まで・［規範クイズ 2/6］の番号つき）→ 間違えたところの復習。
  答案の書き方の一般論・論じる順番の一般論・配点は入れない（この問題の順番と理由は「組み立ての順序」で言う）。AI が全進行を仕切り、楽しく進める。
