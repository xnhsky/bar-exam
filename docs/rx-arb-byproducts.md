# RX / ARB 副産物パイプライン（2026-06-11 導入）
...
| `prompts/new-arb-headless.md` | ARB 生成プロンプト（ARBOR 正典 `C:\...\arbor\ARBOR_v5.0_master_prompt.md` を参照） |
...
- 実行順: ① JX 生成 → ② validate-jx PASS → **②-rx RX → ②-arb ARB** → ③ TTS → …
- **非致命設計**: RX/ARB が失敗しても JX/TTS の進行・連続失敗判定・overall には影響しない
