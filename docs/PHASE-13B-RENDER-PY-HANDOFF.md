...
- `python scripts/render.py 303` で再生成した HTML が、現 `outputs/tx/刑TX/刑TX303.html`（手動 polish 済 = gold standard）と **視覚的に同等** になる
- 306 以降の新規問題でも `python scripts/render.py NNN` だけで fb-2〜fb-5 が適用される
- 検証: `python scripts/validate-tx.py outputs/tx/刑TX/刑TX303.html` で ERROR 0 / WARNING 0
...
各パターンの **抽出元行番号** は `outputs/tx/刑TX/刑TX303.html`（gold standard）における位置。

### 3-1. fb-4: NOTE box grid layout + `.note-body` wrapper ⭐最重要

**抽出元 CSS**: `outputs/tx/刑TX/刑TX303.html:607-640`
...
**抽出元 HTML structure**: `outputs/tx/刑TX/刑TX303.html:3001-3003` ほか 5 箇所
...
**抽出元 CSS**: `outputs/tx/刑TX/刑TX303.html:2115-2183`
...
**抽出元 CSS**: `outputs/tx/刑TX/刑TX303.html:2185-2234`
...
**抽出元 HTML structure**: `outputs/tx/刑TX/刑TX303.html:2527-2546`（A-1 case-description 内 4 件）
...
**抽出元 CSS**: `outputs/tx/刑TX/刑TX303.html:2273-2286`
...
**抽出元 CSS**: `outputs/tx/刑TX/刑TX303.html:2262-2271`
...
**抽出元 CSS**: `outputs/tx/刑TX/刑TX303.html:2287-2304`
...
**抽出元**: `outputs/tx/刑TX/刑TX303.html:38-80`（コメント + :root）
...
**抽出元 SVG**:
- Tree: `outputs/tx/刑TX/刑TX303.html:3095-` 付近（viewBox 1500x820、L0/L1/L2/L3 座標）
- Radial: `outputs/tx/刑TX/刑TX303.html` の `id="mindmap-radial"` section（viewBox 1500x1180）
- Flowchart: `outputs/tx/刑TX/刑TX303.html` の `id="c-5"` 周辺（viewBox 1200x1280）
...
**抽出元 HTML**: `outputs/tx/刑TX/刑TX303.html` の footer 末尾
...
Step 2. diff /tmp/303-baseline.html outputs/tx/刑TX/刑TX303.html | wc -l で差分量を把握
Step 3. fb-4 (NOTE grid + .note-body wrapper) を実装
  ...
  3-D. diff /tmp/303-step3.html outputs/tx/刑TX/刑TX303.html で fb-4 部分が消えたか確認
...
1. **本ドキュメント全文**
2. `outputs/tx/刑TX/刑TX303.html`（gold standard、**read-only ─ コンテンツコピー禁止**）
...
- ❌ `outputs/tx/刑TX/刑TX303.html` の本文・解説文字列を template / render.py にコピーすること（AP-42 / S78 違反）
...
| 検証 | `python scripts/validate-tx.py outputs/tx/刑TX/刑TX303.html` ERROR 0 / WARNING 0 |
