# 引継ぎ：TX v12.2.x（重厚感・教科書化リデザイン＋v12.2.1表示LOCK）を全 v12 _lex へ伝播
...
- `tx-mini-law-para` 付き本文は、直後の本文を `tx-mini-law-body` で包み、ラベル列＋本文列の2カラムにする。折り返し本文をラベル下へぶら下げず、本文列に字下げしてそろえる。判例ボックスも同じ構造にする。
...
### 2026-07-01 追補2：v12.2.1 表示LOCK（今後の更新・新規生成で必須）

参照正典：`docs/tx-v12.2.1-inline-lock.md`

- 問題文と○×は `.tx-inline-card` 内で一体化する。○/×ボタンは `.tx-inline-actions` でカード右端に寄せ、旧 ox-grid を画面主導線に戻さない。
- 解法ナビは問題文直下・インラインカード群の前に置く。ヒントは結論ネタバレではなく、客体・条文要件・判例射程など「どこを見るか」を示す。
- 条文・判例上段の題名チップは下部 `basis-card-header` 由来にする。テーマチップは法理名にする。`題名：` / `テーマ：` の説明文字は表示しない。
- `本条` / `本文` / `①` / `②` / `判旨核心` / `判例` は `.tx-mini-law-para`、本文は `.tx-mini-law-body`。ラベル列＋本文列の2カラム、本文先頭だけ1字下げ、折返しは本文列にそろえる。判例も同じ。
- 条文の `①` / `②` は2項以上ある条文だけ使う。単一項・条文全体は `本条` / `本文`。
- 物語解説は `解答を表示` / `解説だけ閲覧` 前に露出させない。露出後の `data-fa-label` は左寄せ・通常フロー配置で、本文に被せない。
- 最新法令・判例・学説レビューは必須。新旧差分がある場合は `tx-current-law-note` に確認日・参照ソース・旧処理・現在処理・立法経緯/改正経緯・改正趣旨・試験対策上の扱いを入れる。
- ローカル確認は `outputs/ux` をルートにした HTTP サーバーで行う。例：`python -m http.server 8788 --bind 127.0.0.1` を `outputs/ux` で起動し、`http://127.0.0.1:8788/000_TX/001_刑法/刑TX355_lex.html` を見る。
- 機械検査は `validate-tx-core.py` G45 と `check-tx-lex-engine.py`。G45 が落ちる状態は push しない。
...
```bash
# 各 v12 _lex は ALL PASS（G1〜G45。v12.2.1 マーカー付きは G45 表示LOCKも含む）になる。1件でも ERROR なら原因究明（手で直さない）。
for f in $D/刑TX3[6-8]*_lex.html; do python -X utf8 scripts/validate-tx-core.py "$f" | grep -E "ERROR|FAIL|PASS"; done

# push 前ゲート：G41(接ぎ木)+G42(組合せ当否)+G43(空詳説)+G44(回答UI)。v12.2.1 マーカー付きまたは明示指定ファイルは G45(表示LOCK) も対象。検出0で PASS。
