...
- **【G51 空箱＝`basis_refs` を足す】** 各記述カードの 📚BASIS は、旧 `#basis` の条文/判例カードを
  **カード本文中の「記述N」表記**で記述へ配分する。v11 の基盤カードが「記述N」を持たない問題では全 BASIS が
  空箱になり **G51（ERROR）** が出る。その時は `slots['basis_refs']`＝`{ "basis-card-id": ["記述ラベル", …] }` を
  足す（wrapper が隠しヒント span を注入して recanon に配分させる）。id は v11 の `<div class="basis-card … id="law-103"/"case-…">`。
  **各記述に最低1件**割り当てる（中核条文は全記述へ）。実証＝刑TX409（`law-103`→全5・`law-105`→3・`case-inpi`→2・
  `case-s8-10-18`→5・`case-tsumiwo`→1,4）。407 は基盤カードに「記述N」があり不要だった。
