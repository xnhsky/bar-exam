...
- **科目順＝2 レーン並行**（2026-08-31 ユーザー指示）。
  - **レーン①「これから学習する科目」**＝②刑訴 → ③民法 → ④民訴 → ⑤商法 → ⑥憲法 → ⑦行政法
    （TJR の科目検知順から①刑法を抜いたもの）。既存分も将来生成される分も、この順で若番から消化する。
    1 バッチ `-MaxProblems`（既定 10）。
  - **レーン②「学習済みの過去分」**＝**刑法**（`_lex` 427 本）。量が大きいので独立レーンにし、毎バッチ
    `-MaxKeiho`（既定 10）を並行して流す。レーン①の進み具合に影響されない。
  - `-Subject <科目>` を明示すればその科目だけを流せる（刑法を指定した場合は `-MaxProblems` 件）。
