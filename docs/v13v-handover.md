...
- **科目順＝2 レーン並行**（2026-08-31 ユーザー指示）。**学習の区切りは 刑訴TX074**（ここまで学習済み）。
  区切りの単一情報源はランナー冒頭の `$FullyLearned` / `$LearnedUpTo` の 2 行で、番号境界はそこから
  自動導出される（学習が進んだらこの 2 行だけを更新する）。この割り方は S 以外の既存修正にも共通する
  一般則＝`docs/run-patterns.md`「既存展開の優先順（学習の区切りで2レーンに割る）」が正典。
