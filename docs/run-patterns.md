...
## 既存展開の優先順（学習の区切りで2レーンに割る・2026-08-31 ユーザー指示）

> **正典改定を既存ファイルへ後追い展開するとき、対象の選び方はこの一本に統一する。**
> S ストリーム（CONTEXT 帯）だけの話ではなく、**以後の「既存分の修正依頼」全般に適用する考え方**とする。

### 割り方

学習の進度で対象を 2 つに割り、**両方を毎バッチ並行して流す**。

| レーン | 対象 | 順序 | 1バッチ |
|---|---|---|---|
| **① 学習レーン**（これから学習する範囲） | 学習の区切りより先 | 学習順（②刑訴 ③民法 ④民訴 ⑤商法 ⑥憲法 ⑦行政法）で若番から | 既定 10 本 |
| **② 過去分レーン**（学習済みの範囲） | 学習の区切りまで | 学習順のうち学習済みの帯を、その順で若番から（①刑法 → ②刑訴…） | 既定 10 本 |

**なぜ 2 レーンか**：学習中の科目は次の周回で目に入るので改訂の効きが早い。一方、学習済みの帯は
量が大きく（刑法だけで `_lex` 427 本）、後回しにすると永久に終わらない。片方を優先順に並べるだけだと
必ずどちらかが飢えるので、**別々に上限で切ってから合流させる**（量の多い過去分が学習レーンを押し出さない）。

### 区切りの単一情報源

`scripts/v13v-runner.ps1` の冒頭 2 行だけが正である。**学習が進んだらここだけを書き換える**
（レーンごとの番号境界 `Min`/`Max` は、この 2 行から自動的に導出される）。

```powershell
$FullyLearned = @('刑法')        # 全番号が学習済み＝丸ごと過去分レーン
$LearnedUpTo  = @{ '刑訴' = 74 } # その番号までが過去分／それ以降が学習レーン
```

表に無い科目は「まだ学習していない」＝全番号が学習レーンに入る。**番号をスクリプト本体や
プロンプトへ直接書かない**（散らばると次に学習が進んだとき直せなくなる）。

### 2026-08-31 時点の状態

刑法は一巡し、刑訴は TX074 まで学習済み。導出される対象は次のとおり
（刑訴074-100 は同日 §v13w 済みのため、学習レーンで実際に当たるのは 101 以降）。
