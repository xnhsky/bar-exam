...
| **G** | §v14 THE GIST ストーリー型の付随・特別枠（2026-09-17 新設） | **旧型（1 段落）の 💡THE GIST が残る v13 `_lex`**（設置時点＝903 本：刑法281・刑訴312・民法227・民訴83）を、S と同じく**仕事のある科目へ均等に配る**（ラウンドロビン・`-MaxG` 既定10本／科目内は若番順／「TJR処理 刑訴」で寄せられる）で headless（Opus 5 固定）が JSON 仕様を執筆 → `scripts/tx-gist-story.py apply` で組む。対象判定の単一情報源＝`tx-gist-story.py pending`（v13 でない旧版は R の領分なので除外）。ランナーが決定論で判定＝①`check`（全カードがストーリー型・構造・○×の向き）②`scope`（git HEAD と比べて GIST 行と CSS 区画以外が不変／GIST 行が仕様から組んだ HTML と一致／GIST にだけ出てくる判例＝ファイル外からの持ち込みが無い）③`css --check`（正典と一致）④validate-tx-core ⑤check-tx-lex-engine（当該ファイルだけ＝`TX_ENGINE_SKIP_CORPUS=1`）を全部 PASS したときだけ 1問ずつ commit/push（FAIL はロールバック・同一問題2回失敗で ESCALATE・連続3件で打ち切り）。コーパス横断の判例元号割れゲートはバッチ前に1回（NG なら G を見送り・strike なし）。headless が対象外の追跡ファイルを変えたら元に戻して FAIL。claude が何も書かずに死んだ（ログイン切れ等）ら strike なしで停止し exit 2＝TJR は以後のバッチで G を止める。`-NoPush`/`-NoCommit` では claim を取らない。行構造のせいでツールが書き換えられないファイル（2026-09-17 時点 刑訴TX252＝全体が1行）は `pending --unreplaceable` に分けて対象外。push 追随で CSS 区画が落ちたら正典から再注入して追いコミット。二台衝突は claim（`{ID}_v14g`）＋リモート書き換え済み検知で回避。レシピ正典＝`docs/tx-v12.2.1-inline-lock.md` §v14／プロンプト＝`prompts/v14-gist-headless.md`（GIST 外の旧法記述は直さず `logs/v14g-findings.md` へ記録） | `scripts/v14-gist-runner.ps1` | **過渡＝残件ゼロ（完遂）で「該当なし」SKIP＝自然消滅** |
| **D** | §v15 DEDUP（罠枠＝「1語差し替えで裏返る」隣接命題＋📐比較表・記述カードの重複削減・2026-09-22 新設） | **§v15 未適用（罠枠に `data-v15` が無い）の v13 `_lex`**（設置時点＝1,002 本：刑法299・刑訴333・民法227・民訴143）を、G と同じく**仕事のある科目へ均等に配る**（ラウンドロビン・`-MaxD` 既定10本／科目内は若番順／「TJR処理 刑訴」で寄せられる）。ランナーが ①決定論の土台＝**①-a `scripts/tx-lex-verdict-redesign.py`**（§v13x の図解帯 CSS `TX-VERDICT-DGM` とエンジン `appendDiagramBand` を正典から伝播。これを持たない世代に §v15 の比較表を入れると、正誤表へ複製した `.tx-vb-dgm-src` で **G78 が ERROR** になる＝刑訴の実測で 149 本中 53 本が該当・2026-09-22）＋**①-b `scripts/tx-v15-dedup.py base`**（📌POINT／GIST🖼イメージ面／BASIS「本問への帰結」／条文操作末尾の結論文の削除・CSS 同期）→ 2 つ合わせて単独 commit ②headless（Opus 5 固定）が JSON 仕様（罠・比較表・フック・判定・転用）を執筆 → `tx-v15-dedup.py apply` ③`check`（構造・禁止語・表 4×4・正誤表との複製一致）＋`scope --spec`（HEAD と比べ許可領域以外が不変・罠枠が仕様どおり）＋`css --check`＋validate-tx-core＋check-tx-lex-engine（当該ファイルだけ）を全部 PASS したときだけ commit/push（FAIL はロールバック・同一問題2回で ESCALATE・連続3件で打ち切り・基盤障害は exit 2）。対象判定の単一情報源＝`tx-v15-dedup.py pending`。二台衝突は claim（`{ID}_v15d`）＋リモート適用済み検知（`data-v15="1"`）。レシピ正典＝`docs/tx-v12.2.1-inline-lock.md` §v15／プロンプト＝`prompts/v15-dedup-headless.md` | `scripts/v15-dedup-runner.ps1` | **過渡＝残件ゼロ（完遂）で「該当なし」SKIP＝自然消滅** |
...
# D（§v15 DEDUP＝罠枠の比較表化の付随）だけ回す／件数を変える／止める（科目へ均等配分）
pwsh -NoProfile -File scripts\patterns\TJR.ps1 -Only D            # 10本だけ処理（仕事のある科目へ均等）
pwsh -NoProfile -File scripts\patterns\TJR.ps1 -Only D -Subject 刑訴 # いま学習中の科目へ寄せる
pwsh -NoProfile -File scripts\patterns\TJR.ps1 -SkipD             # 通常TJRからDを外す
pwsh -NoProfile -File scripts\v15-dedup-runner.ps1 -DryRun        # 今バッチの対象を確認
python -X utf8 scripts\tx-v15-dedup.py pending                    # 残件数（科目別）
# ※通常の「TJR処理」でも毎バッチ末尾（G の後）で自動的に10本ずつ消化される（完遂まで）
...
## 入力の共有（PC 間セットアップ・Drive が唯一のチャネル）
...
**入力の共有は Drive の往復 2 本で成り立つ。分割した PC は必ず「送り」も回す（2026-09-06）：**

| 向き | スクリプト | いつ回すか |
|---|---|---|
| ローカル → Drive（**送り**） | `scripts/tx-push-inputs-to-drive.ps1` | 一括 PDF を分割した直後（分割した PC で） |
| Drive → ローカル（受け） | `scripts/tx-pull-inputs-from-drive.ps1` | 新しい PC／未取込の科目で |

**2026-08-15 に受け（pull）だけを作り、送り（push）を作らなかったのが原因で再発した**：
OWNER PC が民法〜憲法を分割してもローカルに溜まるだけで Drive は空のまま＝xnrg2 PC は
pull しても取り込むものが無く、TJR-T が全科目「該当なし」だった（2026-09-06 特定・
同日 2,417 件／985MB を送って解消）。**分割したら送る**までが 1 セット。

```powershell
# 分割した PC で（ローカル → Drive）
...
送りも受けと同じ規律＝**ステム全体が数字の 1問1PDF だけ**を配り、分割前の原本は既定で配らない
（`-IncludeSource`）。**既存の Drive ファイルは上書きせずスキップ・Drive 側の余剰は絶対に消さない**
（一方向・`/MIR` ではない）。分割来歴 `_分割一覧.md` は軽いので既定で同送する。

**入力欠落は TJR が起動時に名指しする（2026-09-06）：** 入力が 1 件も無い科目は
T/R の『該当なし』と見分けが付かず、xnrg2 PC では原因不明のまま空振りが続いた。
TJR は起動時にプリフライトを走らせ、`[入力欠落] TX の 1問1PDF が 0 件の科目: …` と
取り込みコマンドを出す（TX・JX 両方）。**この行が出たら仕事が尽きたのではなく入力未取込**。
...
- **Drive 側が未分割なら取り込むものが無い**（→ その場合は分割した PC で先に
  `tx-push-inputs-to-drive.ps1` を回す）。**2026-09-06 現在、TX は全 7 科目が Drive に揃っている**
