...
|---|---|---|---|---|---|---|---|
| **329** | single-choice-5 | 器物損壊罪 (+ allowed_cross_refs: ["信書隠匿罪"]) | H20-16 (司法試験 平成20年第16問) | `2` (integer 単体) | P1 (ローズシャンブル、正答率 89%) | `outputs/tx/刑TX/刑TX329.html` (100,939 bytes) | 全 3 段 PASS |
...
1. 実装着手前の `outputs/tx/刑TX/刑TX326.html` / `刑TX327.html` / `刑TX328.html` を
   `.bak` として退避。
...
- `outputs/tx/刑TX/刑TX326.html` / `327.html` / `328.html` / `329.html` (本書作成時点では再生成しない)
