...
|---|---|---|---|---|---|---|---|
| **330** | combination-5 | 器物損壊罪 (+ allowed_cross_refs: ["窃盗罪", "信書隠匿罪"]) | 予備H23-10 (予備独自問題 平成23年第10問) | `3` (integer 単体、組合せ番号) | P1 (ローズシャンブル、正答率 84%) | `outputs/tx/刑TX/刑TX330.html` (103,945 bytes) | 全 3 段 PASS |
...
1. 実装着手前の `outputs/tx/刑TX/刑TX326-329.html` をそれぞれ `.bak` として退避。
...
- `outputs/tx/刑TX/刑TX326-330.html` (本書作成時点では再生成しない)
