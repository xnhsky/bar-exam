...
## 一問一答「単独成立」監査・優先R対象（2026-07-08・ユーザー指示）

「一問一答カードを単独で見て誰でも○×で答えられるか」の観点で刑法 `_lex` を監査。
**事例（罪名）断片・（成否）メモ・疑問形**の8本（070 073 177 243 338 376 391 401）は
リモートで命題化済み（記号フリー・answer-key保持・validate/oxgrid/engine PASS・PR #10）。
残る**記号がファイル全体に浸透した特殊型4本**は接ぎ木（ox-stmtだけ修正）を避け、
**PDFからの R 再生成を最優先**で実施する（ユーザー指示「残りはTJRで優先的に」）。

| 番号 | 型 | 記号参照数 | PDF | R適格 | 手順 |
|---|---|---|---|---|---|
| **204** | 穴埋め・刑罰論（応報/目的刑）a.〜j. | 110 | あり | ✅ 即R可 | `tx-v13-runner.ps1 -Subject 刑 -Regen -FromNumber 204 -ToNumber 204`（全番号リスト掲載済み） |
| **249** | 見解・住居侵入(130条)の各説 ①〜⑤ | 15 | あり | ✅ 即R可 | 同上 `-FromNumber 249 -ToNumber 249`（全番号リスト掲載済み） |
| **109** | 論点比較・責任能力（判断基準時ほか） | 19 | **なし** | ⛔ 要PDF回収 | Drive「1 TX_短答/001_刑法/抽出PDF」から109を回収→inputs配置→R |
| **406** | 見解・犯人隠避/証拠偽造 見解A/B | 144 | **なし** | ⛔ 要PDF回収 | Drive から406を回収→inputs配置→R |

- **204/249＝PDFあり**：ともに全番号リスト掲載済みでレンジ実行でも拾われるが、**優先**のため
  次回 TJR（刑・R）で番号ピン（`-FromNumber N -ToNumber N`）して最優先実行する。
- **109/406＝PDFなし**：R は `[R-SKIP-NOPDF]` でスキップする。**先に Drive から抽出PDFを回収**して
  `inputs/000_TX/001_刑法/{109,406}.pdf` に置いてから R を掛ける（§8「PDFが見つからない時の確認」）。
- **【L4退化の注意・上記2026-07-08追記と同根】** 204/109/406 は穴埋め・見解型のため、R で v13.1.0 化しても
  `check-lex-oxgrid-integrity.py` の **L4-DEGENERATE-FILL（全○退化）が残りうる**。穴埋め/見解型の○×提示方針
  （2択誘導 blank-mode 化 等）は `docs/lex-oxgrid-integrity-audit.md §4` の設計判断待ち＝R 完了後に別途適用する。
