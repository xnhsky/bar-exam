...
> - 唯一の起点は `canonical/GENESIS-CORE.html`（v11.1.0）。validate は `scripts/validate-tx-core.py`（G1〜G30）。
...
2. **`_lex` に解法ナビを注入**（`canonical/SOLVE-NAV.html` を Read し、3 ブロックを移植・**エンジン JS は逐語コピー**）：
   ...
   - **★sn-sub のネタバレ禁止（G32・spec 第4-3項）**：`[SHELL]` の `sn-sub`（常時表示ヘッダー）を本問値に
     差し替える場合、**各話者・各説の到達結論を断定形で書かない**（例「学生Bは限定説・判例反対、学生Aは
     認識不要説」＝組合せ問題の正答が解く前に割れる・実害 刑TX351_lex）。**手順説明＋「立場の一貫性が鍵」型の
     中立メタヒントに留める**（迷ったら canonical の sn-sub を流用）。結論を確定させる誘導は STEP の `tip`（💡コツ）側で
     段階的・疑問形（「Bは限定説か非限定説か」「賛否はどちらか」）で行う。`validate-tx-core.py` G32 が機械検出。
