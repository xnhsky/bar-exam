...
> - 唯一の起点は `canonical/GENESIS-CORE.html`（v11.1.0）。validate は `scripts/validate-tx-core.py`（G1〜G32）。
...
#### 4h. 二系統出力（公式／Lexia 用 _lex に分離・v11.1.0 最重要）
...
1. 本問の内容（核心論点・各記述の正誤理由）を、**一連の読み物**として執筆。要件：
   ...
   - **意味ラベル（食い出しタブ・必須）**：各段落に**その段落が何を語るかの短い見出し（6〜12字）**を付す。
     Lexia の復習プールはこのラベルを段落の食い出しタブとして表示し、肢ハイライトの文脈にする。
     ラベルも**記号フリー**（①〜/(a)〜/A説・甲乙説/記述記号への言及禁止）＝**論点の実体名**で書く
     （例：「故意の標準」「共同正犯の射程」「判例の結論」「まとめ：覚えるのは理由」）。
   - 形式（**段落＝オブジェクトで label を併記**）：
     `{"title":"この問題を物語で読む ── <短い主題>","paras":[{"text":"段落1<b>…</b>","label":"導入：何が問われるか"},...]}`
     （**6〜9段落**・各 `label` 必須・重要語は `<b>`・①記号やmarkdown禁止）。旧形式 `"paras":["段落",...]`＋
     並行 `"labels":["…",...]` も可。手本＝`outputs/ux/000_TX/001_刑法/刑TX311_lex.html` の `.fa-narrative`。
...
15. **G1〜G30 ERROR 0 件確認**（特に G20 記述単位・G21 禁止句・G22 choice-points・G23 answer-key（_lex）・
    ...
    **G32（GIST/POINT の自己完結・WARNING）が出たら、`.syn-lead`（THE GIST）・`.choice-points`（POINT）から
    同様に記号（①〜/(a)〜）・見解ラベル（A説/B説・甲乙説）・他記述参照を除く**（Lexia がプールに併載・spec 第3-bis項・対象拡張）。
    とくに**組合せ問題の GIST**は「①a＋⑥l」と書かず「限定説と認識必要説の組合せ」と学説の実体名で書く。
...
16-bis. **両ファイルにフッター生成日時＋版を刻む（Lexia が raw 取得して読む・必須）：**
    ...
    `Generated: YYYY-MM-DD HH:MM / TX v11.1.0 LOOP-CORE`・class=lexia-genmeta を刻む・冪等）。素の git commit でも
