...
### Phase 3：canonical の clone と本文初期化

> **【起点の版分岐・最重要】active＝v13.1.0 LOOP-CARD の新規生成／v13 化は起点＝`canonical/GENESIS-CARD.html`**
> （＋`canonical/GENESIS-CARD.placeholder.html` スロット契約）。**GENESIS-CARD を複製し placeholder のスロットだけを
> 埋める**（構造は上部ボックス＝正誤表→体系マップ→横断→記述カード→物語・spec `tx-v13.1.0-loopcard-core.md`）。
> 下記 step 9-10 と Phase 4a〜4g（PART A/B/B+ の section-by-section）は **v12.2.1 LOOP-CORE の記述**で、
> 既存 v12 資産（355-385 等）の保守時のみ `GENESIS-CORE.html` を使う。**共通フェーズ**（Phase 2 命名／Phase 4h 二系統／
> Phase 5 SVG 検査／Phase 6 検証／Phase 7 commit）は両版で使うが、**検証・feature-tag は生成した版に合わせる**。

9. **起点の canonical を Read**（v13＝`canonical/GENESIS-CARD.html`／v12 保守＝`canonical/GENESIS-CORE.html`。
    GENESIS.html や outputs/*.html は使わない）。
10. **対象ファイル名でコピー作成**（Write 経由 or bash `cp`。前面 PowerShell は Copy-Item がブロックされる→
    ...
    `cp canonical/GENESIS-CARD.html outputs/000_TX/{科目TX}/{接頭辞}{NNN}.html`（v12 保守は GENESIS-CORE.html）
...
feature-tag 先頭＝**生成した版**（v13 新規＝**`TX v13.1.0 LOOP-CARD`**／v12 保守＝`TX v12.2.1 LOOP-CORE`）。
版は `scripts/tx-lex-v13-stamp.py --apply` が実体判定（nb-badge＋data-brief-mark）で確定する。続けて genesis-core-baseline／editorial-reskin／
...
> **(a) 安いpython機械ゲート（下記 14-15・トークンほぼ0）を必ず通す**＝validate-tx-core・check-duplicates・check-tx-lex-engine・check-tx-reuse・check-lexia-book-quality。
...
16-bis. **両ファイルにフッター生成日時＋版を刻む（Lexia が raw 取得して読む・必須）：**
    ...
    `Generated: YYYY-MM-DD HH:MM / TX v13.1.0 LOOP-CARD`（v12 保守は v12.2.1）・class=lexia-genmeta を刻む・冪等。
    版は `scripts/tx-lex-v13-stamp.py --apply` が実体判定で最終確定）。素の git commit でも pre-commit フック（§9）が保険で刻む。
