...
- **この 4a は省略不可（LEX-403・2026-07-21）**：§5 宣言コメント（`§5 V3 P{N} {Name} (palette ID: NN) ─ …`）と
  palette :root（block#2＝`--accent`／block#3＝`--accent-light`）を**対で**差し替える。既定配色のまま出荷は
  `validate-tx-core.py` **G71**（ERROR）、宣言と正答率帯・実 hex の不整合は **G72** が弾く（push 前
  `check-tx-lex-engine.py` にも組込み・単一情報源＝`scripts/tx_palette_rules.py`）。第1 `:root`（フォント12変数）は
  不可侵（LEX-388/G68・パレットは第3 `:root`＝`--accent` を含む方だけ）。
...
- **組合せ・穴埋め・会話型**：各空欄／各命題を**独立した1つの○×命題に分解**する。**data-correct-value は
  ...
  **【一問一答自己完結の鉄則・全問共通・最重要】記述カードの見出し（`.tx-inline-stmt-text`）と ox-stmt は、
  ...
  - **断片で終わらせない（何を○×するのか不明になる）**：`…した場合。`（結論欠落）／`事例（○○罪）`（罪名ラベルだけ）／
    `…評価`『…見解』（体言止め）／`…との内容`（設問枠組みの圧縮残り・G70 ERROR＝実害 刑TX405/413）／
    `…＝1個の恐喝罪`（＝ラベル）は不可。必ず動詞で成否・当否を言い切る。
...
14. **両ファイルを検証**（二系統）：
    ...
    - `python scripts/check-tx-lex-engine.py outputs/ux/000_TX/{科目}/{接頭辞}{NNN}_lex.html`（単一エンジン・script2本・G41〜G45／G50-G64／G71-G72 難易度帯パレット）
...
15. **G1〜G35 ERROR 0 件確認**（特に G20 記述単位・G21 禁止句・G22 choice-points・G23 answer-key（_lex）・
    ...
    G33 TX-LEX最終表5点セット）。**G71/G72（難易度帯パレット・2026-07-21）も ERROR 0 を確認**＝Phase 4a を
    スキップして既定配色のまま（G71）／§5 宣言と帯・実 hex の不整合（G72）は配信不可
    （一括修復＝`scripts/tx-lex-repaint-palette.py`・hex 正典＝`memory/reference_palette_v3.md`）。
