...
- **組合せ・穴埋め・会話型**：各空欄／各命題を**独立した1つの○×命題に分解**する。**data-correct-value は
  ...
  **記述カードの見出し（`.tx-inline-stmt-text`）と ox-stmt は、語句群記号（ア/イ・①〜・(a)〜）を外し、
  それ単独で○×を判断できる完結した命題にする**（例：事例＋「私文書偽造罪が成立する。」のように断定文で結ぶ）。
  「…した場合。」で終わる事例断片のまま○×を付けさせない（何を○×するのか不明になる）。見本＝刑TX368_lex。
