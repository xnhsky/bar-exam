---
description: 新規 TX コア（メイン）を問題 PDF から生成（v12.1.0 LOOP-CORE：GENESIS-CORE baseline + インライン肢カード + 詳説トグル + 記憶フック/答案圧縮 + 3層配色体系 + ox-grid/SM2 肢単位管理）
---

新規 TX コア HTML（短答式・周回＋誤答修正が単体で完結するメイン）を問題 PDF から生成する。

> **v12.1.0 LOOP-CORE 経路（2026-06-29 inline canon・刑TX360 を昇格）**：active 正典 `spec/tx-v12.1.0-inline-core.md`。基盤として `spec/tx-v11.0.0-core.md`（肢単位管理）と `spec/tx-v11.1.0-twotrack.md`（公式/ Lexia 二系統）を継承する。
...
> - **配色住み分け（v12.1.0・最重要）**：①**大前提**（ヘッダー/フッター/紙面背景/PART見出し・section-title・リンク）＝従来の **V3 3パターン（11パレット）** 基調色。②**PART A 問題・解答エリア**＝**ナチュラルマイルド色**。③**インライン肢カード**＝Mildliner 系（条文=ブルー、判例=ピンク、補助根拠=グレー、文言/趣旨/射程/転用=レモンイエロー、切断点=ピンク、記憶フック=紫、答案圧縮=ピンク）。正誤の○緑/×赤は semantic 維持。
> - 唯一の起点は `canonical/GENESIS-CORE.html`（v12.1.0）。validate は `scripts/validate-tx-core.py`（G1〜G34）。
...
6-bis. **配色住み分け（v12.1.0・最重要）**：上記 V3 5色は **大前提のみ**（ヘッダー/フッター/紙面背景/PART見出し・
...
- **配色オーバーレイ §18〜§24 は触らない（v12.1.0）**：`<style>` 末尾の §18（バッジ共通規約）〜§24（inline canon
...
#### 4d-bis. PART B+ 差替（横断・比較・罠コラム・v12.1.0）
...
feature-tag 先頭＝**`TX v12.1.0 LOOP-CORE`**（必須）。続けて genesis-core-baseline／editorial-reskin／
...
#### 4h. 二系統出力（公式／Lexia 用 _lex に分離・v12.1.0 最重要）
...
15. **G1〜G34 ERROR 0 件確認**（特に G20 記述単位・G21 禁止句・G22 choice-points・G23 answer-key（_lex）・
...
16-bis. **両ファイルにフッター生成日時＋版を刻む（Lexia が raw 取得して読む・必須）：**
    ...
    `Generated: YYYY-MM-DD HH:MM / TX v12.1.0 LOOP-CORE`・class=lexia-genmeta を刻む・冪等）。素の git commit でも
