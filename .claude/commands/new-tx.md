...
8. 科目接頭辞・出力先：刑→`outputs/000_TX/001_刑法/刑TX{NNN}.html`（憲/民/商/民訴/刑訴/行政 同様）。
...
- A-2 `.answer-area`：
  ...
  - **【最重要・自己完結ルール（spec 第3-bis項・G30）】`.ox-stmt` と論点コアのセルは、Lexia が
    問題本体から切り離して一問一答カードにする実体。その回の問題レイアウト固有のラベルを残さない：**
    - 見解は**実体の学説名で主語化**する：「A説からは…」→「**予見義務説（旧過失論）**からは…」。
      学名が無い抽象見解問題は見解の**定義内容**を主語にする（A説/甲説/第N説をそのまま主語にしない）。
    - 記号穴埋め型（①〜⑤に a〜j）は、論点コア・ox-stmt から**空欄記号①〜⑤・選択肢記号(a)〜(j)を除き
      語句の実体に置換**する（「①b（…）が正しい」→「公共の危険は不特定多数への危険を含む（最決平15.4.14）」）。
    - 他記述・他肢参照（「記述アと異なり」「肢イは」）／「本問」「上記見解」依存を**書かない**。
    - 選択肢原文（記号入り）は **A-1 の `.problem-text` にそのまま温存**＝正規化するのは ox-stmt と論点コアだけ。
      「どの説がどの記述か」を文脈から復元する解法は **PART B の SYNTHESIS（4d）側**に書く。
...
15. **G1〜G30 ERROR 0 件確認**（特に G20 記述単位・G21 禁止句・G22 choice-points・G23 answer-key・
    G24 完全プロファイル不在・G25 ox-grid・G26 PART D不在・G29 answer-value 整合）。WARNING は配信可だが、
    **G27（PART A 参照条文）が出たら §4c に従い PDF 原文を確認し、印刷が無ければ削除**してから配信する。
    **G30（プール対象テキストの自己完結）が出たら §4d の自己完結ルールに従い ox-stmt・論点コアから
    問題ローカルのラベルを除く**（見解は実体名で主語化・記号は語句の実体に置換）。新規生成では極力 0 件に。
16. ERROR があれば該当箇所を修正し再検証。視覚確認推奨 → `present_files`。

### Phase 7：git コミットで永続化（§9）

16-bis. **フッターに生成日時＋版を刻む（Lexia が raw 取得して読む・必須）：**
    commit 前に **`python scripts/stamp-created-date.py`** を実行。新規生成の TX は
    フッターの日本語 footer-date を英語スタンプ `Generated: YYYY-MM-DD HH:MM / TX v11.1.0 LOOP-CORE`
    （class=lexia-genmeta）へ置換する（冪等・既に genmeta があれば無変更）。
17. 検証通過後 `git add outputs/000_TX/{科目TX}/{ファイル名}.html` → 本問単位で `git commit`
