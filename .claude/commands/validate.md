...
1. **ファイルパスから判別**：
   - パスに `outputs/tx/` または `刑TX`／`憲TX`／`民TX`／`商TX`／`民訴TX`／`刑訴TX`／`行政TX` を含む → **TX** と判定
   - パスに `outputs/jx/` または `刑JX`／`憲JX`／`民JX`／`商JX`／`民訴JX`／`刑訴JX`／`行政JX` を含む → **JX** と判定
...
```
TX v8.11.7 検証結果: outputs/tx/刑TX/刑TX302.html
...
```
JX v3.2 検証結果: outputs/jx/民JX/民JX015.html
