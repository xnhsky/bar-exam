...
#
# 【T モード（既定）】inputs/000_TX/{科目}/*.pdf のうち公式 HTML が未生成の番号を最若番から生成。
# 【R モード（-Regen）】outputs/ux/000_TX/{科目}/*_lex.html のうち「版が旧い（v13でない）」かつ
#   「入力PDFが残っている」番号を、PDFから最新v13で作り直す（公式も同時に最新化＝両者の本文整合を保つ）。
...
# === 対象検出 ===
# T: PDF あり かつ 公式 HTML 未生成 ／ R: 旧版_lex（v13でない）かつ PDF あり
$Pending = @()
if ($Regen) {
...
foreach ($t in $Targets) {
    ...
    $prompt = $tmpl `
        ...
        -replace '\{REGEN_FLAG\}',       ($(if ($Regen) { 'REGEN（既存の旧_lexと公式を最新v13で上書き）' } else { 'NEW（新規生成）' }))
    $execHeader = @"
...
"@
    ...
    if ($offBytes -gt 0 -and $lexBytes -gt 0 -and $sentinel -ne "FAILED") {
        ...
        if ($vLexCode -eq 0 -and $vOffCode -eq 0 -and $vEngCode -eq 0) {
            $validate = "PASS"
            Write-Host "[validate] PASS（_lex/公式/エンジン）" -ForegroundColor Green
        } else {
            $validate = "ERROR(lex=$vLexCode,off=$vOffCode,eng=$vEngCode)"
            Write-Host "[validate] ERROR lex=$vLexCode off=$vOffCode eng=$vEngCode — commit しない" -ForegroundColor Yellow
        }
    } else {
    ...
    if ($validate -eq "PASS" -and -not $NoCommit) {
        ...
        try {
            ...
            $msg = if ($Regen) { "feat($($Prefix)TX): $($t.ProblemId) を v13 で再生成（旧_lex最新化・二系統）" }
            ...
            $committed = $true
            Write-Host "[COMMIT] $($t.ProblemId) 永続化$(if($NoPush){'（commit のみ）'}else{'（push 済）'})" -ForegroundColor Green
