...
機械変換（本文保持・全問共通）:
  ...
  - gold-parity 正規化(tx-lex-sysmap-gold-sync)＋v13 polish(tx-lex-v13-polish・review6点)を
    build 末尾で冪等畳み込み＝生成物は最初から polished 済み（別工程の後追いpolish不要）
...
def _apply_v13_polish(h):
    """tx-lex-v13-polish.py の polish() を再利用してユーザーレビュー6点の意匠polish
    （肢カードを .tx-inline-list で包む・縞 nth-of-type・体系マップ題名の自動縮小・
    取説定型文の削除・物語をカード直後へ移設JS・太字軽減CSS）を冪等適用する。
    recanon 偽造出力は素で未包み＝縞も物語アンカーも壊れるため、gold-parity の後に
    畳み込んで「生成物が最初から正しい」状態にする。ロジックは polish 側に単一管理。"""
    import importlib.util
    p = Path(__file__).with_name('tx-lex-v13-polish.py')
    spec = importlib.util.spec_from_file_location('tx_lex_v13_polish', p)
    mod = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(mod)
    before = h
    new = mod.polish(h)
    return new, (new != before)
...
def build(path, slots, out=None, gold_path=GOLD):
    ...
    # --- v13 polish 畳み込み（review 6pt・冪等）：recanon 出力を最初から polished 済みにする ---
    # 別工程 tx-lex-v13-polish.py を後追いで回す運用を廃し、生成＝polish 済みへ一本化。
    h, _pol_changed = _apply_v13_polish(h)
    if _pol_changed:
