...
#
# 【重要・2026-07-09 方針転換】入力は削除せず inputs に恒久保管する運用へ変更した。よって本スクリプトは
#   もう自動経路（jx-finalize.ps1 の②／jx-batch-runner.ps1 の⑦）からは呼ばれない（両方とも削除を恒久無効化済み）。
#   本スクリプトは「どうしても手動で消したい時だけ明示実行する手動専用ツール」として残置する（既定 dry-run のまま）。
#
