...
# 副産物（RX/ARB）の生成元
$RxOutDir  = Join-Path $ProjectRoot "outputs\004_JX_EX\RX\${Subject}RX"
$ArbOutDir = Join-Path $ProjectRoot "outputs\004_JX_EX\ARB\${Subject}ARB"
...
foreach ($t in $Targets) {
    ...
    foreach ($id in $Ids) {
        ...
        # ARBOR 樹形図（outputs/004_JX_EX/ARB/{科目}ARB/{ID}_ARB.html）→ C_ARBOR\00N_科目\
