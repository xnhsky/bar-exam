...
# 旧 B_RX / C_TREE / D_ARIADNE は廃止。Join-Path で {base}\ux\002_RX\… に解決される。
...
function Get-SubjectDirs([string]$base, $info) {
    ...
    [PSCustomObject]@{
        ...
        # 副産物（RX 論証カード / TREE 樹形図 / ARIADNE 解法ナビ）— 系統フォルダ内に科目フォルダ
...
$JxOutDir = Join-Path $ProjectRoot "outputs\001_JX\$($info.html)"
...
foreach ($t in $Targets) {
    ...
    foreach ($id in $Ids) {
        ...
        # RX 論証カード（outputs/ux/002_RX/00N_科目/{ID}/{科目}RX{NNN}_*.html）→ B_RX\00N_科目\{ID}\
        ...
        # TREE 樹形図（outputs/ux/003_TREE/00N_科目/{ID}_TREE.html）→ C_TREE\00N_科目\
        ...
        # ARIADNE 解法ナビ（outputs/ux/001_ARIADNE/00N_科目/{ID}_ARIADNE.html）→ D_ARIADNE\00N_科目\
