...
foreach ($t in $Targets) {
    ...
    foreach ($id in $Ids) {
        ...
        # ARBOR 樹形図（outputs/004_JX_EX/TREE/{科目}TREE/{ID}_TREE.html）→ C_ARBOR\00N_科目\
...
if (-not $DryRun) { Write-Host ("=== 配置完了: HTML {0} / 台本 {1} / wav {2} / PDF {3} / 逐語 {4} / RX {5} / TREE {6}（全系統合計）===" -f $sumHtml, $sumTxt, $sumWav, $sumPdf, $sumTr, $sumRx, $sumArb) -ForegroundColor Green }
exit 0
