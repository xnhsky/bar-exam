...
使い方：
  ...
  python scripts/check-rx-coverage.py --no-color   # ログ保存向けに ANSI 色コードを出さない
...
SCHEMA_VERSION = "lexia-rx-coverage/v1"
...
def set_color(enabled):
    """ANSI color をまとめて切り替える。preflight の保存ログでは --no-color を使う。"""
    global GREEN, YELLOW, RED, MAGENTA, DIM, CYAN, RST
    if enabled:
        GREEN = "\033[32m"; YELLOW = "\033[33m"; RED = "\033[31m"
        MAGENTA = "\033[35m"; DIM = "\033[2m"; CYAN = "\033[36m"; RST = "\033[0m"
    else:
        GREEN = YELLOW = RED = MAGENTA = DIM = CYAN = RST = ""
...
DATA_RX_RE = re.compile(r"\bdata-rx\s*=\s*(['\"])(.*?)\1", re.I | re.S)
...
def referenced_in_ariadne(path):
    ...
    return {m.group(2).strip() for m in DATA_RX_RE.finditer(text) if m.group(2).strip()}
...
def subject_payload(stats):
    keys = ("subject", "jx", "present", "referenced", "uncovered", "unreachable",
            "safetynet", "dangling", "ok", "ari_missing")
    return {
        **{key: stats[key] for key in keys},
        "danglingDetail": [
            {"jx": code, "rx": list(rxs)}
            for code, rxs in stats["dangling_detail"]
        ],
        "unreachableDetail": [
            {
                "jx": code,
                "rx": [{"code": rx, "reason": reason} for rx, reason in rxs],
            }
            for code, rxs in stats["unreachable_detail"]
        ],
    }


def build_payload(filter_token, strict, checked, skipped, grand):
    count_keys = ("jx", "present", "referenced", "uncovered", "unreachable",
                  "safetynet", "dangling", "ok", "ari_missing")
    return {
        "schemaVersion": SCHEMA_VERSION,
        "filter": filter_token or "",
        "strict": bool(strict),
        "supplementCap": SUPPLEMENT_CAP,
        "fail": grand["dangling"] > 0 or (strict and grand["unreachable"] > 0),
        "counts": {key: grand[key] for key in count_keys},
        "subjects": [subject_payload(stats) for stats in checked],
        "skippedSubjects": list(skipped),
    }


def parse_args(argv):
    ap = argparse.ArgumentParser(
        description="ARIADNE 想起 data-rx と RX 実ファイルの到達性を検査する",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=__doc__,
    )
    ap.add_argument("filter", nargs="?", help="科目ディレクトリ名の部分一致フィルタ（例: 刑）")
    ap.add_argument("--summary", action="store_true", help="OK 行を省き、問題行と集計だけ表示")
    ap.add_argument("--strict", action="store_true", help="UNREACHABLE も exit 1 にする")
    ap.add_argument("--no-color", action="store_true", help="ANSI 色コードを出さない")
    ap.add_argument("--json", dest="json_path", help="RX coverage manifest JSON 出力先")
    return ap.parse_args(argv)


def main(argv=None):
    args = parse_args(sys.argv[1:] if argv is None else argv)
    set_color(not args.no_color and "NO_COLOR" not in os.environ)
    ...
    fail = grand["dangling"] > 0 or (strict and grand["unreachable"] > 0)
    if args.json_path:
        out = resolve_output_path(args.json_path)
        out.parent.mkdir(parents=True, exist_ok=True)
        payload = build_payload(filter_token, strict, checked, skipped, grand)
