...
def reflex_core_html(core):
    """Return Lexia final-table 5-line reflex core HTML.

    DATA.json may provide either the legacy string form or a dict:
      {"text": "...", "文言": "...", "趣旨": "...", "射程": "...", "切断点": "...", "転用": "..."}
    Legacy strings are kept in the visible ox-grid as-is, and are wrapped here with
    a conservative fallback so old specs remain runnable while new specs can supply
    full five-part content.
    """
    ...
    for n in nums:
        rows += f'''        <div class="ox-row mc" data-stmt="{n}">
          ...
          <div class="ox-main">
            ...
            <div class="ox-core-wrap"><span class="ox-core-tag">🔑 論点のコア</span><span class="ox-stmt">{core_text(blanks[n]["core"])}</span></div>
          </div>
...
'''
    verdict = ''
    for n in nums:
        verdict += f'            <tr data-stmt="{n}" data-verdict="o"><td style="text-align:center;font-weight:700;">{CIRC[n]}</td><td style="text-align:center;font-weight:800;color:var(--recall-correct-light);">○</td><td>{reflex_core_html(blanks[n]["core"])}</td></tr>\n'
...
{intent_html}{narrative_html}        <p class="fa-summary"><strong>正解</strong>　各空欄の正しい選択がすべて揃う配置＝<strong>組合せ{win}</strong>。組合せ番号を覚えるのではなく、下表の<strong>論点のコア</strong>（各空欄の決め手）を身につける。</p>
        <table class="statement-verdict-table" data-answer-key="{key}">
          <thead><tr><th style="width:3em;">空欄</th><th style="width:3.5em;">判定</th><th>{REFLEX_HEADER}</th></tr></thead>
          <tbody>
{verdict}          </tbody>
        </table>
      </div>
...
'''
    ...
    # navi エンジン
    engine_blanks = {k: {**v, 'core': core_text(v.get('core', ''))} for k, v in blanks.items()}
    eng = ENGINE.replace('__ORDER__', json.dumps(order, ensure_ascii=False)).replace('__WIN__', win).replace('__B__', json.dumps(engine_blanks, ensure_ascii=False))
    ...
    # TX-LEX 5点セット CSS（無ければ注入・冪等）
    if '.tx-reflex-core{' not in h:
        h = h.replace('</style>', REFLEX_CSS + '</style>', 1)
    # 物語解説 CSS（無ければ注入・冪等）
