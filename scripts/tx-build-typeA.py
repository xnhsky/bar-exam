...
"""
import json, re, sys, os, html as _html
...
# 意味ラベル（食い出しタブ）だけの追補 CSS。旧 CSS のみ持つ既存ファイルへ後から昇格注入する。
NARR_TAB_CSS = '''
...
def build(code, data):
    ...
    if nar:
        ...
        plist = nar.get('paras', [])
        labels = nar.get('labels') or []

        def _p(i, p):
            if isinstance(p, dict):
                txt = p.get('text', ''); lbl = (p.get('label') or '').strip()
            else:
                txt = p; lbl = (labels[i].strip() if i < len(labels) and labels[i] else '')
