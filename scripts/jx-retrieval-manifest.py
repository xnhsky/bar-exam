...
出力内容：
  ...
  - 今回のブランチで追加・変更された outputs/jx 配下の HTML 一覧（サイズ付き）
  - 各ファイルの GitHub blob URL（ブラウザでそのまま開ける／Raw でダウンロード可）
  - コピペ用の回収手順（git pull / 直接 DL）

使い方：
  python scripts/jx-retrieval-manifest.py                 # outputs/jx 全体
  python scripts/jx-retrieval-manifest.py --base origin/master
  python scripts/jx-retrieval-manifest.py --glob 'outputs/jx/刑JX/*.html'
...
def _keep(line):
    return line.startswith("outputs/jx/") and line.endswith(".html")
...
def main():
    ...
    ap.add_argument("--glob", default="outputs/jx/**/*.html", help="フォールバック glob")
