...
3. `check-tx-lex-engine.py outputs references`（TX360 inline _lex の canonical エンジン整合・G41）
4. `check-ariadne-canonical.py`
5. `check-rx-coverage.py --summary --strict`

> 3 の `check-tx-lex-engine.py` は、`*_lex.html` に `validate-tx-core.py` の G41 を横断適用し、
> 「旧 _lex ベース流用＋後付けパッチ script 接ぎ木」（旧 Annex C JS に `tx-inline-v1211-upgrade-js`
> を足した codex 366-385 型）を push 前に弾く。`.tx-inline-card` を持つ v12 正典のみ対象＝旧デザイン
> _lex と公式 000_TX は素通り。停止を回避するなら `--skip-tx-engine`。
> **さらに 3 の冒頭で `check-citation-era.py`（判例引用・元号割れ）を全 outputs へ走らせる**（下記）。
...
## 判例引用・元号割れゲート: check-citation-era.py（恒久対策・2026-07-09）

`validate-*.py` は 1 ファイル内検査で、**別ファイル同士で同じ判例を別元号で引く誤記**
（OCR/転記で 大↔昭↔明↔平 が入替わる。例：ある問で `大判大9.5.8`、別問で `大判昭9.5.8`）は
拾えない。これを横断検出するのが `check-citation-era.py`。

- 検出単位＝`(裁判所トークン, 数値年, 月, 日)`。元号（明/大/昭/平/令）の集合が 2 種以上＝「割れ」。
- **179 の教訓（偽陽性対策）**：同一日付でも別論点の別判例が実在する
  （`大判大10.10.24`＝業務妨害「業務」の定義／`大判昭10.10.24`＝従犯の共同正犯・刑集14輯1267頁）。
  よって割れ＝即誤記ではない。一次確認で「正当な別判例」と確定したペアは
  `scripts/citation-era-allowlist.txt` に `裁判所|年|月|日  # メモ` で登録して抑止する。
- 確定は判例百選・刑録/刑集の巻号頁・裁判所裁判例検索で行う（メモリ `feedback_cite_verify_method`）。
  誤記なら**可視ラベルのみ是正しアンカー id/href は不変**（内部キー）。

```
python scripts/check-citation-era.py            # 既定 outputs/ を走査
python scripts/check-citation-era.py --list      # 全引用の分布（棚卸し用）
```

未確認の割れがあれば exit 1。**`check-tx-lex-engine.py` の冒頭に組込み済み**なので
`check-lexia-preflight.py`（Lexia 同期前ゲート）でも自動的に走る。
...
差替スロットだが、カード幅（260px）・viewBox（幅1500）は固定のため、見出しが長い問題では
文字がカード枠・viewBox 右端をはみ出して切れる（SVG の `<text>` は自動改行・自動縮小しない）。
実害＝刑TX382 記述5「収得後知情行使は通貨のみ（152）」が viewBox 右端で切断。

さらに親ツリー（L1 カード）の見出し帯を「見出し fs18（x=-198）＋サブ見出し fs16（x=-120）」の
`<text>` 2本固定 x 並置で書いた問題では、見出しが約 78px を超えた分だけ2本目に**文字が重なる**
（実害＝刑TX395_lex「職務の適法性」×「要件・現在性」＝30px、刑TX125_lex は中央寄せ見出しがサブ文に
完全重畳、ほか計 10 ファイル/16 件・2026-07-16 監査で全是正）。

`tx-sysmap-fit.py` は**本文を1文字も変えず**、
①はみ出す `<text>` だけに `textLength`（カード内に収まる実効幅）＋`lengthAdjust="spacingAndGlyphs"`
を付与して字間・字形を圧縮し、枠内へ収める。
②重なりを含む sysmap の見出し帯 2 本組を正典形＝1本の `<text text-anchor="middle">`＋`<tspan>`
（v13k-bis `tx-lex-sysmap-center.py`／`tx-lex-sysmap-pbox.py` 正典ブロックと同形）へ統合する
（同じ svg 内の非衝突ペアも併せて統一＝親ツリー1行で中央寄せ・左寄せが混在しない）。

```
python scripts/tx-sysmap-fit.py <file...>          # 指定ファイルを修正（書込）
python scripts/tx-sysmap-fit.py --check <file...>  # 検出のみ（残存があれば exit 1）
python scripts/tx-sysmap-fit.py --all              # ux の *_lex.html ＋ 公式 000_TX の全 HTML
```

- **決定論・冪等・生テキスト編集・CRLF 保存**（全文シリアライズによる無関係差分・改行コード変換を
  出さない）。2 回目以降は無変更。
- 判定は単一情報源 **`scripts/tx_sysmap_geom.py`**（全角=1em/半角≈0.58em の保守モデル・tspan の
  font-size 尊重・重なりは `iter_text_collisions`／見出し帯ペアは `iter_header_pairs`）を、修正ツールと
  検証ゲート `validate-tx-core.py` **G66/G69** が共用（基準がズレない）。
- 三層防御＝①canonical `GENESIS-CARD.html`・placeholder 契約（見出し帯は 1 text＋tspan・2本並置禁止）／
  ②G66（viewBox 端クリップ=ERROR・カード枠越え=WARNING）＋G69（同一行の重なり 6px 超=ERROR・
  2〜6px=WARNING）／③push 前 `check-tx-lex-engine.py` に G66/G69 を組込み（クリップ・重なりは push を止める）。
- 公式（本物5択）も sysmap を共有するため `--all` の対象（実害＝刑TX293.html が公式側にも重なりを保持していた）。
