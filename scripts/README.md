...
## 体系マップSVG 文字はみ出し修正: tx-sysmap-fit.py（恒久対策・2026-07-13）
...
`tx-sysmap-fit.py` は**本文を1文字も変えず**、はみ出す `<text>` だけに `textLength`（カード内に
収まる実効幅）＋`lengthAdjust="spacingAndGlyphs"` を付与して字間・字形を圧縮し、枠内へ収める。
...
- **決定論・冪等・生テキスト編集**（全文シリアライズによる無関係差分を出さない）。2 回目以降は無変更。
- はみ出し判定は単一情報源 **`scripts/tx_sysmap_geom.py`**（全角=1em/半角≈0.58em の保守モデル・
  tspan の font-size 尊重）を、修正ツールと検証ゲート `validate-tx-core.py` **G66** が共用（基準がズレない）。
- 三層防御＝①canonical `GENESIS-CARD.html` の例見出しは枠内に収める／②G66（viewBox 端クリップ=ERROR・
  カード枠越え=WARNING）／③push 前 `check-tx-lex-engine.py` に G66 を組込み（viewBox クリップは push を止める）。
