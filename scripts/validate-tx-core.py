...
_TRAP_YOKO = (
    "横串", "cross-cut", "CROSS-CUT", "混同", "別の軸", "別次元", "別レイヤー", "別の論点", "別概念", "別問題",
...
class Validator:
    ...
    #       判定枠として機能する定型（〜との記述/とする記述/との判定/とする結論/
    #       との見解/との批判 等）は除外。ひらがな終わり（「〜は成立しない」等の用言終止・
    #       句点欠落のみ）は判定可能なので発火しない。
    #       「〜との内容/とする内容」は除外しない＝設問枠組み（語句選択の「第1語句の当否」等）の
    #       圧縮残りで、単独カードでは判定対象が読めない（実害：刑TX405/413・実機報告 2026-07-18。
    #       要件事実込みの断定命題へ書き換えて是正済み）。
    #   要件事実の充足（意味の自己完結）は機械化しない＝執筆規律（spec 第5-bis-2項）と G65 が担う。
    _G70_TAIL_PAREN_RE = re.compile(r"[（(【][^（）()【】]*[）)】]$")
    _G70_ALLOWED_TAIL_RE = re.compile(
        r"(?:との記述|とする記述|との判定|とする判定|との結論|とする結論|"
    ...
    def g70_ox_stmt_proposition_form(self):
        ...
        for row in self.soup.select(".ox-row[data-stmt]"):
            ...
            if body.endswith("か"):
                ...
                continue
            # 「〜との内容/とする内容」終わり＝設問枠組み（語句選択の「第1語句の当否」等）の圧縮残り。
            # 断定命題なら末尾は用言＋句点になるため誤検出は想定されず、句点の有無に関わらず ERROR で弾く
            # （実害：刑TX405/413＝旧・定型除外で素通りし一問一答カードが判定不能・実機報告 2026-07-18）。
            if body.endswith("との内容") or body.endswith("とする内容"):
                self.err("G70", f"記述{label}: ox-stmt が『〜との内容』型の枠組み断片＝単独カードで"
                                "判定対象が読めない。事案と主張を織り込んだ断定完結命題（文末は句点）"
                                "に書き換える（spec 第5-bis-2項・実害＝刑TX405/413）。")
                continue
            if text.endswith("。"):
                continue
    ...
    # 同じ中身を 結論／🧭現在地／場面→問題→判例・条文→理由／📘キーワード／🖼イメージ／判定 の面へ割る。
