...
def extract_divs(html, cls):
    ...
    return [(t, i) for _s, _e, t, i in extract_divs_pos(html, cls)]

def extract_divs_pos(html, cls):
    """extract_divs の位置付き版 → [(start, end, open_tag, inner_html)]。"""
    blocks = []
...
def main():
    ...
    cards_pos = extract_divs_pos(html, 'self-check-quiz')
    cards = [(t, i) for _s, _e, t, i in cards_pos]
    bc_spans = [(s, e) for s, e, _t, _i in extract_divs_pos(html, 'bc-wrap')]
    card_in_bc = [any(b0 < s < b1 for b0, b1 in bc_spans) for s, _e, _t, _i in cards_pos]
    ...
    a40_errors = []
    for ci, (tag, inner) in enumerate(cards):
        has_arena = 'data-arena="1"' in tag
        # A13: 解法ナビ側は data-arena 必須。bc-wrap（作法）はページ内専用＝対象外（v1.4.0）。
        if not has_arena and not card_in_bc[ci]: bad_arena += 1
        ...
        # A40: arena 純度（v1.4.0 ARENA-PURE）。
        if card_in_bc[ci] and has_arena:
            a40_errors.append(f'bc-wrap内ドリル{ci+1}に data-arena（作法ドリルはプール対象外）: {qtext[:28]}')
        elif has_arena and qtext and METHOD_RE.search(qtext):
            a40_errors.append(f'arenaドリル{ci+1}が科目共通の答案方法論（法的実体へ差替か削除）: {qtext[:36]}')

    (E if bad_arena else P)('A13', f'data-arena="1" 欠落 {bad_arena} 枚（解法ナビ側・欠落するとkind=self-checkで復習プール除外）' if bad_arena else '解法ナビ側カードは data-arena="1"（kind=arena でプール対象・bc-wrap作法ドリルは対象外）')
    ...
    # ---- A40 arena 純度（v1.4.0 ARENA-PURE・2026-07-11）----
    # arena（復習プール対象）に載せてよいのは当該問題の法的実体（規範・要件・判別・判例・条文）だけ。
    # 科目共通の答案方法論（ariadne_arena_rules.METHOD_RE）と bc-wrap 作法ドリルの data-arena を弾く。
    # 背景：74ファイル×平均6枚の同一方法論命題がプールへ流れていた（2026-07-11 監査）。
    if a40_errors:
        E('A40', f'arena純度違反 {len(a40_errors)} 枚: ' + '／'.join(a40_errors[:6]) + (f'／他{len(a40_errors)-6}件' if len(a40_errors) > 6 else ''))
    else:
    ...
    # ---- A27 答案構成の作法（教授のひとことコラム・spec §12・2026-06-22）----
    # v1.4.0 ARENA-PURE：作法ドリルの枚数要求は撤廃（旧「ドリル≥5」は全問同一の汎用命題を
    # プールへ量産させる強制側だった）。コラム5枚のみ必須。ドリルは任意のページ内確認
    # （data-arena 無し＝A40）で、枚数は情報表示にとどめる。
    bc = extract_divs(html, 'bc-wrap')
    if bc:
        body = bc[0][1]
        cols = body.count('class="bc-col"')          # 🎓 教授のひとこと（5ステップ）
        quizzes = body.count('self-check-quiz')        # ページ内確認ドリル（任意）
        if cols >= 5:
            P('A27', f'答案構成の作法あり（🎓教授のひとこと {cols}・ページ内ドリル {quizzes}＝任意）')
        else:
            W('A27', f'答案構成の作法が不完全（コラム {cols}・5想定・spec §12）')
    else:
