...
# -*- coding: utf-8 -*-
"""
8 テンプレの part_b 領域全体を {{PART_B_FRAME}} 化する（Phase 4-14、パターン E 応用 2 例目）。

part_b は check_template_sync.py の diff-allowed 領域、8 templates × 6 variants
（KTX/comb5 ・ msel5/sc5 ・ ox4 ・ fillin ・ ox3comb8 ・ fillin8）。本パッチは
scripts/render.py の render_part_b(instruction_type) が返す literal（{{CHOICE_X_*}} 等
slot placeholder を保持した完成形）を OLD として各 template から検出し、共通 NEW =
{{PART_B_FRAME}} に置換する。

β variant 別 OLD dispatch（Phase 4-6 TOC / Phase 4-9 pre_part_a / Phase 4-11
basis_secnav / Phase 4-12 part_a / Phase 4-13 a2 と同形）。OLD の生成元が render.py
単一であるため、二重管理リスクなし。

byte-identical 復元の前提:
  - render.py 側 render_part_b(instruction_type) が同じ instruction_type → literal
    対応で part_b HTML を組み立てる (Phase 4-14 Commit 2 で実装、全 8 templates で
    byte-identical 検証済)
  - 本スクリプトは render.py の render_part_b を直接 import して OLD を生成（
    対応表は持たず、render.py が source of truth）

dry-run:
  python scripts/upgrade_templates_part_b_slot.py --dry-run
apply:
  python scripts/upgrade_templates_part_b_slot.py
"""
import sys
...
from render import render_part_b  # noqa: E402
...
def main(argv: list[str]) -> int:
    ...
    print(f"=== upgrade_templates_part_b_slot.py {mode} ===")
