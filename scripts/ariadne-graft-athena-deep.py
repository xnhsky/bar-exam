...
.athena-graft .card,.athena-graft .ref-entry{background:#fff; border:1px solid #D6C9DC; border-left:5px solid #8e6e9a; border-radius:6px; padding:0 19px 16px; margin:14px 0; overflow:hidden; box-shadow:0 2px 8px rgba(70,58,80,.07); font-family:var(--f-body); font-size:.9rem; line-height:1.92; color:#2f2935}

/* カードヘッダ ＝ TX .basis-card-header（型別淡色帯・明朝） */
.athena-graft .ref-entry > h4{font-family:var(--f-disp); font-weight:700; font-size:1.05em; color:#2c2632; letter-spacing:.03em; margin:0 -19px 14px; padding:12px 19px; background:#f3f4f6; border-bottom:1.5px solid rgba(0,0,0,.12); display:flex; align-items:center; gap:9px; flex-wrap:wrap}
...
.athena-graft p{margin:.45em 0; text-indent:1em}

/* 条文文言（statute）＝ TX 条文引用風（白パネル・左罫・明朝） */
.athena-graft blockquote.statute{margin:8px 0 12px; padding:12px 16px; border-radius:8px; font-family:var(--f-disp); font-size:.95rem; line-height:1.95; color:#2f2935; background:rgba(255,255,255,.72); border:1px solid #d8d2dd; border-left:4px solid #6b7280}
.athena-graft blockquote.statute p{margin:0; text-indent:0}

/* 判旨（case）＝ TX .judgment-text（【判旨】ティールバッジ・明朝） */
.athena-graft blockquote.case{position:relative; margin:18px 0 12px; padding:14px 16px 12px; border-radius:8px; font-family:var(--f-disp); font-weight:600; font-size:.95rem; line-height:1.95; color:#2f2935; background:#fff; border:1px solid var(--a-mid); border-top:2px solid var(--a-mid)}
.athena-graft blockquote.case::before{content:"⚖ 判旨"; position:absolute; top:-12px; left:16px; font-family:var(--f-code); font-size:.7rem; font-weight:700; letter-spacing:.04em; color:#fff; background:var(--a-mid); padding:3px 12px; border-radius:3px; box-shadow:0 1px 3px rgba(0,0,0,.16)}
.athena-graft blockquote.case p{margin:0; text-indent:0}

/* テーブル ＝ TX 同一（border-collapse＋全セル罫線・violetヘッダ・ゼブラ）｜表頭=丸ゴ／本文=角ゴ */
.athena-graft table{width:100%; border-collapse:collapse; margin:9px 0 5px; font-size:.84rem; font-family:var(--f-body); background:#fff}
.athena-graft th,.athena-graft td{border:1px solid #cbbfd2; padding:8px 12px; vertical-align:top; text-align:left; line-height:1.7}
.athena-graft th{background:#8e6e9a; color:#fff; font-family:var(--f-soft); font-weight:700; letter-spacing:.04em; white-space:nowrap}

/* TX328 踏襲：ヘッダ型別アイコン */
.athena-graft section[id^="ref-stat"] > h4::before{content:"📜 "; font-family:var(--f-body)}
.athena-graft section[id^="ref-case"] > h4::before{content:"⚖ "; font-family:var(--f-body)}
...
.athena-graft section[id^="ref-doctrine"]{background:#E7F4D9; border-color:#BFE09C; border-left-color:#4E7536}
.athena-graft section[id^="ref-doctrine"] > h4{background:#D4EBBF}
.athena-graft section[id^="ref-doctrine"] th{background:#4E7536}
.athena-graft section[id^="ref-term"]{background:#FCF3C5; border-color:#EAD879; border-left-color:#7A6818}
.athena-graft section[id^="ref-term"] > h4{background:#F3E79E}
.athena-graft section[id^="ref-term"] th{background:#7A6818}
.athena-graft tr:nth-child(even) td{background:rgba(142,110,154,.08)}
...
.athena-graft .rank-A{color:#fff; background:var(--li); border:1.5px solid var(--li-deep)}
.athena-graft .rank-B{color:var(--li-deep); background:#b79cc2}
.athena-graft .rank-C{color:#3e6363; background:var(--li-soft)}
.athena-graft .tan{margin-left:4px}
.athena-graft .tan-super{color:#fff; background:var(--li); border:1.5px solid var(--li-deep)}
.athena-graft .tan-high{color:var(--li-deep); background:#b79cc2}
.athena-graft .tan-std{color:#3e6363; background:var(--li-soft)}
/* 本文蛍光（薄） */
